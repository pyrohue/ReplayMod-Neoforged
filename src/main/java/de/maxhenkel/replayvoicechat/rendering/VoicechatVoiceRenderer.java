...
public class VoicechatVoiceRenderer extends Thread
{
	...
	public static void onStartRendering()
	{
		...
		{
			ReplayVoicechat.LOGGER
				.warn("Started rendering when already rendering");
		}
	}
			...
			try
			{
				Vec3d cameraLocation =
					MinecraftClient.getInstance().gameRenderer.getCamera()
						.getPos();
				float yrot = MinecraftClient.getInstance().gameRenderer
					.getCamera().getPitch();
				...
				client.getTalkCache().updateTalking(packet.getPacketId(),
					packet instanceof EntitySoundPacket p && p.isWhispering());
				if(ReplayInterface.INSTANCE
					.isPlayerHidden(packet.getPacketId()))
				{
					return;
				}
				...
				{
					INSTANCE.packets
						.put(new PacketWrapper(packet,
							ReplayInterface.INSTANCE.videoRenderer
								.getVideoTime(),
							yrot, cameraLocation,
							ReplayInterface.getCurrentSpeed()));
				}
			}catch(InterruptedException e)
			{
				ReplayVoicechat.LOGGER.error("Failed to handle sound packet",
					e);
			}
		}
	}
	...
	private VoicechatVoiceRenderer()
	{
		...
		setUncaughtExceptionHandler((thread, throwable) -> {
			ReplayVoicechat.LOGGER.error("Error in voice render thread",
				throwable);
		});
	}
	...
	{
		String videoFileName = ReplayInterface.INSTANCE.videoRenderer
			.getRenderSettings().getOutputFile().getName();
		String filename =
			videoFileName.substring(0, videoFileName.lastIndexOf('.'));
		Path path = ReplayInterface.INSTANCE.videoRenderer.getRenderSettings()
			.getOutputFile().toPath().getParent().resolve(filename + "_audio");
	...
	public void run()
	{
		while(this.running || packets.size() > 0)
		{
			...
			{
				ReplayVoicechat.LOGGER
					.error("Failed to poll voice chat packets", e);
			}
	...
	private void onLocationalSoundPacket(PacketWrapper packetWrapper,
		LocationalSoundPacket locationalSoundPacket)
	{
		try
		{
			Position location = locationalSoundPacket.getLocation();
			recorder.appendChunk(locationalSoundPacket.getPacketId(),
				packetWrapper.timestamp,
				PositionalAudioUtils.convertToStereoForRecording(
					locationalSoundPacket.getDistance(),
					packetWrapper.cameraPos, packetWrapper.yrot,
					new Vec3d(location.getX(), location.getY(),
						location.getZ()),
					setSpeed(locationalSoundPacket.getPacketId(),
						locationalSoundPacket.getRawAudio(),
						packetWrapper.speed)));
		}catch(IOException e)
		{
			ReplayVoicechat.LOGGER.error("Failed to render locational sound",
				e);
		}
	}

	private void onEntitySoundPacket(PacketWrapper packetWrapper,
		EntitySoundPacket entitySoundPacket)
	{
		try
		{
			if(MC.world == null)
			...
			@Nullable
			PlayerEntity player =
				MC.world.getPlayerByUuid(entitySoundPacket.getPacketId());
			if(player == null)
			{
				return;
			}
			Vec3d pos = player.getEyePos();
			recorder.appendChunk(entitySoundPacket.getPacketId(),
				packetWrapper.timestamp,
				PositionalAudioUtils.convertToStereoForRecording(
					entitySoundPacket.getDistance(), packetWrapper.cameraPos,
					packetWrapper.yrot, pos,
					setSpeed(entitySoundPacket.getPacketId(),
	...
	private void onStaticSoundPacket(PacketWrapper packetWrapper,
		StaticSoundPacket staticSoundPacket)
	{
		try
		{
			recorder.appendChunk(staticSoundPacket.getPacketId(),
				packetWrapper.timestamp,
				PositionalAudioUtils
					.convertToStereo(setSpeed(staticSoundPacket.getPacketId(),
						staticSoundPacket.getRawAudio(), packetWrapper.speed)));
		}catch(IOException e)
		{
	...
	public static class PacketWrapper
	{
		...
		public PacketWrapper(AbstractSoundPacket<?> packet, int timestamp,
			float yrot, Vec3d cameraPos, double speed)
		{
			this.packet = packet;
