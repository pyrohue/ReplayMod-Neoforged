...
public class VoicechatRecorder
{
	...
	public static void onEntitySound(ClientReceiveSoundEvent.EntitySound event)
	{
		send(new EntitySoundPacket(event.getId(), event.getRawAudio(),
			event.isWhispering(), event.getDistance()));
	}

	public static void onLocationalSound(
		ClientReceiveSoundEvent.LocationalSound event)
	{
		send(new LocationalSoundPacket(event.getId(), event.getRawAudio(),
			event.getPosition(), event.getDistance()));
	}
	...
	public static void onSound(ClientSoundEvent event)
	{
		...
		}else
		{
			send(new EntitySoundPacket(id, rawAudio, event.isWhispering(),
				(float)ReplayVoicechatPlugin.CLIENT_API
	...
	public static void send(Packet<?> packet)
	{
		...
		ReplayInterface.INSTANCE
			.sendFakePacket(new CustomPayloadS2CPacket(packet)); // this needs
																	// testing
	}
