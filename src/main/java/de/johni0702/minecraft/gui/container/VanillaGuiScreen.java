...
public class VanillaGuiScreen extends GuiScreen
	implements Draggable, Typeable, Scrollable, Tickable
{

	private static final Map<net.minecraft.client.gui.screen.Screen, VanillaGuiScreen> WRAPPERS =
		Collections.synchronizedMap(new WeakHashMap<>());

	public static VanillaGuiScreen wrap(
		net.minecraft.client.gui.screen.Screen originalGuiScreen)
	{
		VanillaGuiScreen gui = WRAPPERS.get(originalGuiScreen);
		if(gui == null)
		{
			WRAPPERS.put(originalGuiScreen,
	...
	@Deprecated
	public static VanillaGuiScreen setup(
		net.minecraft.client.gui.screen.Screen originalGuiScreen)
	{
	...
	// Needs to be called from or after GuiInitEvent.Post, will auto-unregister
	// on any GuiOpenEvent
	public void register()
	{
		if(!eventHandler.active)
		{
			...
			getSuperMcGui().init(MCVer.getMinecraft(), mcScreen.width,
				mcScreen.height);
		}
	}
	...
	public void setBackground(Background background)
	{
		throw new UnsupportedOperationException(
			"Cannot set background of vanilla gui screen.");
	}
	...
	public void tick()
	{
		// TODO this is a workaround for ReplayMod#560 until we remove the inner
		// mc screen
		// see also the note in ReplayMod's GuiBackgroundProcesses
		// If this screen ever becomes the main screen, something has gone
		// wrong.
	...
	// #endif
	{
		...
		{
			stack.draw(); // flush any buffered changes before we draw using
							// legacy primitives
			// #else
			// $$ private void onGuiRender(MatrixStack stack, float
			// partialTicks) {
			// #endif
		...
		@Override
		public boolean mouseDrag(double x, double y, int button, double dx,
			double dy)
		{
		...
		@Override
		public boolean mouseScroll(double x, double y, double horizontal,
			double vertical)
		{
		...
		// $$ @SubscribeEvent(priority = EventPriority.LOWEST)
		// #if MC>=10800
		// $$ public void onKeyboardInput(GuiScreenEvent.KeyboardInputEvent.Pre
		// event) throws IOException {
		// #else
		// $$ public void onKeyboardInput(KeyboardInputEvent event) throws
		// IOException {
		// #endif
		// $$ handled = true;
