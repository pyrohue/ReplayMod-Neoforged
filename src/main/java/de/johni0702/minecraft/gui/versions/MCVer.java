...
public class MCVer
{
	...
	{
		private static final ScissorBounds DISABLED =
			new ScissorBounds(0, 0, Integer.MAX_VALUE, Integer.MAX_VALUE);
		private final int x;
		private final int y;
	...
	private static final ArrayDeque<ScissorBounds> scissorStateStack =
		new ArrayDeque<>();
	...
	private static void setScissorBounds(ScissorBounds newState)
	{
		...
		if(isEnabled)
		{
			...
			GL11.glScissor(scissorState.x, scissorState.y, scissorState.width,
				scissorState.height);
		}else
		{
	...
	public static void addDetail(CrashReportSection category, String name,
	...
	{
		drawRect(left, top, right - left, bottom - top, ReadableColor.WHITE,
			ReadableColor.WHITE, ReadableColor.WHITE, ReadableColor.WHITE);
	}

	public static void drawRect(int x, int y, int width, int height,
		ReadableColor tl, ReadableColor tr, ReadableColor bl, ReadableColor br)
	{
		// #if MC>=12100
		VertexConsumerProvider.Immediate provider =
			getMinecraft().getBufferBuilders().getEntityVertexConsumers();
		VertexConsumer vertexConsumer =
			provider.getBuffer(RenderLayer.getGui());
		vertexConsumer.vertex(x, y + height, 0).color(bl.getRed(),
			bl.getGreen(), bl.getBlue(), bl.getAlpha());
		vertexConsumer.vertex(x + width, y + height, 0).color(br.getRed(),
			br.getGreen(), br.getBlue(), br.getAlpha());
		vertexConsumer.vertex(x + width, y, 0).color(tr.getRed(), tr.getGreen(),
			tr.getBlue(), tr.getAlpha());
		vertexConsumer.vertex(x, y, 0).color(tl.getRed(), tl.getGreen(),
			tl.getBlue(), tl.getAlpha());
		provider.draw();
		// #else
		// $$ //#if MC>=10800
		...
		// $$ //#if MC>=10809
		// $$ //#if MC>=11700
		// $$ vertexBuffer.begin(VertexFormat.DrawMode.QUADS,
		// VertexFormats.POSITION_COLOR);
		// $$ //#else
		// $$ //$$ vertexBuffer.begin(GL11.GL_QUADS,
		// VertexFormats.POSITION_COLOR);
		// $$ //#endif
		// $$ vertexBuffer.vertex(x, y + height, 0).color(bl.getRed(),
		// bl.getGreen(), bl.getBlue(), bl.getAlpha()).next();
		// $$ vertexBuffer.vertex(x + width, y + height, 0).color(br.getRed(),
		// br.getGreen(), br.getBlue(), br.getAlpha()).next();
		// $$ vertexBuffer.vertex(x + width, y, 0).color(tr.getRed(),
		// tr.getGreen(), tr.getBlue(), tr.getAlpha()).next();
		// $$ vertexBuffer.vertex(x, y, 0).color(tl.getRed(), tl.getGreen(),
		// tl.getBlue(), tl.getAlpha()).next();
		// $$ //#else
		// $$ //$$ vertexBuffer.startDrawingQuads();
		// $$ //$$ vertexBuffer.setColorRGBA(bl.getRed(), bl.getGreen(),
		// bl.getBlue(), bl.getAlpha());
		// $$ //$$ vertexBuffer.addVertex(x, y + height, 0);
		// $$ //$$ vertexBuffer.setColorRGBA(br.getRed(), br.getGreen(),
		// br.getBlue(), br.getAlpha());
		// $$ //$$ vertexBuffer.addVertex(x + width, y + height, 0);
		// $$ //$$ vertexBuffer.setColorRGBA(tr.getRed(), tr.getGreen(),
		// tr.getBlue(), tr.getAlpha());
		// $$ //$$ vertexBuffer.addVertex(x + width, y, 0);
		// $$ //$$ vertexBuffer.setColorRGBA(tl.getRed(), tl.getGreen(),
		// tl.getBlue(), tl.getAlpha());
	...
	// #if FABRIC<=0
	// $$ public static RenderGameOverlayEvent.ElementType
	// getType(RenderGameOverlayEvent event) {
	// #if MC>=10904
	...
	// $$ public static void tryBlendFuncSeparate(int l, int r, int vl, int vr)
	// { OpenGlHelper.glBlendFunc(l, r, vl, vr); }
	// $$ public static void shadeModel(int mode) { GL11.glShadeModel(mode); }
	// $$ public static void enableColorLogic() {
	// GL11.glEnable(GL_COLOR_LOGIC_OP); }
	// $$ public static void disableColorLogic() {
	// GL11.glDisable(GL_COLOR_LOGIC_OP); }
	...
	public static abstract class Keyboard
	{
		...
		{
			// #if MC>=11903
			// These are now always enabled and we no longer need to manually
			// toggle it when opening screens
			// #else
