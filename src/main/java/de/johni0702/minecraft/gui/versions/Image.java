...
/**
 * As of LWJGL 3, AWT must never be initialized, otherwise GLFW will be broken
 * on OSX.
...
public class Image implements AutoCloseable
{
	...
	{
		// Great, now we're using a language with GC but still need to take care
		// of memory management.. thanks MC
		close();
		super.finalize();
	}
	...
	public void setRGBA(int x, int y, int r, int g, int b, int a)
	{
		...
		inner.setColor(x, y, ((a & 0xff) << 24) | ((b & 0xff) << 16)
			| ((g & 0xff) << 8) | (r & 0xff));
	}
	...
	{
		// Minecraft as of 1.20.4 artificially limits itself to images with
		// valid PNG header.
		...
		if(bytes[0] != (byte)0x89 || bytes[1] != 0x50 || bytes[2] != 0x4E
			|| bytes[3] != 0x47)
		{
	...
	public Image scaledSubRect(int x, int y, int width, int height,
		int scaledWidth, int scaledHeight)
	{
		NativeImage dst = new NativeImage(inner.getFormat(), scaledWidth,
			scaledHeight, false);
