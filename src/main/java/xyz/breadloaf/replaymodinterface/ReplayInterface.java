...
public class ReplayInterface
{
	...
	{
		return ((ConnectionEventHandlerAccessor)ReplayModRecording.instance
			.getConnectionEventHandler()).getRecordingEventHandler() != null;
	}
	...
	public void sendFakePacket(Packet<?> packet)
	{
		if(ReplayModRecording.instance.getConnectionEventHandler() != null)
		{
			ReplayModRecording.instance.getConnectionEventHandler()
				.getPacketListener().save(packet);
		}
	}

	public boolean isPlayerHidden(UUID uuid)
	{
		Optional<PlayerOverview> playerOverview =
			ReplayModExtras.instance.get(PlayerOverview.class);
		return playerOverview.map(overview -> overview.isHidden(uuid))
			.orElse(false);
	}
	...
	public static double getCurrentSpeed()
	{
		...
		Timeline tl =
			ReplayModSimplePathing.instance.getCurrentTimeline().getTimeline();
		...
		if(!(ReplayModSimplePathing.instance
			.getGuiPathing() instanceof GuiPathingAccessor guiPathingAccessor))
		{
			return 1D;
		}
		...
		{
			currentRealtime =
				ReplayInterface.INSTANCE.videoRenderer.getVideoTime();
		}else
		{
		...
		for(Path path : paths)
		{
			...
			for(Keyframe keyframe : keyframes)
			{
				...
				TimeKeyframe current =
					new TimeKeyframe(realtimeKeyframe, mcKeyframe);

				if(realtimeKeyframe >= currentRealtime)
				{
					if(last == null)
					{
						// Time will stand still because we aren't past our
						// first time keyframe
						return 0D;
					}else
					{
						...
						double realtimeDuration =
							realtimeKeyframe - last.realtimeKeyframe;
