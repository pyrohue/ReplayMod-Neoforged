...
public abstract class AbstractTimelinePlayer extends EventRegistrations
{
	...
	public ListenableFuture<Void> start(Timeline timeline)
	{
		...
		Iterator<Keyframe> iter =
			Iterables.concat(Iterables.transform(timeline.getPaths(),
		...
		}else
		{
			...
			{
				@Override
				public int compare(@Nullable Keyframe left,
					@Nullable Keyframe right)
				{
					assert left != null;
					assert right != null;
	...
	public void onTick()
	{
		...
		// Apply a second time in case same of the packets have moved the camera
		// from where it was
		timeline.applyToGame(time, replayHandler);
