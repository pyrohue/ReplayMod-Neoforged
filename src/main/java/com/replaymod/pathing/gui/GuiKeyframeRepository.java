...
public class GuiKeyframeRepository extends GuiScreen
	implements Closeable, Typeable
{
	private static final Logger LOGGER = LogManager.getLogger();

	public final GuiPanel contentPanel =
		new GuiPanel(this).setBackgroundColor(Colors.DARK_TRANSPARENT);
	public final GuiLabel title = new GuiLabel(contentPanel)
		.setI18nText("replaymod.gui.keyframerepository.title");
	public final GuiVerticalList list = new GuiVerticalList(contentPanel)
	...
	public final GuiButton overwriteButton =
			...
			public void run()
			{
				GuiYesNoPopup
					.open(GuiKeyframeRepository.this,
						new GuiLabel()
							.setI18nText("replaymod.gui.keyframerepo.overwrite")
		...
		}).setSize(75, 20).setI18nLabel("replaymod.gui.overwrite")
			.setDisabled();
	public final GuiButton saveAsButton = new GuiButton().onClick(new Runnable()
	{
		...
		public void run()
		{
			final GuiTextField nameField =
				new GuiTextField().setSize(200, 20).setFocused(true);
			final GuiYesNoPopup popup = GuiYesNoPopup
				.open(GuiKeyframeRepository.this,
					new GuiLabel().setI18nText("replaymod.gui.saveas")
						.setColor(Colors.BLACK),
					nameField)
				.setYesI18nLabel("replaymod.gui.save")
			...
			}).onTextChanged(new Consumer<String>()
			{
				...
				{
					popup.getYesButton()
						.setEnabled(!nameField.getText().isEmpty()
	...
	public final GuiButton loadButton = new GuiButton().onClick(new Runnable()
	{
		...
		public void run()
		{
			...
			{
				Timeline timeline =
					timelines.get(selectedEntries.iterator().next().name);
	...
	public final GuiButton renameButton = new GuiButton().onClick(new Runnable()
	{
		...
		public void run()
		{
			...
			final GuiTextField nameField = new GuiTextField().setSize(200, 20)
				.setFocused(true).setText(selectedEntry.name);
			final GuiYesNoPopup popup = GuiYesNoPopup
				.open(GuiKeyframeRepository.this,
					new GuiLabel().setI18nText("replaymod.gui.rename")
						.setColor(Colors.BLACK),
					nameField)
				.setYesI18nLabel("replaymod.gui.done")
				.setNoI18nLabel("replaymod.gui.cancel");
			popup.getYesButton().setDisabled();
			((VerticalLayout)popup.getInfo().getLayout()).setSpacing(7);
			nameField.onEnter(new Runnable()
		...
		public void run()
		{
			...
			try
			{
				TimelineSerialization serialization =
					new TimelineSerialization(registry, null);
				setClipboardString(serialization.serialize(toBeSerialized));
			}catch(Throwable t)
			{
				t.printStackTrace();
				CrashReport report =
					CrashReport.create(t, "Copying timeline(s)");
	...
	public final GuiButton pasteButton = new GuiButton().onClick(new Runnable()
	{
		...
		public void run()
		{
			try
			{
				TimelineSerialization serialization =
					new TimelineSerialization(registry, null);
				for(Map.Entry<String, Timeline> entry : serialization
					.deserialize(getClipboardString()).entrySet())
				{
			...
			{
				// Intentionally not making a fuzz about it cause this will
				// likely happen when they have anything
				// else in their clipboard.
				// If it's actually not working, they'll go complain and we'll
				// just check the log.
				t.printStackTrace();
			}
		}
	...
	public final GuiButton addToQueueButton =
		new GuiButton().onClick(new Runnable()
		{
			...
			public void run()
			{
				...
				GuiRenderQueue queue = new GuiRenderQueue(
					GuiKeyframeRepository.this, replayHandler, null);
				queue.open();

				Set<String> selected = selectedEntries.stream().map(e -> e.name)
					.collect(Collectors.toSet());
				ArrayDeque<Map.Entry<String, Timeline>> toBeAdded =
					new ArrayDeque<>();
				// Iterating over timelines to get consistent ordering (cause
				// selectedEntries is an unordered set)
				...
				new Runnable()
				{
					...
					public void run()
					{
						Map.Entry<String, Timeline> entry =
							toBeAdded.pollFirst();
						if(entry == null)
						{
							return;
						...
						settingsGui.buttonPanel
							.removeElement(settingsGui.renderButton);
						settingsGui.setOutputFileBaseName(name);
						Runnable orgOnClick =
							settingsGui.queueButton.getOnClick();
	...
	public final GuiPanel buttonPanel = new GuiPanel(contentPanel)
		...
		.addElements(null, overwriteButton, saveAsButton, renameButton,
			removeButton, loadButton, addToQueueButton, copyButton,
			pasteButton);
...

	{
		...
		setLayout(new CustomLayout<GuiScreen>()
		{
			...
			{
				pos(contentPanel, width / 2 - width(contentPanel) / 2,
					height / 2 - height(contentPanel) / 2);
			}
		});
		contentPanel.setLayout(new CustomLayout<GuiPanel>()
		{
			...
			protected void layout(GuiPanel container, int width, int height)
			{
				...
				size(list, width, height - 10 - height(buttonPanel) - 10
					- y(title) - height(title) - 5);
				pos(list, width / 2 - width(list) / 2,
					y(title) + height(title) + 5);
				pos(buttonPanel, width / 2 - width(buttonPanel) / 2,
					y(list) + height(list) + 10);
			}
			...
			public ReadableDimension calcMinSize(GuiContainer<?> container)
			{
				...
				return new Dimension(screenSize.getWidth() - 10,
					screenSize.getHeight() - 10);
			}
		});
	}

	public GuiKeyframeRepository(PathingRegistry registry,
		ReplayFile replayFile, Timeline currentTimeline) throws IOException
	{
		...
		for(Map.Entry<String, Timeline> entry : timelines.entrySet())
		{
			if(entry.getKey().isEmpty())
				continue; // don't show auto-save slot
	...
	public void save()
	{
		...
		{
			e.printStackTrace();
			ReplayMod.instance.printWarningToChat(
				"Error saving timelines: " + e.getMessage());
		}
	}
	...
	public class Entry extends AbstractGuiClickableContainer<Entry>
	{
		...
		public Entry(String name)
		{
			...
			setLayout(new CustomLayout<Entry>()
			{
				...
				public ReadableDimension calcMinSize(GuiContainer<?> container)
				{
					return new Dimension(buttonPanel.calcMinSize().getWidth(),
						16);
				}
			});
			label.setText(name);
			...
			RenderInfo renderInfo)
		{
			if(selectedEntries.contains(this))
			{
				renderer.drawRect(0, 0, size.getWidth(), size.getHeight(),
					Colors.BLACK);
