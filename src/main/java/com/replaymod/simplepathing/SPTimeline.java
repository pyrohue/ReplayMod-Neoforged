...
public class SPTimeline implements PathingRegistry
{
	...
	{
		Preconditions.checkState(this.entityTracker == null,
			"Entity tracker already set");
		this.entityTracker = entityTracker;
	}

	public void setDefaultInterpolatorType(
		InterpolatorType defaultInterpolatorType)
	{
		Validate.isTrue(defaultInterpolatorType != InterpolatorType.DEFAULT,
			"Must not be DEFAULT");
		InterpolatorType prevType = this.defaultInterpolatorType;
		this.defaultInterpolatorType =
			Validate.notNull(defaultInterpolatorType);
	...
	{
		Preconditions.checkState(defaultInterpolatorType != null,
			"Default interpolator type not set.");
		Validate.isInstanceOf(defaultInterpolatorType.getInterpolatorClass(),
			interpolator);

		registerPositionInterpolatorProperties(interpolator);

		Change change = CombinedChange.create(positionPath.getSegments()
			.stream()
			// Ignore explicitly set segments
			.filter(s -> !s.getStartKeyframe()
				.getValue(ExplicitInterpolationProperty.PROPERTY).isPresent())
			// Ignore spectator segments
			.filter(s -> !isSpectatorSegment(s))
			// Update interpolator for every remaining segment
			// This will create a fragmented interpolator which is split by the
			// updateInterpolators call
			.map(s -> SetInterpolator.create(s, interpolator))
			.toArray(Change[]::new));
	...
	public boolean isSpectatorKeyframe(long time)
	{
		...
		return keyframe != null
			&& keyframe.getValue(SpectatorProperty.PROPERTY).isPresent();
	}

	public void addPositionKeyframe(long time, double posX, double posY,
		double posZ, float yaw, float pitch, float roll, int spectated)
	{
		LOGGER.debug(
			"Adding position keyframe at {} pos {}/{}/{} rot {}/{}/{} entId {}",
			time, posX, posY, posZ, yaw, pitch, roll, spectated);

		Path path = positionPath;

		Preconditions.checkState(positionPath.getKeyframe(time) == null,
		...
		UpdateKeyframeProperties.Builder builder =
			UpdateKeyframeProperties.create(path, keyframe);
		builder.setValue(CameraProperties.POSITION,
			Triple.of(posX, posY, posZ));
		builder.setValue(CameraProperties.ROTATION,
			Triple.of(yaw, pitch, roll));
		if(spectated != -1)
		{
		...
		// while this is overkill, it is far simpler than updating differently
		// for every possible case
		change =
			CombinedChange.createFromApplied(change, updateInterpolators());
	...
	public Change updatePositionKeyframe(long time, double posX, double posY,
		double posZ, float yaw, float pitch, float roll)
	{
		LOGGER.debug(
			"Updating position keyframe at {} to pos {}/{}/{} rot {}/{}/{}",
	...
	public void addTimeKeyframe(long time, int replayTime)
	{
		...
		Preconditions.checkState(path.getKeyframe(time) == null,
			"Keyframe already exists");
		...
		UpdateKeyframeProperties updateChange =
			UpdateKeyframeProperties.create(path, keyframe)
	...
	{
		LOGGER.debug("Updating time keyframe at {} to time {}", time,
			replayTime);
	...
	{
		LOGGER.debug(
			"Setting interpolator of position keyframe at {} to the default",
			time);
	...
	{
		LOGGER.debug("Setting interpolator of position keyframe at {} to {}",
			time, interpolator);

		Keyframe keyframe = positionPath.getKeyframe(time);
		Preconditions.checkState(keyframe != null, "Keyframe does not exists");
		PathSegment segment = positionPath.getSegments().stream()
			.filter(s -> s.getStartKeyframe() == keyframe).findFirst()
			.orElseThrow(() -> new IllegalStateException(
				"Keyframe has no following segment."));

		registerPositionInterpolatorProperties(interpolator);

		Change change = CombinedChange.create(
			UpdateKeyframeProperties.create(positionPath, keyframe)
				.setValue(ExplicitInterpolationProperty.PROPERTY,
					ObjectUtils.NULL)
				.done(),
			SetInterpolator.create(segment, interpolator));
		change.apply(timeline);
	...
	{
		LOGGER.debug("Moving keyframe on {} from {} to {}", spPath, oldTime,
			newTime);
		Path path = getPath(spPath);
		Keyframe keyframe = path.getKeyframe(oldTime);

		Preconditions.checkState(keyframe != null,
			"No keyframe at specified time");

		// Interpolator of the first segment might be required later if it is
		// the only one
		Optional<Interpolator> firstInterpolator = path.getSegments().stream()
			.findFirst().map(PathSegment::getInterpolator);

		// The interpolator that will be lost once we remove the old keyframe
		// and has to be restored afterwards
		Optional<Interpolator> lostInterpolator =
			path.getSegments().stream().filter(s -> {
				// If this is the last keyframe,
				if(Iterables.getLast(path.getKeyframes()) == keyframe)
				{
					return s.getEndKeyframe() == keyframe; // the previous
															// interpolator will
															// be lost
				}else
				{ // otherwise
					return s.getStartKeyframe() == keyframe; // the following
																// interpolator
																// will be lost
				}
			}).findFirst().map(PathSegment::getInterpolator);
		...
		UpdateKeyframeProperties.Builder builder =
			UpdateKeyframeProperties.create(path, path.getKeyframe(newTime));
		...
		{ // Unless this is the last keyframe
			// the interpolator of the following segment has been lost and needs
			// to be restored
			restoreInterpolatorChange =
				lostInterpolator
					.<Change> flatMap(interpolator -> path.getSegments()
						.stream().filter(s -> s.getStartKeyframe() == newKf)
						.findFirst().map(segment -> SetInterpolator
							.create(segment, interpolator)))
					.orElseGet(CombinedChange::create);
		}else
		{ // If it is the last keyframe however,
			// the interpolator of the previous segment has been lost and needs
			// to be restored
			restoreInterpolatorChange = path.getSegments().stream()
				.filter(s -> s.getEndKeyframe() == newKf).findFirst()
				.flatMap(segment -> lostInterpolator.map(interpolator -> {
					// additionally, if the interpolation of this keyframe was
					// set to explicit, that property
					// has to be transferred to the start keyframe of the new
					// segment
					if(newKf.getValue(ExplicitInterpolationProperty.PROPERTY)
						.isPresent())
					{
						return CombinedChange.create(
							SetInterpolator.create(segment, interpolator),
							UpdateKeyframeProperties
								.create(path, segment.getStartKeyframe())
								.setValue(
									ExplicitInterpolationProperty.PROPERTY,
									ObjectUtils.NULL)
								.done());
					}else
					{
		...
		{
			// Time keyframes only need updating when only one segment of them
			// exists
			if(path.getSegments().size() == 1)
			{
				assert firstInterpolator
					.isPresent() : "One segment should have existed before as well";
				interpolatorUpdateChange =
					SetInterpolator.create(path.getSegments().iterator().next(),
						firstInterpolator.get());
			}else
			{
				interpolatorUpdateChange = CombinedChange.create(); // Noop
																	// change
			}
		...
		return CombinedChange.createFromApplied(removeChange, addChange,
			propertyChange, restoreInterpolatorChange, interpolatorUpdateChange,
			spectatorChange);
	}

	// Helper method because generics cannot be defined on blocks
	private <T> void copyProperty(Property<T> property, Keyframe from,
		UpdateKeyframeProperties.Builder to)
	{
		from.getValue(property)
			.ifPresent(value -> to.setValue(property, value));
	}

	private Change updateInterpolators()
	{
		Collection<PathSegment> pathSegments = positionPath.getSegments();
		// Contains updated pathsegment-interpolator mappings, may be changed
		// multiple times over the course of this method
		...
		for(PathSegment segment : pathSegments)
		{
			...
			{
				// If the last segment was a spectator segment, then use its
				// interpolator for this segment as well
				...
				// Now that we have an interpolator, set it for the current
				// segment
				updates.put(segment, interpolator);
			}else
			{
				// Not a spectator segment, make sure we don't re-use the last
				// interpolator (it would not be continuous)
				interpolator = null;
			}
		}
		...
		pathSegments.stream()
			...
			// Ignore spectator segments
			.filter(s -> !isSpectatorSegment(s))
			// Ignore already correct segments, should ignore all if default
			// hasn't changed
			.filter(s -> !s.getInterpolator().getClass()
				.equals(defaultInterpolatorType.getInterpolatorClass()))
			// Finally, set the interpolators
			// This will create one interpolator per segment, they will be
			// merged later
			.forEach(
				segment -> updates.put(segment, createDefaultInterpolator()));

		// All interpolators should now be set appropriately but may still be
		// fragmented.
		// Cleaning that up is a two step process

		// Firstly, all interpolators that are not continuous need to be split
		// up
		Interpolator lastInterpolator = null;
		Set<Interpolator> used =
		...
		for(PathSegment segment : pathSegments)
		{
			...
			Interpolator currentInterpolator =
				updates.getOrDefault(segment, segment.getInterpolator());
			...
			{
				// It has been used before, we need to clone it and use the
				// clone instead
				// This will create a new interpolator for each segment which
		...
		// Secondly, all neighbouring interpolators that are equal need to be
		// merged
		lastInterpolator = null;
		...
		for(PathSegment segment : pathSegments)
		{
			...
			if(lastInterpolator != currentInterpolator
				&& serialized.equals(lastInterpolatorSerialized))
			{
				// This interpolator is equal to the last one but not the same,
				// needs merging
				updates.put(segment, lastInterpolator);
				continue;
			}
			// New interpolator, not related to the previous one, merged by
			// definition
			lastInterpolator = currentInterpolator;
		...
		Change change = CombinedChange.create(updates.entrySet().stream()
			.map(e -> SetInterpolator.create(e.getKey(), e.getValue()))
			.toArray(Change[]::new));
		change.apply(timeline);
		return change;
	}
	...
	{
		return segment.getStartKeyframe().getValue(SpectatorProperty.PROPERTY)
			.isPresent()
			&& segment.getEndKeyframe().getValue(SpectatorProperty.PROPERTY)
				.isPresent();
	}
	...
	private Change updateSpectatorPositions()
	{
		...
		{
			Optional<Integer> spectator =
				keyframe.getValue(SpectatorProperty.PROPERTY);
			if(spectator.isPresent())
			{
				Optional<Integer> time = timePath
					.getValue(TimestampProperty.PROPERTY, keyframe.getTime());
				if(!time.isPresent())
				{
					continue; // No time keyframes set at this video time,
								// cannot determine replay time
				}
				Location expected = entityTracker
					.getEntityPositionAtTimestamp(spectator.get(), time.get());
				if(expected == null)
				{
					continue; // We don't have any data on this entity for some
								// reason
				}
				Triple<Double, Double, Double> pos =
					keyframe.getValue(CameraProperties.POSITION)
						.orElse(Triple.of(0D, 0D, 0D));
				Triple<Float, Float, Float> rot =
					keyframe.getValue(CameraProperties.ROTATION)
						.orElse(Triple.of(0F, 0F, 0F));
				Location actual = new Location(pos.getLeft(), pos.getMiddle(),
					pos.getRight(), rot.getLeft(), rot.getRight());
				if(!expected.equals(actual))
				{
					changes.add(
						UpdateKeyframeProperties.create(positionPath, keyframe)
							.setValue(CameraProperties.POSITION,
								Triple.of(expected.getX(), expected.getY(),
									expected.getZ()))
							.setValue(CameraProperties.ROTATION, Triple
								.of(expected.getYaw(), expected.getPitch(), 0f))
							.done());
				}
			}
		}
		return CombinedChange
			.create(changes.toArray(new Change[changes.size()]));
	}

	private Interpolator createDefaultInterpolator()
	{
		return registerPositionInterpolatorProperties(
			defaultInterpolatorType.newInstance());
	}

	private Interpolator registerPositionInterpolatorProperties(
	...
	@Override
	public void serializeInterpolator(JsonWriter writer,
		Interpolator interpolator) throws IOException
	{
		...
		}else if(interpolator instanceof CatmullRomSplineInterpolator)
		{
			...
			writer.name("alpha")
				.value(((CatmullRomSplineInterpolator)interpolator).getAlpha());
			writer.endObject();
		}else
		{
	...
	@Override
	public Interpolator deserializeInterpolator(JsonReader reader)
		throws IOException
	{
		String type;
		JsonObject args;
		...
		switch(type)
		{
			...
			if(args == null || !args.has("alpha"))
			{
				throw new IOException(
					"Missing alpha value for catmull-rom-spline.");
			}
			return new CatmullRomSplineInterpolator(
				args.get("alpha").getAsDouble());
			default:
	...
	private String serializeInterpolator(Interpolator interpolator)
	{
		...
		{
			CrashReport crash =
				CrashReport.create(e, "Serializing interpolator");
			CrashReportSection category =
				crash.addElement("Serializing interpolator");
	...
	private Interpolator deserializeInterpolator(String json)
	{
		...
		{
			CrashReport crash =
				CrashReport.create(e, "De-serializing interpolator");
			CrashReportSection category =
				crash.addElement("De-serializing interpolator");
