...
public abstract class GuiEditKeyframe<T extends GuiEditKeyframe<T>>
	extends AbstractGuiPopup<T> implements Typeable
{
	private static GuiNumberField newGuiNumberField()
	{
		return new GuiNumberField().setPrecision(0)
	...
	public final GuiPanel timePanel = new GuiPanel()
		.setLayout(new HorizontalLayout(HorizontalLayout.Alignment.RIGHT)
			.setSpacing(3))
		.addElements(new HorizontalLayout.Data(0.5),
			new GuiLabel().setI18nText(
				"replaymod.gui.editkeyframe.timelineposition"),
			timeMinField, new GuiLabel().setI18nText("replaymod.gui.minutes"),
			timeSecField, new GuiLabel().setI18nText("replaymod.gui.seconds"),
			timeMSecField,
			new GuiLabel().setI18nText("replaymod.gui.milliseconds"));

	public final GuiButton saveButton =
		new GuiButton().setSize(150, 20).setI18nLabel("replaymod.gui.save");

	public final GuiButton cancelButton = new GuiButton().onClick(this::close)
...

	{
		...
		popup.setLayout(new VerticalLayout().setSpacing(10)).addElements(
			new VerticalLayout.Data(0.5, false), title, inputs, timePanel,
			buttons);
	}
	...
	public GuiEditKeyframe(GuiPathing gui, SPPath path, long time, String type)
	{
		...
		Consumer<String> updateSaveButtonState =
			s -> saveButton.setEnabled(canSave());
		...
		saveButton.onClick(() -> {
			Change change = save();
			long newTime =
				(timeMinField.getInteger() * 60 + timeSecField.getInteger())
					* 1000 + timeMSecField.getInteger();
			if(newTime != time)
			{
				change = CombinedChange.createFromApplied(change, gui.getMod()
					.getCurrentTimeline().moveKeyframe(path, time, newTime));
				if(gui.getMod().getSelectedPath() == path
					&& gui.getMod().getSelectedTime() == time)
				{
	...
	{
		long newTime =
			(timeMinField.getInteger() * 60 + timeSecField.getInteger()) * 1000
				+ timeMSecField.getInteger();
	...
	public static class Time extends GuiEditKeyframe<Time>
	{
...

		{
			inputs
				...
				.addElements(new HorizontalLayout.Data(0.5),
					new GuiLabel()
						.setI18nText("replaymod.gui.editkeyframe.timestamp"),
					timestampMinField,
					new GuiLabel().setI18nText("replaymod.gui.minutes"),
					timestampSecField,
					new GuiLabel().setI18nText("replaymod.gui.seconds"),
					timestampMSecField,
					new GuiLabel().setI18nText("replaymod.gui.milliseconds"));
		}
		...
		public Time(GuiPathing gui, SPPath path, long keyframe)
		{
			...
			this.keyframe.getValue(TimestampProperty.PROPERTY)
				.ifPresent(time -> {
			...
			link(timestampMinField, timestampSecField, timestampMSecField,
				timeMinField, timeSecField, timeMSecField);
		...
		protected Change save()
		{
			int time = (timestampMinField.getInteger() * 60
				+ timestampSecField.getInteger()) * 1000
				+ timestampMSecField.getInteger();
			return guiPathing.getMod().getCurrentTimeline()
				.updateTimeKeyframe(keyframe.getTime(), time);
		}
	...
	public static class Position extends GuiEditKeyframe<Position>
	{
		public final GuiNumberField xField =
			newGuiNumberField().setSize(60, 20).setPrecision(5);
		...
		{
			GuiPanel positionInputs =
				new GuiPanel()
					.setLayout(new GridLayout().setCellsEqualSize(false)
						.setColumns(4).setSpacingX(3).setSpacingY(5))
					.addElements(new GridLayout.Data(1, 0.5),
						new GuiLabel()
							.setI18nText("replaymod.gui.editkeyframe.xpos"),
						xField,
						new GuiLabel()
							.setI18nText("replaymod.gui.editkeyframe.camyaw"),
						yawField,
						new GuiLabel()
							.setI18nText("replaymod.gui.editkeyframe.ypos"),
						yField,
						new GuiLabel()
							.setI18nText("replaymod.gui.editkeyframe.campitch"),
						pitchField,
						new GuiLabel()
							.setI18nText("replaymod.gui.editkeyframe.zpos"),
						zField, new GuiLabel().setI18nText(
							"replaymod.gui.editkeyframe.camroll"),
						rollField);

			inputs.setLayout(new VerticalLayout().setSpacing(10)).addElements(
				new VerticalLayout.Data(0.5, false), positionInputs,
				interpolationPanel);
		}
		...
		public Position(GuiPathing gui, SPPath path, long keyframe)
		{
			...
			link(xField, yField, zField, yawField, pitchField, rollField,
				timeMinField, timeSecField, timeMSecField);
		...
		protected Change save()
		{
			...
			Change positionChange =
				timeline.updatePositionKeyframe(time, xField.getDouble(),
					yField.getDouble(), zField.getDouble(), yawField.getFloat(),
					pitchField.getFloat(), rollField.getFloat());
			if(interpolationPanel.getSettingsPanel() == null)
			{
				// The last keyframe doesn't have interpolator settings because
				// there is no segment following it
				return positionChange;
			}
			Interpolator interpolator =
				interpolationPanel.getSettingsPanel().createInterpolator();
			if(interpolationPanel
				.getInterpolatorType() == InterpolatorType.DEFAULT)
			{
				return CombinedChange.createFromApplied(positionChange,
					timeline.setInterpolatorToDefault(time),
			...
			extends AbstractGuiContainer<InterpolationPanel>
		{
			...
			public InterpolationPanel()
			{
				...
				dropdown = new GuiDropdownMenu<InterpolatorType>()
					...
					.onSelection(
						i -> setSettingsPanel(dropdown.getSelectedValue()));

				// set hover tooltips
				for(Map.Entry<InterpolatorType, IGuiClickable> e : dropdown
					.getDropdownEntries().entrySet())
				{
					e.getValue().setTooltip(new GuiTooltip()
						.setI18nText(e.getKey().getI18nDescription()));
				}

				GuiPanel dropdownPanel = new GuiPanel()
					.setLayout(new GridLayout().setCellsEqualSize(false)
						.setColumns(2).setSpacingX(3).setSpacingY(5))
					.addElements(new GridLayout.Data(1, 0.5),
						new GuiLabel().setI18nText(
							"replaymod.gui.editkeyframe.interpolator"),
						dropdown);
				...
				if(segment.isPresent())
				{
					...
					InterpolatorType type =
						InterpolatorType.fromClass(interpolator.getClass());
					if(keyframe.getValue(ExplicitInterpolationProperty.PROPERTY)
						.isPresent())
					{
						dropdown.setSelected(type); // trigger the callback once
													// to display settings panel
					}else
					{
						setSettingsPanel(InterpolatorType.DEFAULT);
						type = InterpolatorType.DEFAULT;
					}
					if(getInterpolatorTypeNoDefault(type).getInterpolatorClass()
						.isInstance(interpolator))
					{
			...
			protected InterpolatorType getInterpolatorTypeNoDefault(
				InterpolatorType interpolatorType)
			{
				if(interpolatorType == InterpolatorType.DEFAULT
					|| interpolatorType == null)
				{
					InterpolatorType defaultType = InterpolatorType.fromString(
						guiPathing.getMod().getCore().getSettingsRegistry()
							.get(Setting.DEFAULT_INTERPOLATION));
					return defaultType;
				}
			...
			public class CatmullRomSettingsPanel extends
				SettingsPanel<CatmullRomSplineInterpolator, CatmullRomSettingsPanel>
			{
				public final GuiLabel alphaLabel =
					new GuiLabel().setColor(Colors.BLACK).setI18nText(
						"replaymod.gui.editkeyframe.interpolator.catmullrom.alpha");
				public final GuiNumberField alphaField =
					new GuiNumberField().setSize(100, 20).setPrecision(5)
						.setMinValue(0).setValidateOnFocusChange(true);

				{
					setLayout(new HorizontalLayout(
						HorizontalLayout.Alignment.CENTER));
					addElements(new HorizontalLayout.Data(0.5), alphaLabel,
						alphaField);
				}

				@Override
				public void loadSettings(
					CatmullRomSplineInterpolator interpolator)
				{
			...
			public class CubicSettingsPanel extends
				SettingsPanel<CubicSplineInterpolator, CubicSettingsPanel>
			{
