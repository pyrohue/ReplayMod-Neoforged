...
public class GuiKeyframeTimeline
	extends AbstractGuiTimeline<GuiKeyframeTimeline> implements Draggable
{
	...
	/**
	 * The keyframe (time on timeline) that was last clicked on using the left
	 * mouse button.
	 */
	...
	/**
	 * Whether we have surpassed the initial threshold and are actually dragging
	 * the keyframe.
	 */
	...
	/**
	 * Change caused by dragging. Whenever the user moves the keyframe further,
	 * the previous change is undone
	 * and a new one is created. This way when the mouse is released, only one
	 * change is in the undo history.
	 */
	...
	@Override
	protected void drawTimelineCursor(GuiRenderer renderer,
		ReadableDimension size)
	{
		...
		timeline.getTimeline().getPaths().stream()
			.flatMap(path -> path.getKeyframes().stream()).forEach(keyframe -> {
				if(keyframe.getTime() >= startTime
					&& keyframe.getTime() <= endTime)
				{
					double relativeTime = keyframe.getTime() - startTime;
					int positonX = BORDER_LEFT
						+ (int)(relativeTime / visibleTime * visibleWidth)
						- KEYFRAME_SIZE / 2;
					int u = KEYFRAME_TEXTURE_X
						+ (mod.isSelected(keyframe) ? KEYFRAME_SIZE : 0);
					int v = KEYFRAME_TEXTURE_Y;
					if(keyframe.getValue(CameraProperties.POSITION).isPresent())
					{
						if(keyframe.getValue(SpectatorProperty.PROPERTY)
							.isPresent())
						{
							v += 2 * KEYFRAME_SIZE;
						}
						renderer.drawTexturedRect(positonX, BORDER_TOP, u, v,
							KEYFRAME_SIZE, KEYFRAME_SIZE);
					}
					Optional<Integer> timeProperty =
						keyframe.getValue(TimestampProperty.PROPERTY);
					if(timeProperty.isPresent())
					{
						v += KEYFRAME_SIZE;
						renderer.drawTexturedRect(positonX,
							BORDER_TOP + KEYFRAME_SIZE, u, v, KEYFRAME_SIZE,
							KEYFRAME_SIZE);

						GuiMarkerTimeline replayTimeline = gui.overlay.timeline;
						GuiKeyframeTimeline keyframeTimeline = this;

						ReadableDimension replayTimelineSize =
							replayTimeline.getLastSize();
						ReadableDimension keyframeTimelineSize =
							this.getLastSize();
						if(replayTimelineSize == null
							|| keyframeTimelineSize == null)
						{
							return;
						}
						...
						replayTimeline.getContainer().convertFor(replayTimeline,
							replayTimelinePos);
						keyframeTimeline.getContainer()
							.convertFor(keyframeTimeline, keyframeTimelinePos);
						replayTimelinePos.setLocation(-replayTimelinePos.getX(),
							-replayTimelinePos.getY());
						keyframeTimelinePos.setLocation(
							-keyframeTimelinePos.getX(),
							-keyframeTimelinePos.getY());

						int replayTimelineLeft = replayTimelinePos.getX();
						int replayTimelineRight = replayTimelinePos.getX()
							+ replayTimelineSize.getWidth();
						int replayTimelineTop = replayTimelinePos.getY();
						int replayTimelineBottom = replayTimelinePos.getY()
							+ replayTimelineSize.getHeight();
						int replayTimelineWidth = replayTimelineRight
							- replayTimelineLeft - BORDER_LEFT - BORDER_RIGHT;

						int keyframeTimelineLeft = keyframeTimelinePos.getX();
						int keyframeTimelineTop = keyframeTimelinePos.getY();

						float positionXReplayTimeline =
							BORDER_LEFT + timeProperty.get()
								/ (float)replayTimeline.getLength()
								* replayTimelineWidth;
						float positionXKeyframeTimeline =
							positonX + KEYFRAME_SIZE / 2f;

						final int color = 0xff0000ff;
						Tessellator tessellator = Tessellator.getInstance();
						// #if MC>=12100
						BufferBuilder buffer = tessellator.begin(
							net.minecraft.client.render.VertexFormat.DrawMode.LINE_STRIP,
							VertexFormats.LINES);
						// #else
						// $$ BufferBuilder buffer = tessellator.getBuffer();
						// $$
						// buffer.begin(net.minecraft.client.render.VertexFormat.DrawMode.LINE_STRIP,
						// VertexFormats.LINES);
						// #endif

						// Start just below the top border of the replay
						// timeline
						Vector2f p1 = new Vector2f(
							replayTimelineLeft + positionXReplayTimeline,
							replayTimelineTop + BORDER_TOP);
						// Draw vertically over the replay timeline, including
						// its bottom border
						Vector2f p2 = new Vector2f(
							replayTimelineLeft + positionXReplayTimeline,
							replayTimelineBottom);
						// Now for the important part: connecting to the
						// keyframe timeline
						Vector2f p3 = new Vector2f(
							keyframeTimelineLeft + positionXKeyframeTimeline,
							keyframeTimelineTop);
						// And finally another vertical bit (the timeline is
						// already crammed enough, so only the border)
		...
		{
			if(segment.getInterpolator() == null || !segment.getInterpolator()
				.getKeyframeProperties().contains(SpectatorProperty.PROPERTY))
			{
				continue; // Not a spectator segment
			}
			drawQuadOnSegment(renderer, visibleWidth, segment, BORDER_TOP + 1,
				0xFF0088FF);
		}

		// Draw red quads on time path segments that would require time going
		// backwards
		for(PathSegment segment : timeline.getTimePath().getSegments())
		{
			long startTimestamp =
				segment.getStartKeyframe().getValue(TimestampProperty.PROPERTY)
					.orElseThrow(IllegalStateException::new);
			long endTimestamp =
				segment.getEndKeyframe().getValue(TimestampProperty.PROPERTY)
					.orElseThrow(IllegalStateException::new);
			if(endTimestamp >= startTimestamp)
			{
				continue; // All is fine, time is not moving backwards
			}
			drawQuadOnSegment(renderer, visibleWidth, segment,
				BORDER_TOP + KEYFRAME_SIZE + 1, 0xFFFF0000);
		}

		super.drawTimelineCursor(renderer, size);
	}

	private void drawQuadOnSegment(GuiRenderer renderer, int visibleWidth,
		PathSegment segment, int y, int color)
	{
		int startTime = getOffset();
		int visibleTime = (int)(getZoom() * getLength());
		int endTime = getOffset() + visibleTime;
		...
		int startX = BORDER_LEFT
			+ Math.max(0, (int)(relativeStart / visibleTime * visibleWidth)
				+ KEYFRAME_SIZE / 2 + 1);
		int endX = BORDER_LEFT + Math.min(visibleWidth,
			(int)(relativeEnd / visibleTime * visibleWidth)
				- KEYFRAME_SIZE / 2);
		if(startX < endX)
		{
			renderer.drawRect(startX + 1, y, endX - startX - 2,
				KEYFRAME_SIZE - 2, color);
		}
	}
	...
	private Pair<SPPath, Long> getKeyframe(ReadablePoint position)
	{
		...
		if(time != -1)
		{
			...
			if(mouseY > BORDER_TOP && mouseY < BORDER_TOP + 2 * KEYFRAME_SIZE)
			{
				...
				int visibleTime = (int)(getZoom() * getLength());
				int tolerance = visibleTime * KEYFRAME_SIZE
					/ (getLastSize().getWidth() - BORDER_LEFT - BORDER_RIGHT)
					/ 2;
				Optional<Keyframe> keyframe = gui.getMod().getCurrentTimeline()
					.getPath(path).getKeyframes().stream()
					.filter(k -> Math.abs(k.getTime() - time) <= tolerance)
					.sorted(
						Comparator.comparing(k -> Math.abs(k.getTime() - time)))
					.findFirst();
				return Pair.of(path,
					keyframe.map(Keyframe::getTime).orElse(null));
			}
		}
	...
	public boolean mouseClick(ReadablePoint position, int button)
	{
		...
		if(pathKeyframePair.getRight() != null)
		{
			...
			{ // Left click
				...
				{
					// Clicked the same keyframe again, potentially a double
					// click
			...
			{ // Right click
				Keyframe keyframe = gui.getMod().getCurrentTimeline()
					.getKeyframe(path, keyframeTime);
		...
		}else if(time != -1)
		{
			...
			{ // Right click
				...
				{
					// Apply the value of the clicked path at the clicked
					// position
					Path path = gui.getMod().getCurrentTimeline()
						.getPath(pathKeyframePair.getLeft());
					path.getKeyframes().stream()
						.flatMap(k -> k.getProperties().stream()).distinct()
						.forEach(p -> applyPropertyToGame(p, path, time));
				}
			}
			return true;
		}
	...
	private <T> void applyPropertyToGame(Property<T> property, Path path,
		long time)
	{
		Optional<T> value = path.getValue(property, time);
		if(value.isPresent())
		{
			property.applyToGame(value.get(),
				ReplayModReplay.instance.getReplayHandler());
		}
	}

	// Helper method because generics cannot be defined on blocks
	private <T> void applyPropertyToGame(Property<T> property,
		Keyframe keyframe)
	{
	...
	@Override
	public boolean mouseDrag(ReadablePoint position, int button,
		long timeSinceLastCall)
	{
		if(!dragging)
		{
			if(button == 0)
			{
				// Left click, the user might try to move the cursor by clicking
				// and holding
				int time = getTimeAt(position.getX(), position.getY());
				if(time != -1)
				{
					// and they are still on the timeline, so update the time
					// appropriately
					setCursorPosition(time);
					return true;
				}
		...
		if(actuallyDragging)
		{
			if(!gui.loadEntityTracker(
				() -> mouseDrag(position, button, timeSinceLastCall)))
				return true;
			// Threshold passed
			...
			double segmentTime =
				segmentLength * (mouseX - BORDER_LEFT) / bodyWidth;
			int newTime = Math.min(
				Math.max((int)Math.round(getOffset() + segmentTime), 0),
				getLength());
			if(newTime < 0)
			{
				return true;
			...
			// If there already is a keyframe at the target time, then increase
			// the time by one until there is none
	...
	public boolean mouseRelease(ReadablePoint position, int button)
	{
		if(dragging)
		{
			if(actuallyDragging)
			{
				gui.getMod().getCurrentTimeline().getTimeline()
					.pushChange(draggingChange);
				draggingChange = null;
