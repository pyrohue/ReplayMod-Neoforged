...
public class GuiPathing
{
	...
	}.setSize(20, 20).setTexture(ReplayMod.TEXTURE, ReplayMod.TEXTURE_SIZE)
	...
	public final GuiButton renderButton = new GuiButton().onClick(new Runnable()
	{
		...
		public void run()
		{
			...
			new GuiRenderQueue(screen, replayHandler,
				() -> preparePathsForPlayback(false))
			{
				@Override
		...
		.setSpriteUV(40, 0).setTooltip(new GuiTooltip()
			.setI18nText("replaymod.gui.ingame.menu.renderpath"));
	...
	public final GuiButton positionKeyframeButton = new GuiButton()
	{
		...
		public GuiElement getTooltip(RenderInfo renderInfo)
		{
			...
			{
				String label;
				if(getSpriteUV().getY() == 40)
				{ // Add keyframe
					if(getSpriteUV().getX() == 0)
					{ // Position
				...
				tooltip.setText(I18n.translate(label) + " ("
					+ mod.keyPositionKeyframe.getBoundKey() + ")");
			}
			return tooltip;
		}
	}.setSize(20, 20).setTexture(ReplayMod.TEXTURE, ReplayMod.TEXTURE_SIZE)
	...
	{
		@Override
		public void draw(GuiRenderer renderer, ReadableDimension size,
	...
	public final GuiHorizontalScrollbar scrollbar =
		new GuiHorizontalScrollbar().setSize(Integer.MAX_VALUE, 9);
	{
		scrollbar.onValueChanged(new Runnable()
		{
			...
			{
				timeline.setOffset(
					(int)(scrollbar.getPosition() * timeline.getLength()));
	...
	public final GuiTimelineTime<GuiKeyframeTimeline> timelineTime =
		new GuiTimelineTime<GuiKeyframeTimeline>().setTimeline(timeline);

	public final GuiButton zoomInButton =
		new GuiButton().setSize(9, 9).onClick(new Runnable()
		{
			@Override
			public void run()
			{
			...
			.setSpriteUV(40, 20).setTooltip(new GuiTooltip()
				.setI18nText("replaymod.gui.ingame.menu.zoomin"));
	...
	public final GuiButton zoomOutButton =
		new GuiButton().setSize(9, 9).onClick(new Runnable()
		{
			...
			.setSpriteUV(40, 30).setTooltip(new GuiTooltip()
				.setI18nText("replaymod.gui.ingame.menu.zoomout"));

	public final GuiPanel zoomButtonPanel = new GuiPanel()
		.setLayout(
			new VerticalLayout(VerticalLayout.Alignment.CENTER).setSpacing(2))
		.addElements(null, zoomInButton, zoomOutButton);

	public final GuiPanel timelinePanel = new GuiPanel()
		.setSize(Integer.MAX_VALUE, 40).setLayout(new CustomLayout<GuiPanel>()
		{
			@Override
	...
	public final GuiPanel panel = new GuiPanel().setLayout(
		new HorizontalLayout(HorizontalLayout.Alignment.CENTER).setSpacing(5))
		.addElements(new HorizontalLayout.Data(0.5), playPauseButton,
			renderButton, positionKeyframeButton, timeKeyframeButton,
			timelinePanel);
	...
	// Whether any error which occured during entity tracker loading has already
	...
	public GuiPathing(final ReplayMod core, final ReplayModSimplePathing mod,
		final ReplayHandler replayHandler)
	{
		...
		timeline.setLength(
			core.getSettingsRegistry().get(Setting.TIMELINE_LENGTH) * 1000);
		...
		}).onClick(new Runnable()
		{
			...
			public void run()
			{
				...
				{
					boolean ignoreTimeKeyframes =
						Keyboard.isKeyDown(Keyboard.KEY_LSHIFT);

					Timeline timeline =
						preparePathsForPlayback(ignoreTimeKeyframes)
							.okOrElse(err -> {
					...
					// Start from cursor time unless the control key is pressed
					// (then start from beginning)
					int startTime = Keyboard.isKeyDown(Keyboard.KEY_LCONTROL)
						? 0 : GuiPathing.this.timeline.getCursorPosition();
					ListenableFuture<Void> future =
						player.start(timeline, startTime);
					...
					Futures.addCallback(future, new FutureCallback<Void>()
					{
						...
						public void onSuccess(@Nullable Void result)
						{
							...
							{
								core.printInfoToChat(
									"replaymod.chat.pathinterrupted");
							}else
							{
								core.printInfoToChat(
									"replaymod.chat.pathfinished");
							}
		...
		positionKeyframeButton.setSpriteUV(new ReadablePoint()
		{
			...
			public int getX()
			{
				...
				if(keyframePath != SPPath.POSITION)
				{
					...
					keyframePath = mod.getCurrentTimeline().isPositionKeyframe(
						keyframeTime) ? SPPath.POSITION : null;
				}
				if(keyframePath != SPPath.POSITION)
				{
				...
				{
					return mod.getCurrentTimeline()
						.isSpectatorKeyframe(keyframeTime) ? 40 : 0;
				}
			}
			...
			public int getY()
			{
				...
				if(keyframePath != SPPath.POSITION)
				{
					// No keyframe selected but there might be one at exactly
					// the position of the cursor
					keyframePath = mod.getCurrentTimeline().isPositionKeyframe(
						timeline.getCursorPosition()) ? SPPath.POSITION : null;
				}
			...
			public int getY()
			{
				...
				if(keyframePath != SPPath.TIME)
				{
					...
					keyframePath = mod.getCurrentTimeline().isTimeKeyframe(
						timeline.getCursorPosition()) ? SPPath.TIME : null;
				}
		...
		overlay
			.setLayout(new CustomLayout<GuiReplayOverlay>(overlay.getLayout())
			{
				@Override
				protected void layout(GuiReplayOverlay container, int width,
					int height)
				{
					checkForAutoSync();
					pos(panel, 10,
						y(overlay.topPanel) + height(overlay.topPanel) + 3);
	...
	private void abortPathPlayback()
	{
		...
		// Tear down of the player might only happen the next tick after it was
		// cancelled
		player.onTick();
	}
	...
	public void keyframeRepoButtonPressed()
	{
		...
		try
		{
			TimelineSerialization serialization =
				new TimelineSerialization(mod.getCurrentTimeline(), null);
			String serialized = serialization.serialize(Collections
				.singletonMap("", mod.getCurrentTimeline().getTimeline()));
			Timeline currentTimeline =
				serialization.deserialize(serialized).get("");
			GuiKeyframeRepository gui =
				new GuiKeyframeRepository(mod.getCurrentTimeline(),
					replayHandler.getReplayFile(), currentTimeline);
			Futures.addCallback(gui.getFuture(), new FutureCallback<Timeline>()
			{
				...
				{
					t.printStackTrace();
					core.printWarningToChat(
						"Error loading timeline: " + t.getMessage());
				}
			}, Runnable::run);
			gui.display();
		}catch(IOException e)
	...
	{
		GuiYesNoPopup
			.open(replayHandler.getOverlay(),
				new GuiLabel().setI18nText("replaymod.gui.clearcallback.title")
	...
	private Integer computeSyncTime(int cursor)
	{
		...
		Keyframe keyframe = mod.getCurrentTimeline().getTimePath()
			.getKeyframes().stream().filter(it -> it.getTime() <= cursor)
			.reduce((__, last) -> last).orElse(null);
		if(keyframe != null)
		{
			// Cursor position at the keyframe
			int keyframeCursor = (int)keyframe.getTime();
			// Replay time at the keyframe
			// This is a keyframe from the time path, so it _should_ always have
			// a time property
			int keyframeTime =
				keyframe.getValue(TimestampProperty.PROPERTY).get();
			// Replay time passed
			int timePassed = time - keyframeTime;
			// Speed (set to 1 when shift is held)
			double speed = Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) ? 1
				: replayHandler.getOverlay().getSpeedSliderValue();
			// Cursor time passed
	...
	public void syncTimeButtonPressed()
	{
		...
		while(true)
		{
			...
			if(updatedCursor == null)
			{
				...
				// Let's just use the last value we got, this shouldn't happen
				// with ordinary timelines anyway.
				break;
			}
			...
			if(updatedCursor < cursor)
			{
				// We've gone backwards, we'll likely get stuck in a loop, so
	...
	private void startLoadingEntityTracker()
	{
		...
		new Thread(() -> {
			EntityPositionTracker tracker =
				new EntityPositionTracker(replayHandler.getReplayFile());
			try
			{
				...
				logger.info("Loaded entity tracker in "
					+ (System.currentTimeMillis() - start) + "ms");
			}catch(Throwable e)
			{
				logger.error("Loading entity tracker:", e);
				mod.getCore().runLater(() -> {
					mod.getCore().printWarningToChat(
						"Error loading entity tracker: %s",
	...
	private Result<Timeline, String[]> preparePathsForPlayback(
		boolean ignoreTimeKeyframes)
	{
		...
		{
			TimelineSerialization serialization =
				new TimelineSerialization(spTimeline, null);
			String serialized = serialization.serialize(
				Collections.singletonMap("", spTimeline.getTimeline()));
		...
		{
			error(LOGGER, replayHandler.getOverlay(),
				CrashReport.create(t, "Cloning timeline"), () -> {});
			return Result.err(null);
		}
	}

	private String[] validatePathsForPlayback(SPTimeline timeline,
		boolean ignoreTimeKeyframes)
	{
		...
		{
			int time = keyframe.getValue(TimestampProperty.PROPERTY)
				.orElseThrow(IllegalStateException::new);
			if(time < lastTime)
			{
				// We are going backwards in time
				return new String[]{"replaymod.error.negativetime1",
	...
	public boolean loadEntityTracker(Runnable thenRun)
	{
		if(entityTracker == null && !errorShown)
		{
			...
			LoadEntityTrackerPopup popup =
				new LoadEntityTrackerPopup(replayHandler.getOverlay());
			entityTrackerLoadingProgress =
				p -> popup.progressBar.setProgress(p.floatValue());
			Futures.addCallback(entityTrackerFuture, new FutureCallback<Void>()
			{
				...
				public void onFailure(@Nonnull Throwable t)
				{
					...
					{
						String message =
							"Failed to load entity tracker, spectator keyframes will be broken.";
						GuiReplayOverlay overlay = replayHandler.getOverlay();
						Utils.error(LOGGER, overlay,
							CrashReport.create(t, message), () -> {
								popup.close();
								thenRun.run();
							});
						errorShown = true;
	 ...
	 *
	 * @param path
	 *            {@code TIME} for the time property button, {@code POSITION}
	 *            for the place property button
	 * @param neverSpectator
	 *            when true, will insert a position keyframe even when currently
	...
	public void toggleKeyframe(SPPath path, boolean neverSpectator)
	{
		...
		if(timeline.getPositionPath().getKeyframes().isEmpty()
			&& timeline.getTimePath().getKeyframes().isEmpty() && time > 1000)
		{
			String text = I18n.translate(
				"replaymod.gui.ingame.first_keyframe_not_at_start_warning");
		...
		switch(path)
		{
			...
			{
				LOGGER.debug(
					"Selected keyframe is time keyframe -> removing keyframe");
				timeline.removeTimeKeyframe(mod.getSelectedTime());
				mod.setSelected(null, 0);
			}else if(timeline.isTimeKeyframe(time))
			{
				LOGGER.debug(
					"Keyframe at cursor position is time keyframe -> removing keyframe");
				timeline.removeTimeKeyframe(time);
				mod.setSelected(null, 0);
			}else
			{
				LOGGER.debug("No time keyframe found -> adding new keyframe");
				timeline.addTimeKeyframe(time,
					replayHandler.getReplaySender().currentTimeStamp());
			...
			{
				LOGGER.debug(
					"Selected keyframe is position keyframe -> removing keyframe");
				timeline.removePositionKeyframe(mod.getSelectedTime());
				mod.setSelected(null, 0);
			}else if(timeline.isPositionKeyframe(time))
			{
				LOGGER.debug(
					"Keyframe at cursor position is position keyframe -> removing keyframe");
				timeline.removePositionKeyframe(time);
				mod.setSelected(null, 0);
			}else
			{
				LOGGER
					.debug("No position keyframe found -> adding new keyframe");
				CameraEntity camera = replayHandler.getCameraEntity();
				int spectatedId = -1;
				if(!replayHandler.isCameraView() && !neverSpectator)
				{
					spectatedId = replayHandler.getOverlay().getMinecraft()
						.getCameraEntity().getId();
				}
				timeline.addPositionKeyframe(time, camera.getX(), camera.getY(),
					camera.getZ(), camera.getYaw(), camera.getPitch(),
					camera.roll, spectatedId);
				mod.setSelected(path, time);
			}
			break;
		}
	...
	public void openEditKeyframePopup(SPPath path, long time)
	{
		...
		{
			new GuiEditKeyframe.Spectator(this, path, keyframe.getTime())
				.open();
	...
	private class LoadEntityTrackerPopup
		extends AbstractGuiPopup<LoadEntityTrackerPopup>
	{
		private final GuiProgressBar progressBar = new GuiProgressBar(popup)
			.setSize(300, 20).setI18nLabel("replaymod.gui.loadentitytracker");
