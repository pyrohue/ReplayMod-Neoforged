...
public abstract class Mixin_Omnidirectional_Rotation
{
	...
	private Matrix4f replayModRender_setupCubicFrameRotation(Matrix4f matrix)
	{
		...
		// #endif
		if(getHandler() != null
			&& getHandler().data instanceof CubicOpenGlFrameCapturer.Data)
		{
			CubicOpenGlFrameCapturer.Data data =
				(CubicOpenGlFrameCapturer.Data)getHandler().data;
			float angle = 0;
			float x = 0;
			float y = 0;
			switch(data)
			...
			// org.joml.Quaternionf().fromAxisAngleDeg(new Vector3f(x, y, 0),
			// angle));
			// #else
