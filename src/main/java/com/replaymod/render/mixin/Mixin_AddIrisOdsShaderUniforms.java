...
{
	// Using ModifyVariable, so we only depend on a single argument. Hoping that
	// reduces the chance of breaking changes.
	@ModifyVariable(method = "generalCommonUniforms",
		at = @At("HEAD"),
		argsOnly = true)
	private static UniformHolder addReplayModOdsUniforms(UniformHolder uniforms)
	{
		...
		if(ods != null)
		{
			uniforms.uniform1b(UniformUpdateFrequency.PER_FRAME, "leftEye",
				ods::isLeftEye);
			uniforms.uniform1i(UniformUpdateFrequency.PER_FRAME, "direction",
				ods::getDirection);
		}
		return uniforms;
	}
