...
/**
 * Forces the sky to always render when chroma keying is active. Ordinarily it
 * only renders when the render distance is
 * at 4 or greater.
 */
// #if MC>=11500
...
public abstract class Mixin_ChromaKeyForceSky
{
	...
	// #elseif MC>=11400
	// $$ @ModifyConstant(method = "renderCenter", constant = @Constant(intValue
	// = 4))
	// #else
	...
	// this.client.gameRenderer).replayModRender_getHandler();
	// $$ if (handler != null) {
