...
public class Lwjgl3Loader extends URLClassLoader
{
	...
	private Lwjgl3Loader(Path jarFile)
		throws IOException, ReflectiveOperationException
	{
		super(new URL[]{jarFile.toUri().toURL()},
			Lwjgl3Loader.class.getClassLoader());

		// Need to use a different directory for natives than MC because native
		// files can only be loaded once
		Path nativesDir = ReplayMod.instance.folders.getCacheFolder()
			.resolve("lwjgl-natives");

		Class<?> configClass =
			Class.forName("org.lwjgl.system.Configuration", true, this);
	...
	private static synchronized Path getJarFile() throws IOException
	{
		if(tempJarFile == null)
		{
			...
			try(InputStream in =
				Lwjgl3Loader.class.getResourceAsStream("lwjgl.jar"))
			{
				if(in == null)
				{
					throw new IOException(
						"Failed to find embedded lwjgl.jar file.");
				}
	...
	public static <P extends Frame> FrameConsumer<P> createFrameConsumer(
		Class<? extends FrameConsumer<P>> implClass, Class<?>[] parameterTypes,
		Object[] args)
	{
		try
		{
			...
			Class<?> realClass =
				Class.forName(implClass.getName(), true, loader);
			Constructor<?> constructor =
				realClass.getConstructor(parameterTypes);
