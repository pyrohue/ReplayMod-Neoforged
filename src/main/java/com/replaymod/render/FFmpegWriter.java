...
public class FFmpegWriter implements FrameConsumer<BitmapFrame>
{
	...
	public FFmpegWriter(final VideoRenderer renderer) throws IOException
	{
		...
		try
		{
			cmdline = new CommandLine(executable)
				.addArguments(commandArgs, false).toStrings();
		...
		{
			process =
				new ProcessBuilder(cmdline).directory(outputFolder).start();
		}catch(IOException e)
		{
			throw new NoFFmpegException(e);
		}
		File exportLogFile =
			new File(MCVer.getMinecraft().runDirectory, "export.log");
		OutputStream exportLogOut =
			new TeeOutputStream(new FileOutputStream(exportLogFile), ffmpegLog);
	...
	public void close() throws IOException
	{
		...
		do
		{
			...
			rem =
				TimeUnit.SECONDS.toNanos(30) - (System.nanoTime() - startTime);
		}while(rem > 0);
	...
	public void consume(Map<Channel, BitmapFrame> channels)
	{
		...
		}catch(Throwable t)
		{
			...
			try
			{
				// Check whether this is a failure right at the beginning of the
				// rendering process
				// or at some later point (ffmpeg won't print the output file
				// until the first frame
				// has been written to stdin, so we can't already check for
				// invalid args in <init>).
				getVideoFile();
			...
			CrashReportSection exportDetails =
				report.addElement("Export details");
		...
		{
			channels.values()
				.forEach(it -> ByteBufferPool.release(it.getByteBuffer()));
		}
	}
	...
	{
		isTrue(width == settings.getVideoWidth(),
			"Width has to be %d but was %d", settings.getVideoWidth(), width);
		isTrue(height == settings.getVideoHeight(),
			"Height has to be %d but was %d", settings.getVideoHeight(),
			height);
	}
	...
	public File getVideoFile() throws FFmpegStartupException
	{
		...
		for(String line : log.split("\n"))
		{
			...
			{
				String fileName = line.substring(line.indexOf(", to '") + 6,
					line.lastIndexOf('\''));
				return new File(settings.getOutputFile().getParentFile(),
					fileName);
			}
		}
