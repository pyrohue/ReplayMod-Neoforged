...
public class EntityRendererHandler extends EventRegistrations
	implements WorldRenderer
{
	...
	public void renderWorld(final float partialTicks, CaptureData data)
	{
		this.data = data;
		long offsetMillis;
		if(ReplayMod.instance.getSettingsRegistry()
			.get(Setting.FRAME_TIME_FROM_WORLD_TIME))
		{
			offsetMillis = ReplayModReplay.instance.getReplayHandler()
				.getReplaySender().currentTimeStamp();
		}else
		{
			offsetMillis = renderInfo.getFramesDone() * 1_000L
	...
	public void renderWorld(float partialTicks, long finishTimeNano)
	{
		...
		{
			GameRendererAccessor gameRenderer =
			...
			try
			{
				mc.currentScreen = null; // do not want to render the current
											// screen (that'd just be the
											// progress gui)
				mc.options.pauseOnLostFocus = false; // do not want the pause
														// menu to open if the
														// window is unfocused
				if(omnidirectional)
				{
					gameRenderer.setRenderHand(false); // makes no sense, we
														// wouldn't even know
														// where to put it
				}
				...
				// #elseif MC>=11400
				// $$ mc.gameRenderer.render(partialTicks, finishTimeNano,
				// true);
				// #else
				// $$ mc.setIngameNotInFocus(); // this should already be the
				// case but it somehow still sometimes is not
				// #if MC>=10809
