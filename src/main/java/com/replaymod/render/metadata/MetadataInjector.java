...
public class MetadataInjector
{
	...
	private static final String SPHERICAL_XML_CONTENTS =
		"<GSpherical:Spherical>true</GSpherical:Spherical> "
			...
			+ "<GSpherical:StitchingSoftware>" + STITCHING_SOFTWARE
	...
	private static final String STEREO_XML_CONTENTS =
	...
	private static final String XML_MONO_METADATA = SPHERICAL_XML_HEADER
		+ SPHERICAL_XML_CONTENTS + SPHERICAL_CROP_XML + SPHERICAL_XML_FOOTER;
	private static final String XML_STEREO_METADATA =
		SPHERICAL_XML_HEADER + SPHERICAL_XML_CONTENTS + SPHERICAL_CROP_XML
	 ...
	 * https://github.com/google/spatial-media/tree/master/360-Videos-Metadata
	 */
	private static final byte[] UUID_BYTES = new byte[]{(byte)0xff, (byte)0xcc,
		(byte)0x82, (byte)0x63, (byte)0xf8, (byte)0x55, (byte)0x4a, (byte)0x93,
		(byte)0x88, (byte)0x14, (byte)0x58, (byte)0x7a, (byte)0x02, (byte)0x52,
		(byte)0x1f, (byte)0xdd};

	public static void injectMetadata(RenderSettings.RenderMethod renderMethod,
		File videoFile, int videoWidth, int videoHeight, int sphericalFovX,
		int sphericalFovY)
	{
		String xmlString;
		switch(renderMethod)
		...
		Dimension original = getOriginalDimensions(videoWidth, videoHeight,
			sphericalFovX, sphericalFovY);
		writeMetadata(videoFile,
			String.format(xmlString, original.getWidth(), original.getHeight(),
				videoWidth, videoHeight, (original.getWidth() - videoWidth) / 2,
				(original.getHeight() - videoHeight) / 2));
	}

	private static Dimension getOriginalDimensions(int videoWidth,
		...
		try
		{
			...
			if(trackBox == null)
				throw new IOException(
					"Could not find trak box inside moov box");
			...
			// to reduce by the Metadata Box's size to preserve the Video's File
			// size
			FreeBox freeBox = (FreeBox)getBoxByName(tempIsoFile, "free");
			if(freeBox == null)
				throw new IOException("Could not find free box inside IsoFile");

			int freeSize = Math.max(0,
				freeBox.getData().capacity() - (int)metadataBox.getSize());
