...
public class CameraPathExporter
{
	...
	{
		timeBuffer =
			ByteBuffer.allocate(4 * totalFrames).order(ByteOrder.LITTLE_ENDIAN);
		cameraTranslationBuffer = ByteBuffer.allocate(4 * totalFrames * 3)
			.order(ByteOrder.LITTLE_ENDIAN);
		cameraRotationBuffer = ByteBuffer.allocate(4 * totalFrames * 4)
			.order(ByteOrder.LITTLE_ENDIAN);
	}
	...
	public void recordFrame(float tickDelta)
	{
		...
		// #else
		// $$ Entity entity = mc.renderViewEntity == null ? mc.thePlayer :
		// mc.renderViewEntity;
		// #endif
		...
		// #endif
		// $$ float x = (float) (entity.prevPosX + (entity.posX -
		// entity.prevPosX) * tickDelta);
		// $$ float y = (float) (entity.prevPosY + (entity.posY -
		// entity.prevPosY) * tickDelta + eyeHeight);
		// $$ float z = (float) (entity.prevPosZ + (entity.posZ -
		// entity.prevPosZ) * tickDelta);
		// $$ float yaw = entity.prevRotationYaw + (entity.rotationYaw -
		// entity.prevRotationYaw) * tickDelta + 180;
		// $$ float pitch = entity.prevRotationPitch + (entity.rotationPitch -
		// entity.prevRotationPitch) * tickDelta;
		// #endif
		float roll =
			entity instanceof CameraEntity ? ((CameraEntity)entity).roll : 0;

		Quaternion quatYaw = new Quaternion();
		Quaternion quatPitch = new Quaternion();
		Quaternion quatRoll = new Quaternion();

		quatYaw.setFromAxisAngle(
			new Vector4f(0, -1, 0, (float)Math.toRadians(yaw)));
		quatPitch.setFromAxisAngle(
			new Vector4f(-1, 0, 0, (float)Math.toRadians(pitch)));
		quatRoll.setFromAxisAngle(
			new Vector4f(0, 0, -1, (float)Math.toRadians(roll)));
		...
		float[] rotation = new float[]{quaternion.getX(), quaternion.getY(),
	...
	public void finish() throws IOException
	{
		...
		int cameraTranslationBufferSize =
			cameraTranslationBuffer.rewind().remaining();
		int cameraRotationBufferSize =
			cameraRotationBuffer.rewind().remaining();
		...
		glTF.addAnimations(configure(new Animation(), animation -> {
			animation.addChannels(configure(new AnimationChannel(), channel -> {
				channel.setTarget(
					configure(new AnimationChannelTarget(), target -> {
						target.setNode(0);
		...
		glTF.addCameras(configure(new Camera(), camera -> {
			...
			camera.setPerspective(
				configure(new CameraPerspective(), perspective -> {
					float aspectRatio = (float)settings.getVideoWidth()
						/ (float)settings.getVideoHeight();
					perspective.setAspectRatio(aspectRatio);
					perspective.setYfov(
						(float)Math.toRadians(mc.options.getFov().getValue()));
					perspective.setZnear(0.05f);
					perspective
						.setZfar((float)mc.options.getViewDistance().getValue()
							* 16 * 4);
				}));
		}));
		glTF.addNodes(configure(new Node(), node -> node.setCamera(0)));
		glTF.addBuffers(configure(new Buffer(),
			buffer -> buffer.setByteLength(binaryData.limit())));
		// Time
		...
		java.nio.file.Path glbBasePath = Files.isDirectory(videoPath)
			...
			: videoPath.resolveSibling(
				FilenameUtils.getBaseName(videoPath.getFileName().toString())
					+ ".glb");
		java.nio.file.Path glbPath = glbBasePath;
		for(int i = 0; Files.exists(glbPath); i++)
		{
			String baseName =
				FilenameUtils.getBaseName(glbBasePath.getFileName().toString());
