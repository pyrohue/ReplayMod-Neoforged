	...
	implements FrameCapturer<F>
{
	...
	public OpenGlFrameCapturer(WorldRenderer worldRenderer,
	...
	{
		ByteBuffer buffer =
			ByteBufferPool.allocate(getFrameWidth() * getFrameHeight() * 4);
		frameBuffer().beginWrite(true);
		...
		return new OpenGlFrame(frameId,
			new Dimension(getFrameWidth(), getFrameHeight()), 4, buffer);
	}
