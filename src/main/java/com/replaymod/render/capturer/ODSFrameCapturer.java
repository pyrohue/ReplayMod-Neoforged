...
{
	private static final Identifier vertexResource =
		Identifier.of("replaymod", "shader/ods.vert");
	private static final Identifier fragmentResource =
		Identifier.of("replaymod", "shader/ods.frag");
	...
	public ODSFrameCapturer(WorldRenderer worldRenderer,
		final RenderInfo renderInfo, int frameSize)
	{
		...
		right =
			new CubicStereoFrameCapturer(worldRenderer, fakeInfo, frameSize);
		try
		{
		...
		}catch(Exception e)
		{
			throw new CrashException(
				CrashReport.create(e, "Creating ODS shaders"));
		}
	}
	...
	private void bindProgram()
	{
		...
		Program.Uniform fogUniform =
			shaderProgram.getUniformVariable("fogEnabled");
	...
	public Map<Channel, ODSOpenGlFrame> process()
	{
		...
		if(leftChannels != null && rightChannels != null)
		{
			...
			for(Channel channel : Channel.values())
			{
				...
				{
					result.put(channel,
						new ODSOpenGlFrame(leftFrame, rightFrame));
				}
			}
			return result;
		}
	...
	{
		public CubicStereoFrameCapturer(WorldRenderer worldRenderer,
		...
		protected OpenGlFrame renderFrame(int frameId, float partialTicks,
			CubicOpenGlFrameCapturer.Data captureData)
		{
