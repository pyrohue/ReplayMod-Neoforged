	...
	extends OpenGlFrameCapturer<F, D>
{
	...
	public PboOpenGlFrameCapturer(WorldRenderer worldRenderer,
		RenderInfo renderInfo, Class<D> type, int framePixels)
	{
		super(worldRenderer, renderInfo);

		withDepth = renderInfo.getRenderSettings().isDepthMap();
		data = type.getEnumConstants();
		int bufferSize = framePixels
			* (4 /* bgra */ + (withDepth ? 4 /* float */ : 0)) * data.length;
		pbo = new PixelBufferObject(bufferSize, PixelBufferObject.Usage.READ);
		otherPBO =
			new PixelBufferObject(bufferSize, PixelBufferObject.Usage.READ);
	}
	...
	private F readFromPbo(ByteBuffer pboBuffer, int bytesPerPixel)
	{
		...
		int frameBufferSize =
			getFrameWidth() * getFrameHeight() * bytesPerPixel;
		for(int i = 0; i < frames.length; i++)
		{
			...
			frameBuffer.rewind();
			frames[i] = new OpenGlFrame(framesDone - 2, frameSize,
				bytesPerPixel, frameBuffer);
		}
	...
	protected OpenGlFrame captureFrame(int frameId, D captureData)
	{
		...
		int offset =
			captureData.ordinal() * getFrameWidth() * getFrameHeight() * 4;
		frameBuffer().beginWrite(true);
		GL11.glReadPixels(0, 0, getFrameWidth(), getFrameHeight(), GL12.GL_BGRA,
			GL11.GL_UNSIGNED_BYTE, offset);
		if(withDepth)
		{
			offset += data.length * getFrameWidth() * getFrameHeight() * 4;
			GL11.glReadPixels(0, 0, getFrameWidth(), getFrameHeight(),
				GL11.GL_DEPTH_COMPONENT, GL11.GL_FLOAT, offset);
		}
