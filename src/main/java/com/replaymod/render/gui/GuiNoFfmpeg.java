...
public class GuiNoFfmpeg extends GuiScreen
{
	...
	private static final String LINK =
		"https://www.replaymod.com/docs/#installing-ffmpeg";

	private final GuiLabel message =
		new GuiLabel().setI18nText("replaymod.gui.rendering.error.message");
	private final GuiLabel link = new GuiLabel().setText(LINK);
	private final GuiButton openLinkButton =
		new GuiButton().setI18nLabel("chat.link.open").setSize(100, 20)
			.onClick(() -> openURL(URI.create(LINK)));
	private final GuiButton copyToClipboardButton =
		new GuiButton().setI18nLabel("chat.copy").setSize(100, 20)
			.onClick(() -> setClipboardString(LINK));
	private final GuiButton backButton =
		new GuiButton().setI18nLabel("gui.back").setSize(100, 20);
...

	{
		...
		setLayout(
			new VerticalLayout(VerticalLayout.Alignment.CENTER).setSpacing(30));
