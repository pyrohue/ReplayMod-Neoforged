...
public class VirtualWindow implements Closeable
{
	...
	public VirtualWindow(MinecraftClient mc)
	{
		...
		// #if MC>=11700
		guiFramebuffer =
			new WindowFramebuffer(framebufferWidth, framebufferHeight);
		// #else
		// $$ guiFramebuffer = new Framebuffer(framebufferWidth,
	...
	/**
	 * Updates the size of the window's framebuffer. Must only be called while
	 * this window is bound.
	 */
	public void onResolutionChanged(int newWidth, int newHeight)
	{
		if(newWidth == 0 || newHeight == 0)
		{
			...
			// Creating zero-sized framebuffers however will throw an error, so
			// we never want to switch to zero values.
			return;
		}
		...
		{
			mc.currentScreen.resize(mc, window.getScaledWidth(),
	...
	{
		// #if MC>=11400
		window.setScaleFactor(window.calculateScaleFactor(
			mc.options.getGuiScale().getValue(), mc.forcesUnicodeFont()));
		// #else
		// $$ // Nothing to do, ScaledResolution re-computes the scale factor
