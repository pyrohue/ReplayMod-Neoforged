...
{
	private static final Identifier NO_PREVIEW_TEXTURE =
		Identifier.of("replaymod", "logo.png");

	private final VideoRenderer renderer;

	public final GuiLabel title =
		new GuiLabel().setI18nText("replaymod.gui.rendering.title");
	...
	public final GuiCheckbox previewCheckbox =
		new GuiCheckbox().setI18nLabel("replaymod.gui.rendering.preview");
	...
	public final GuiPanel buttonPanel =
		new GuiPanel().setLayout(new HorizontalLayout().setSpacing(4));
	public final GuiButton pauseButton =
		new GuiButton(buttonPanel).onClick(new Runnable()
		{
			@Override
			public void run()
			{
	...
	{
		final GuiPanel contentPanel =
			new GuiPanel(this).setLayout(new CustomLayout<GuiPanel>()
			{
				@Override
				protected void layout(GuiPanel container, int width, int height)
				{
					...
					pos(buttonPanel, width / 2 - width(buttonPanel) / 2,
						height - height(buttonPanel));
					pos(progressBar, width / 2 - width(progressBar) / 2,
						y(buttonPanel) - 5 - height(progressBar));
					pos(renderTime, 0, y(progressBar) - 2 - height(renderTime));
					pos(remainingTime, width - width(remainingTime),
						y(progressBar) - 2 - height(renderTime));
					pos(previewCheckbox, width / 2 - width(previewCheckbox) / 2,
						y(renderTime) - 10 - height(previewCheckbox));
					size(imagePanel, width,
						y(previewCheckbox) - 5 - y(imagePanel));
				}
			}).addElements(null, title, imagePanel, previewCheckbox, renderTime,
				remainingTime, progressBar, buttonPanel);
		setLayout(new CustomLayout<GuiScreen>()
		{
			@Override
			protected void layout(GuiScreen container, int width, int height)
			{
				// Make sure we have some space around our central GUI
				// components
				pos(contentPanel, 5, 3);
				size(contentPanel, width - 10, height - 10);
			}
		});
		// FIXME default background doesn't work during rendering because the
		// blur effect relies on the framebuffer
		// #if MC>=12006
		setBackground(Background.NONE);
		setBackgroundColor(
			new de.johni0702.minecraft.gui.utils.lwjgl.Color(32, 32, 32));
		// #else
	...
	// the amount of frames that were rendered when the time left was last
	// updated
	...
	public void tick()
	{
		...
		// first, update the total render time (only if rendering is not paused
		// and has already started)
		if(!renderer.isPaused() && renderer.getFramesDone() > 0
			&& prevTime > -1)
		{
			renderTimeTaken += (current - prevTime);
		}else
		{
			// if the rendering process is paused, we have to update the frame
			// start time to prevent huge render times
			// for the currently rendered frame(s)
			frameStartTime = current;
		}

		// always update prevTime, so we don't get huge time jumps when
		// unpausing the rendering process
		prevTime = current;
		...
		if(prevRenderedFrames < renderer.getFramesDone())
		{
			...
			if(prevRenderedFrames > 0)
			{
				...
				// calculate the amount of frames that have been rendered since
				// the last update
				int framesRendered =
					renderer.getFramesDone() - prevRenderedFrames;

				// calculate the time it took to render these frames
				int renderTime = (int)(current - frameStartTime);

				// calculate the average time it took for each of these frames
				// to render
				...
				// calculate the average render time for the previous [up to 50]
				// frames
				float averageRenderTime =
					validValues > 0 ? totalTime / validValues : 0;

				// calculate the remaining render time in seconds
				renderTimeLeft = Math.round((averageRenderTime
					* (renderer.getTotalFrames() - renderer.getFramesDone()))
					/ 1000);
			}

			// set the render start time of the next frame(s) to the current
			// timestamp
			frameStartTime = current;
		...
		renderTime.setText(I18n.translate("replaymod.gui.rendering.timetaken")
			+ ": " + secToString(renderTimeTaken / 1000));
		remainingTime.setText(I18n.translate("replaymod.gui.rendering.timeleft")
			+ ": " + secToString(renderTimeLeft));

		int framesDone = renderer.getFramesDone(),
			framesTotal = renderer.getTotalFrames();
		progressBar.setI18nLabel("replaymod.gui.rendering.progress", framesDone,
			framesTotal);
	...
	private String secToString(int seconds)
	{
		...
		if(hours > 0)
			builder.append(hours).append(I18n.translate("replaymod.gui.hours"));
		if(min > 0 || hours > 0)
			builder.append(min).append(I18n.translate("replaymod.gui.minutes"));
		builder.append(sec).append(I18n.translate("replaymod.gui.seconds"));

		return builder.toString();
	}

	private synchronized void renderPreview(GuiRenderer guiRenderer,
		ReadableDimension size)
	{
		...
		if(previewTexture == null)
		{
			// #if MC>=11400
			previewTexture =
				new NativeImageBackedTexture(videoWidth, videoHeight, true);
			// #else
	...
	private void renderNoPreview(GuiRenderer guiRenderer,
		ReadableDimension size)
	{
	...
	private void renderPreviewTexture(GuiRenderer guiRenderer,
		ReadableDimension size, int videoWidth, int videoHeight)
	{
		Dimension dimension =
			Utils.fitIntoBounds(new Dimension(videoWidth, videoHeight), size);
		...
		guiRenderer.drawTexturedRect(x, y, 0, 0, width, height, videoWidth,
	...
	public void updatePreview(ByteBuffer buffer, ReadableDimension size)
	{
		if(previewCheckbox.isChecked() && previewTexture != null)
		{
			...
			synchronized(this)
			{
				...
				// #endif
				// Note: Optifine changes the texture data array to be three
				// times as long (for use by shaders),
				// we only want to initialize the first third and since we use
				// our frame size, not the array size,
				// we're good to go.
				...
				for(int y = 0; y < size.getHeight(); y++)
				{
					for(int x = 0; x < width; x++)
					{
						...
						data.setColor(x, y, value); // actually takes ABGR, not
													// RGBA
						// #else
