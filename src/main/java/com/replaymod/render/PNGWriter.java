...
public class PNGWriter implements FrameConsumer<BitmapFrame>
{
	...
	public void consume(Map<Channel, BitmapFrame> channels)
	{
		...
		try
		{
			if(bgraFrame != null)
			{
				withImage(bgraFrame, image -> image.writePNG(outputFolder
					.resolve(bgraFrame.getFrameId() + ".png").toFile()));
			}
			if(depthFrame != null)
			{
				withImage(depthFrame, image -> image.writePNG(outputFolder
					.resolve(depthFrame.getFrameId() + ".depth.png").toFile()));
			}
		}catch(Throwable t)
		{
	...
	private void withImage(BitmapFrame frame, IOConsumer<Image> consumer)
		throws IOException
	{
