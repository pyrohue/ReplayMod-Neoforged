...
public class Pipeline<R extends Frame, P extends Frame> implements Runnable
{
	...
	public Pipeline(WorldRenderer worldRenderer, FrameCapturer<R> capturer,
		FrameProcessor<R, P> processor, FrameConsumer<P> consumer)
	{
		...
		float near = 0.05f;
		float far =
			getMinecraft().options.getViewDistance().getValue() * 16 * 4;
	...
	public synchronized void run()
	{
		...
		int processThreads = Math.max(1, processors - 2); // One processor for
															// the main thread
															// and one for
															// ffmpeg, sorry OS
															// :(
		ExecutorService processService =
			new ThreadPoolExecutor(processThreads, processThreads, 0L,
		...
		try
		{
			processService.awaitTermination(Long.MAX_VALUE,
		...
		}catch(Throwable t)
		{
			CrashReport crashReport =
				CrashReport.create(t, "Cleaning up rendering pipeline");
		...
		public void run()
		{
			try
			{
				...
				for(Map.Entry<Channel, R> entry : rawChannels.entrySet())
				{
					...
					if(entry.getKey() == Channel.DEPTH
						&& processedFrame instanceof BitmapFrame)
					{
			...
			{
				CrashReport crashReport =
					CrashReport.create(t, "Processing frame");
