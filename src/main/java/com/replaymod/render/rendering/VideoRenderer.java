...
{
	private static final Identifier SOUND_RENDER_SUCCESS =
		Identifier.of("replaymod", "render_success");
	...
	public VideoRenderer(RenderSettings settings, ReplayHandler replayHandler,
		Timeline timeline) throws IOException
	{
		...
		}else
		{
			...
			if(settings
				.getEncodingPreset() == RenderSettings.EncodingPreset.EXR)
			{
				frameConsumer =
					EXRWriter.create(settings.getOutputFile().toPath(),
						settings.isIncludeAlphaChannel());
			}else if(settings
				.getEncodingPreset() == RenderSettings.EncodingPreset.PNG)
			{
			...
			ffmpegWriter = frameConsumer instanceof FFmpegWriter
				? (FFmpegWriter)frameConsumer : null;
			FrameConsumer<BitmapFrame> previewingFrameConsumer =
				new FrameConsumer<BitmapFrame>()
				{
					...
					public void consume(Map<Channel, BitmapFrame> channels)
					{
						...
						if(bgra != null)
						{
							synchronized(this)
							{
								...
								{
									lastFrameId = frameId;
									gui.updatePreview(bgra.getByteBuffer(),
										bgra.getSize());
								}
							}
						}
			...
			this.renderingPipeline = Pipelines.newPipeline(
				settings.getRenderMethod(), this, previewingFrameConsumer);
		}
	 ...
	 * Render this video.
	 *
	 * @return {@code true} if rendering was successful, {@code false} if the
	 *         user aborted rendering (or the window was closed)
	 */
	public boolean renderVideo() throws Throwable
	{
		...
		// Because this might take some time to prepare we'll render the GUI at
		// least once to not confuse the user
		drawGui();

		ReplayTimer timer = (ReplayTimer)((MinecraftAccessor)mc).getTimer();

		// Play up to several seconds before starting to render
		// This is necessary in order to ensure that all entities have at least
		// two position packets
		// and their first position in the recording is correct.
		// Additionally, this gives time for player skins to load from the
		// network.
		// Note that it is impossible to also get the interpolation between
		// their latest position
		// and the one in the recording correct as there's no reliable way to
		// tell when the server ticks
		...
		try
		{
			preloadDelaySeconds = ReplayModRender.instance.getCore()
				.getSettingsRegistry().get(Setting.SKIN_PRELOAD_DELAY);
		}catch(Exception e)
		{
			LOGGER.warn(
				"Failed to load skin preload delay setting, using default value of 3 seconds",
				e);
		}
		int preloadDelayMs = preloadDelaySeconds * 1000;
		Optional<Integer> optionalVideoStartTime =
			timeline.getValue(TimestampProperty.PROPERTY, 0);
		...
		// mc.worldRenderer).setRenderEntitiesStartupCounter(0);
		// #endif
		...
		if(((MinecraftAccessor)mc).getCrashReporter() != null)
		{
			throw new CrashException(
				((MinecraftAccessor)mc).getCrashReporter().get());
		}

		if(settings.isInjectSphericalMetadata())
		{
			MetadataInjector.injectMetadata(settings.getRenderMethod(),
				settings.getOutputFile(), settings.getTargetVideoWidth(),
				settings.getTargetVideoHeight(), settings.getSphericalFovX(),
	...
	{
		// because the jGui lib uses Minecraft's displayWidth and displayHeight
		// values, update these temporarily
		guiWindow.bind();
		...
		// Updating the timer will cause the timeline player to update the game
		// state
		ReplayTimer timer = (ReplayTimer)((MinecraftAccessor)mc).getTimer();
		// #if MC>=11600
		int elapsedTicks =
	...
	private void setup()
	{
		...
		for(SoundCategory category : SoundCategory.values())
		{
			...
			{
				originalSoundLevels.put(category,
					mc.options.getSoundVolume(category));
		...
		gui.toMinecraft().init(mc, mc.getWindow().getScaledWidth(),
	...
	private void finish()
	{
		...
		// Tear down of the timeline player might only happen the next tick
		...
		for(Map.Entry<SoundCategory, Float> entry : originalSoundLevels
			.entrySet())
		{
			mc.options.getSoundVolumeOption(entry.getKey())
				.setValue((double)entry.getValue());
		}
		mc.setScreen(null);
		...
		mc.getSoundManager().play(PositionedSoundInstance
			.master(SoundEvent.of(SOUND_RENDER_SUCCESS), 1));

		try
		{
			if(!hasFailed() && ffmpegWriter != null)
			{
				new GuiRenderingDone(ReplayModRender.instance,
					ffmpegWriter.getVideoFile(), totalFrames, settings)
						.display();
			}
		...
		// Finally, resize the Minecraft framebuffer to the actual width/height
		// of the window
		resizeMainWindow(mc, guiWindow.getFramebufferWidth(),
			guiWindow.getFramebufferHeight());
	}
	...
	private void executeTaskQueue()
	{
		...
		while(true)
		{
			...
			CompletableFuture<Void> resourceReloadFuture =
				((MinecraftAccessor)mc).getResourceReloadFuture();
			if(resourceReloadFuture != null)
			{
				((MinecraftAccessor)mc).setResourceReloadFuture(null);
				mc.reloadResources()
					.thenRun(() -> resourceReloadFuture.complete(null));
				continue;
			}
			break;
		}
		...
		// #else
		// $$ Queue<FutureTask<?>> scheduledTasks = ((MinecraftAccessor)
		// mc).getScheduledTasks();
	...
	public boolean drawGui()
	{
		...
		do
		{
			if(GLFW.glfwWindowShouldClose(window.getHandle())
			...
			// #if MC>=11700
			RenderSystem
				.setProjectionMatrix(com.replaymod.core.versions.MCVer.ortho(0,
			...
			gui.toMinecraft().init(mc, window.getScaledWidth(),
				window.getScaledHeight());

			// Events are polled on 1.13+ in mainWindow.update which is called
			// later
			// #if MC<11400
			// #if MC>=10800
			// $$ try {
			...
			// $$ // It isn't actually thrown here, so we'll deal with it the
			// easy way
			// $$ throw new RuntimeException(e);
			// $$ }
			// #else
			...
			int mouseY = (int)mc.mouse.getY() * window.getScaledHeight()
			...
			// #endif
			// #else
			// $$ int mouseX = Mouse.getX() * window.getScaledWidth() /
			// mc.displayWidth;
			// $$ int mouseY = window.getScaledHeight() - Mouse.getY() *
			// window.getScaledHeight() / mc.displayHeight - 1;
			// $$
	...
	{
		return new Dimension(settings.getVideoWidth(),
	...
	{
		return settings.map(VideoRenderer::checkCompat).filter(Objects::nonNull)
	...
	public static String[] checkCompat(RenderSettings settings)
	{
		...
		if(EmbeddiumFlawlessFramesHelper.hasEmbeddium()
			&& !EmbeddiumFlawlessFramesHelper.supportFlawlessFrames())
		{
			return new String[]{
		...
		if(SodiumFlawlessFramesHelper.hasSodium()
			&& !SodiumFlawlessFramesHelper.supportFlawlessFrames())
		{
			return new String[]{
