...
public class Serializer
{
	...
	public <T extends CFacade> CPointer<T> writeMajor(Object obj, DId id,
		Class<T> clazz) throws IOException
	{
		if(serialized.containsKey(obj))
		{
			throw new IllegalStateException(
				"Object " + obj + " already serialized.");
		}
		...
		if(id.code != ID_DATA)
		{
			...
			int counter =
				usedNames.compute(name, (n, i) -> i == null ? 0 : i + 1);
	...
	public <T extends CFacade> CPointer<T> maybeMajor(Object obj, DId id,
		Class<T> clazz, IOCallable<IOConsumer<T>> prepare) throws IOException
	{
	...
	public CArrayFacade<Byte> writeString0(String str) throws IOException
	{
		...
		CArrayFacade<Byte> pointer = factory().newCArrayBlock(
			BlockCodes.ID_DATA, Byte.class, align4(bytes.length));
	...
	public <T extends CFacade> CArrayFacade<T> writeData(Class<T> clazz,
	...
	public <T extends CFacade> CArrayFacade<T> writeData(Class<T> clazz,
		int count, IOBiConsumer<Integer, T> forElem) throws IOException
	{
		if(count == 0)
			return null;
	...
	public <T> CArrayFacade<CPointer<T>> writeDataPArray(Class<T> clazz,
		int count, Util.IOFunction<Integer, CPointer<T>> forElem)
		throws IOException
	{
		if(count == 0)
			return null;
		CArrayFacade<CPointer<T>> arrayFacade = factory().newCPointerBlock(
			ID_DATA, new Class[]{CPointer.class, clazz}, count);
		for(int i = 0; i < count; i++)
		{
	...
	public <T extends CFacade> void writeDataList(Class<T> clazz,
		ListBase listBase, int size, IOBiConsumer<Integer, T> forElem)
		throws IOException
	{
