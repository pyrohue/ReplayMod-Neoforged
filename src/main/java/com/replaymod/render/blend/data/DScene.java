...
public class DScene
{
	...
	public CPointer<Scene> serialize(Serializer serializer) throws IOException
	{
		return serializer.maybeMajor(this, id, Scene.class, () -> {
			Set<DObject> allObjects =
				findAllObjects(null, new LinkedHashSet<>());
			List<CPointer<BlenderObject>> bases =
				new ArrayList<>(allObjects.size());
			for(DObject dObject : allObjects)
			{
				bases.add(dObject.serialize(serializer));
			}
			return scene -> {
				serializer.writeDataList(Base.class, scene.getBase(),
					bases.size(), (i, base) -> {
				...
				ToolSettings toolSettings =
					serializer.writeData(ToolSettings.class);
