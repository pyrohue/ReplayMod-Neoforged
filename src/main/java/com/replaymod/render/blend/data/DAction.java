...
public class DAction
{
	...
	public CPointer<bAction> serialize(Serializer serializer) throws IOException
	{
		return serializer.maybeMajor(this, id, bAction.class, () -> {
			return bAction -> {
				serializer.writeDataList(FCurve.class, bAction.getCurves(),
					curves.size(), (i, fCurve) -> {
						DFCurve dfCurve = curves.get(i);
						fCurve.setRna_path(
							serializer.writeString0(dfCurve.rnaPath));
						fCurve.setArray_index(dfCurve.rnaArrayIndex);
						fCurve.setTotvert(dfCurve.keyframes.size());
						fCurve.setBezt(serializer.writeData(BezTriple.class,
							dfCurve.keyframes.size(), (j, fBezTriple) -> {
								DKeyframe dKeyframe = dfCurve.keyframes.get(j);
								fBezTriple
									.setIpo((byte)dKeyframe.interpolationType
										.ordinal());
								CArrayFacade<Float> vec =
									fBezTriple.getVec().get(1);
