...
public class DMesh
{
		...
		return serializer.maybeMajor(this, id, Mesh.class, () -> {
			...
			for(DMaterial material : this.materials)
			{
				...
				images.add(material.textures.isEmpty() ? null
					: material.textures.get(0).texture.image
						.serialize(serializer));
			}
			return mesh -> {
				mesh.setDrawflag(3 /* Edges & Faces */);
				CArrayFacade<CPointer<Material>> mat =
					serializer.writeDataPArray(Material.class, materials.size(),
						materials::get);
				mesh.setMat(mat);
				...
				if(!vertices.isEmpty())
				{
					CArrayFacade<MVert> mVerts =
						serializer.writeData(MVert.class, vertices.size(),
							(i, mVert) -> vertices.get(i).serialize(mVert));

					CustomDataLayer vDataLayer =
						serializer.writeData(CustomDataLayer.class);
				...
				if(!edges.isEmpty())
				{
					CArrayFacade<MEdge> mEdges =
						serializer.writeData(MEdge.class, edges.size(),
					...
					mesh.setMface(
						serializer.writeData(MFace.class, faces.size(),
							(i, mFace) -> faces.get(i).serialize(mFace)));
					mesh.setTotface(faces.size());
				}
				if(!loops.isEmpty())
				{
					CArrayFacade<MLoop> mLoops =
						serializer.writeData(MLoop.class, loops.size(),
							(i, mLoop) -> loops.get(i).serialize(mLoop));
					CArrayFacade<MLoopUV> mLoopUVs =
						serializer.writeData(MLoopUV.class, loops.size(),
							(i, mLoopUV) -> loops.get(i).serialize(mLoopUV));
					CArrayFacade<MLoopCol> mLoopCols =
						serializer.writeData(MLoopCol.class, loops.size(),
							(i, mLoopCol) -> loops.get(i).serialize(mLoopCol));

					CArrayFacade<CustomDataLayer> dataLayers =
						serializer.writeData(CustomDataLayer.class, 3);
					int i = 0;
				...
				if(!polys.isEmpty())
				{
					CArrayFacade<MPoly> mPolys =
						serializer.writeData(MPoly.class, polys.size(),
							(i, mPoly) -> polys.get(i).serialize(mPoly));
					CArrayFacade<MTexPoly> mTexPolys = serializer.writeData(
						MTexPoly.class, polys.size(), (i, mTexPoly) -> mTexPoly
							.setTpage(images.get(polys.get(i).materialSlot)));
	...
	public void addTriangle(DMesh.Vertex v1, DMesh.Vertex v2, DMesh.Vertex v3,
		Vector2f uv1, Vector2f uv2, Vector2f uv3, int c1, int c2, int c3,
		int materialSlot)
	{
	...
	public void addQuad(DMesh.Vertex v1, DMesh.Vertex v2, DMesh.Vertex v3,
		DMesh.Vertex v4, Vector2f uv1, Vector2f uv2, Vector2f uv3, Vector2f uv4,
		int c1, int c2, int c3, int c4, int materialSlot)
	{
		int vOffset = vertices.size();
		int eOffset = edges.size();
		int lOffset = loops.size();
