...
public class DObject
{
	...
	{
		if(frame < lastVisibleFrame)
			throw new IllegalStateException(
				"Already at frame " + lastVisibleFrame);
	...
	public void keyframe(String rnaPath, int rnaArrayIndex, int frame,
		float value)
	{
		if(frame < 0)
			return;
	...
	public void keyframe(String rnaPath, int rnaArrayIndex,
		DAction.DKeyframe keyframe, boolean constant)
	{
		if(action == null)
		{
			action = new DAction();
		...
		{
			if(curve.rnaArrayIndex == rnaArrayIndex
				&& curve.rnaPath.equals(rnaPath))
			{
				theCurve = curve;
				break;
			}
		...
		keyframe.interpolationType =
			constant ? DAction.InterpolationType.CONSTANT
				: DAction.InterpolationType.LINEAR;
		if(constant)
		{
			if(!theCurve.keyframes.isEmpty())
			{
				DAction.DKeyframe prev =
					theCurve.keyframes.get(theCurve.keyframes.size() - 1);
		...
		}else
		{
			if(theCurve.keyframes.size() >= 2)
			{
				DAction.DKeyframe prev =
					theCurve.keyframes.get(theCurve.keyframes.size() - 1);
				DAction.DKeyframe prev2 =
					theCurve.keyframes.get(theCurve.keyframes.size() - 2);
				float m =
					(prev.value - prev2.value) / (prev.frame - prev2.frame);
				float interpolatedValue =
					prev.value + m * (keyframe.frame - prev.frame);
	...
	public CPointer<BlenderObject> serialize(Serializer serializer)
		throws IOException
	{
		return serializer.maybeMajor(this, id, BlenderObject.class, () -> {
			...
			CPointer<BlenderObject> parent =
				this.parent == null ? null : this.parent.serialize(serializer);
			CPointer<Mesh> mesh =
				this.mesh == null ? null : this.mesh.serialize(serializer);
			CPointer<bAction> action =
				this.action == null ? null : this.action.serialize(serializer);
			CPointer<BlenderObject> pointAt = this.pointAt == null ? null
				: this.pointAt.serialize(serializer);

			return object -> {
				object.setParent(parent);
				if(parent != null)
				{
					CArrayFacade<CArrayFacade<Float>> parentinv =
						object.getParentinv();
				...
				if(mesh != null)
				{
					...
					if(totcol > 0)
					{
						...
						object.setMat(serializer.writeDataPArray(Material.class,
							totcol, i -> Util.plus(meshObj.getMat(), i).get()));
				...
				if(pointAt != null)
				{
					serializer.writeDataList(bConstraint.class,
						object.getConstraints(), 1, (i, bConstraint) -> {
							bConstraint.setEnforce(1);
							bConstraint.setType(
								(short)2 /* CONSTRAINT_TYPE_TRACKTO */);
							bTrackToConstraint constraint =
								serializer.writeData(bTrackToConstraint.class);
							constraint.setTar(pointAt);
							constraint.setReserved1(1 /* TRACK_Y */);
							constraint.setReserved2(2 /* UP_Z */);
							bConstraint.setData(constraint.__io__addressof()
								.cast(Object.class));
						});
				}
			};
