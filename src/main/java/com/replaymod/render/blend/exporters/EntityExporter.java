...
// $$ // so we position it at 0/0/0 and instead have the entities themselves
// move more
...
// $$ }
// $$
// $$ public void preRender(Entity entity, double dx, double dy, double dz,
// float yaw, float renderPartialTicks) {
...
// $$
// $$ // Normally the entity renderer will translate to dx/dy/dz but we want
...
// $$ if (!(entity instanceof LivingEntity)) { // except for EntityLivingBase
// which get special (better) treatment
// $$ Matrix4f modelView = getGlModelViewMatrix();
// $$ Matrix4f.translate(new Vector3f((float) dx, (float) dy, (float) dz),
// modelView, modelView);
...
// $$ // For any entity which extends EntityLivingBase, we capture its rotation
// and scale after it has been setup
// $$ // in addition to its position (which we capture in preRender for other
// entities).
// $$ // We could add custom hooks for various other entities types as well but
...
// $$ }
// $$
// $$ public void postRender(Entity entity, double dx, double dy, double dz,
// float yaw, float renderPartialTicks) {
// $$ renderState.pop();
// $$ }
// $$ }
