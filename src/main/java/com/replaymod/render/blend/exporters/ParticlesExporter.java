...
// $$ // so we position it at 0/0/0 and instead have the particles themselves
// move more
...
// $$ particleObject = new DObject(DObject.Type.OB_EMPTY); // mesh generation is
// delayed, see below
...
// $$
// $$ // Normally the particle renderer will translate to dx/dy/dz but we want
// that translation outside of
// $$ // the particle object (on the particle object itself).
// $$ // So, we translate there, then store the model-view-matrix and then
// translate back to let the
// $$ // renderer translate there again.
// $$ // Instead of actually translating, we just add the translation on the
// current model-view-matrix.
// $$ Matrix4f modelView = getGlModelViewMatrix();
// $$ ParticleAccessor acc = (ParticleAccessor) particle;
// $$ double dx = acc.getPrevPosX() + (acc.getPosX() - acc.getPrevPosX()) *
// renderPartialTicks;
// $$ double dy = acc.getPrevPosY() + (acc.getPosY() - acc.getPrevPosY()) *
// renderPartialTicks;
// $$ double dz = acc.getPrevPosZ() + (acc.getPosZ() - acc.getPrevPosZ()) *
// renderPartialTicks;
// #if MC>=11500
...
// $$ // Some particles start out as single points, so we can never scale those
// first meshes to other sizes
// $$ if (mesh.hasZeroLengthEdge(0.1f)) {
// $$ // therefore we must further delay mesh generation in these cases and
// assume a current model scale of 0
...
// $$ // Note: currently assumes the same scale on all relevant axes (should be
...
// $$ );
// #else
// $$ particle.func_180434_a(builder,
// Minecraft.getMinecraft().getRenderViewEntity(), 0, 1, 1, 0, 0, 0);
// #endif
// $$ builder.end();
// $$ return mesh;
// $$ }
...
// $$ .forEach(entry -> entry.getValue().keyframe("hide", 0,
// renderState.getFrame(), 1f));
