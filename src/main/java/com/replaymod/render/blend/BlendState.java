...
public class BlendState implements Exporter
{
	...
	public void setup()
	{
		for(Exporter exporter : exporters)
		{
			...
			{
				CrashReport report =
					CrashReport.create(e, "Setup of blend exporter");
	...
	public void tearDown()
	{
		for(Exporter exporter : exporters)
		{
			...
			{
				CrashReport report =
					CrashReport.create(e, "Tear down of blend exporter");
				CrashReportSection category = report.addElement("Exporter");
				category.add("Exporter", exporter::toString);
				throw new CrashException(report);
			}
		...
		try
		{
			...
			// TODO pre-configure render settings serializer.writeMajor(new
			// Object(), new DId(BlockCodes.ID_REND))
	...
	public void preFrame(int frame)
	{
		for(Exporter exporter : exporters)
		{
			...
			{
				CrashReport report =
					CrashReport.create(e, "Pre frame of blend exporter");
				CrashReportSection category = report.addElement("Exporter");
				category.add("Exporter", exporter::toString);
	...
	public void postFrame(int frame)
	{
		for(Exporter exporter : exporters)
		{
			...
			{
				CrashReport report =
					CrashReport.create(e, "Post frame of blend exporter");
				CrashReportSection category = report.addElement("Exporter");
				category.add("Exporter", exporter::toString);
				category.add("Frame", () -> String.valueOf(frame));
				throw new CrashException(report);
