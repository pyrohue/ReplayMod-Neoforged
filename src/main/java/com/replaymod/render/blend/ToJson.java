...
public class ToJson
{
	...
	public static void main(String[] args)
		throws IOException, JSONCodecException
	{
		...
		{
			System.err.println(
				"Warning: Conversion will probably fail due to version mismatch!");
		}
		...
		File fJsonOut = new File(fBlend.getParentFile(),
			fBlend.getName().replace(".blend", ".json"));
	...
	private static void addBlock(JSONArray aBlocks, Block b)
		throws JSONCodecException, IOException
	{
		...
		header.put("sdnaIndex",
			struct.getSignature() + "(" + b.header.getSdnaIndex() + ")");

		System.out.print("[" + b.header.getAddress() + ", "
			+ (b.header.getAddress() + b.header.getSize()) + "] ");
		if(b.header.getCode().equals(BlockCodes.ID_DATA)
			&& b.header.getSdnaIndex() == 0)
		{
			System.out
				.println(b.header.getCode().toString() + ": Link(0) or undef");
			byte[] buffer = new byte[b.header.getSize()];
			b.data.readFully(buffer);
			String str = toHexStr(buffer);
			oBlock.put("raw", str);
			int requiredSize = b.header.getCount()
				* struct.sizeof(blend.getEncoding().getAddressWidth());
		...
		{
			System.out.println(
				b.header.getCode().toString() + ": " + struct.getSignature());
			addStruct(oBlock, b, struct);
		}
	}
	...
	private static Object createStruct(Block b, int index, CStruct struct)
	{
		...
		try
		{
			Class<?> cStruct = Class.forName(PACKAGE + "."
				+ Renaming.mapStruct2Class(struct.getSignature()));
			long address = b.header.getAddress()
				+ index * struct.sizeof(blend.getEncoding().getAddressWidth());
			Constructor<?> constructor = cStruct.getDeclaredConstructor(
				long.class, Block.class, BlockTable.class);
	...
	private static Object toJson(Class<?> type, Object value) throws Throwable
	{
		...
		}catch(NullPointerException e)
		{
			...
			// 1. The sdna index stored in the block header is not correct. In
			// many
			// cases index 0 (Link) is used for raw data.
			// 2. A pointer references non-persistent data which was not stored
			// in the file.
