...
public class Util
{
	...
	private static FloatBuffer floatBuffer =
		BufferUtils.createByteBuffer(16 * 4).asFloatBuffer();
	...
	public static boolean isGlTextureMatrixIdentity()
	{
		...
		// Assumes that glLoadIdentity uses positive zero floating point value
		// (seems to be the case, at least on Mesa)
	...
	// Original CPointer.plus method is broken, this one works with any
	// non-negative numbers
	public static <T> CPointer<T> plus(CPointer<T> lhs, int rhs)
		throws IOException
	{
		while(rhs > 0)
		{
			lhs = lhs.plus(1); // 1 is the only value that can be passed to the
								// original plus
			rhs--;
		}
		return lhs;
	...
	public static void insert(ListBase list, CPointer<Link> element)
		throws IOException
	{
	...
	{
		// #if MC>=12006
		return net.minecraft.block.entity.BlockEntityType
			.getId(tileEntity.getType()).toString();
		// #else
		// #if MC>=11800
