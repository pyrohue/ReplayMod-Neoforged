...
public class BlendFrameCapturer implements FrameCapturer<BitmapFrame>
{
	...
	public BlendFrameCapturer(WorldRenderer worldRenderer,
	...
	public Map<Channel, BitmapFrame> process()
	{
		...
		worldRenderer.renderWorld(MinecraftClient.getInstance()
			.getRenderTickCounter().getTickDelta(true), null);
		BlendState.getState().postFrame(framesDone);

		BitmapFrame frame = new BitmapFrame(framesDone++, new Dimension(0, 0),
			0, ByteBufferPool.allocate(0));
