...
public class RenderSettings
{
	public enum RenderMethod
	{
		...
		{
			return I18n.translate("replaymod.gui.rendersettings.renderer."
				+ name().toLowerCase());
		}
		...
		{
			return Arrays.stream(values()).filter(RenderMethod::isSupported)
				.toArray(RenderMethod[]::new);
		}
	}
	...
	{
		MP4_CUSTOM(
			"-an -c:v libx264 -b:v %BITRATE% -pix_fmt yuv420p \"%FILENAME%\"",
			"mp4"),

		MP4_POTATO(
			"-an -c:v libx264 -preset ultrafast -crf 51 -pix_fmt yuv420p \"%FILENAME%\"",
			"mp4"),

		WEBM_CUSTOM(
			"-an -c:v libvpx -b:v %BITRATE% -pix_fmt yuv420p \"%FILENAME%\"",
			"webm"),

		MKV_LOSSLESS("-an -c:v libx264 -preset ultrafast -qp 0 \"%FILENAME%\"",
			"mkv"),
		...
		public String toString()
		{
			return I18n.translate("replaymod.gui.rendersettings.presets."
		...
		{
			return Arrays.stream(values()).filter(EncodingPreset::isSupported)
				.toArray(EncodingPreset[]::new);
		}
	}
	...
	public enum AntiAliasing
	{
		...
		public String toString()
		{
			return I18n.translate("replaymod.gui.rendersettings.antialiasing."
				+ name().toLowerCase());
		}
	}
	...
	// We switched from rgb24 to bgra for performance at one point, so for
	// backwards compatibility we need to
	// reset the arguments if they're from an older version. Easiest way to do
	// that is to just change the key
	// and handle the null during loading.
	@SerializedName("exportArguments")
	// using an empty string as default because old versions will realize it's
	// not a preset and prompt the user
	...
	{
		this(RenderSettings.RenderMethod.DEFAULT,
			RenderSettings.EncodingPreset.MP4_CUSTOM, 1920, 1080, 60, 20 << 20,
			null, true, false, false, false, false, null, 360, 180, false,
			false, false, RenderSettings.AntiAliasing.NONE, "",
			RenderSettings.EncodingPreset.MP4_CUSTOM.getValue(), false);
	}

	public RenderSettings(RenderMethod renderMethod,
		EncodingPreset encodingPreset, int videoWidth, int videoHeight,
		int framesPerSecond, int bitRate, File outputFile,
		boolean renderNameTags, boolean includeAlphaChannel,
		boolean stabilizeYaw, boolean stabilizePitch, boolean stabilizeRoll,
		ReadableColor chromaKeyingColor, int sphericalFovX, int sphericalFovY,
		boolean injectSphericalMetadata, boolean depthMap,
		boolean cameraPathExport, AntiAliasing antiAliasing,
		String exportCommand, String exportArguments, boolean highPerformance)
	{
		...
		this.chromaKeyingColor =
			chromaKeyingColor == null ? null : new Color(chromaKeyingColor);
	...
	{
		return new RenderSettings(renderMethod, encodingPreset, videoWidth,
			videoHeight, framesPerSecond, bitRate, outputFile, renderNameTags,
			includeAlphaChannel, stabilizeYaw, stabilizePitch, stabilizeRoll,
			chromaKeyingColor, sphericalFovX, sphericalFovY,
			injectSphericalMetadata, depthMap, cameraPathExport, antiAliasing,
			exportCommand, exportArguments, highPerformance);
	}

	/**
	 * @return the width of the output video during rendering, including the
	 *         upscale for Anti-Aliasing.
	 */
	...
	/**
	 * @return the height of the output video during rendering, including the
	 *         upscale for Anti-Aliasing.
	 */
	...
	public String getVideoFilters()
	{
		...
		if(antiAliasing != AntiAliasing.NONE)
		{
			...
			filters.append(
				String.format("-filter:v scale=iw*%1$s:ih*%1$s ", factor));
		}
	...
	private static String findFFmpeg()
	{
		...
		{
			case WINDOWS:
			// Allow windows users to unpack the ffmpeg archive into a
			// sub-folder of their .minecraft folder
			File dotMinecraft = MCVer.getMinecraft().runDirectory;
			File inDotMinecraft =
				new File(dotMinecraft, "ffmpeg/bin/ffmpeg.exe");
			if(inDotMinecraft.exists())
			{
				LOGGER.debug("FFmpeg found in .minecraft/ffmpeg");
				return inDotMinecraft.getAbsolutePath();
			}
			// But a significant amount of people are not even able to follow
			...
			// So, for support's sake, let's just search the entire .minecraft
			// folder for an `ffmpeg.exe` file.
			// We retain above check to have a fast path for any old
			// installations.
			try
			{
				Path[] result = new Path[1];
				Files.walkFileTree(dotMinecraft.toPath(),
			...
			break;
			case OSX:
			// The PATH doesn't seem to be set as expected on OSX, therefore we
			// check some common locations ourselves
			for(String path : new String[]{"/usr/local/bin/ffmpeg",
				"/usr/bin/ffmpeg"})
			{
			...
			// Homebrew doesn't seem to reliably symlink its installed binaries
			// either
			...
			for(String path : new String[]{"/usr/local", "/opt/homebrew"})
			{
				...
				Optional<File> latestOpt =
					Arrays.stream(homebrewVersions).map(ComparableVersion::new) // Convert
																				// file
																				// name
																				// to
																				// comparable
																				// version
						.sorted(Comparator.reverseOrder()) // Sort for latest
															// version
						.map(ComparableVersion::toString) // Convert back to
															// file name
						.map(v -> new File(new File(homebrewFolder, v),
							"bin/ffmpeg")) // Convert to binary files
						.filter(File::exists) // Filter invalid installations
												// (missing executable)
				...
				if(latestOpt.isPresent())
				{
					...
					LOGGER.debug(
						"Found {} versions of FFmpeg installed with homebrew, chose {}",
						homebrewVersions.length, latest);
					return latest.getAbsolutePath();
				}
			}
			break;
			case LINUX: // Linux users are entrusted to have their PATH
						// configured correctly (most package manager do this)
			case SOLARIS: // Never heard of anyone running this mod on Solaris
							// having any problems
	...
	public String toString()
	{
		return "RenderSettings{" + "renderMethod=" + renderMethod
			+ ", encodingPreset=" + encodingPreset + ", videoWidth="
			+ videoWidth + ", videoHeight=" + videoHeight + ", framesPerSecond="
			+ framesPerSecond + ", bitRate=" + bitRate + ", outputFile="
			+ outputFile + ", renderNameTags=" + renderNameTags
			+ ", includeAlphaChannel=" + includeAlphaChannel + ", stabilizeYaw="
			+ stabilizeYaw + ", stabilizePitch=" + stabilizePitch
			+ ", stabilizeRoll=" + stabilizeRoll + ", chromaKeyingColor="
			+ chromaKeyingColor + ", sphericalFovX=" + sphericalFovX
			+ ", sphericalFovY=" + sphericalFovY + ", injectSphericalMetadata="
			+ injectSphericalMetadata + ", depthMap=" + depthMap
			+ ", cameraPathExport=" + cameraPathExport + ", antiAliasing="
			+ antiAliasing + ", exportCommand='" + exportCommand + '\''
