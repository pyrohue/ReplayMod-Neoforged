...
public class ReplayModRender extends EventRegistrations implements Module
{
	...
	public File getVideoFolder()
	{
		...
		File folder = new File(
			path.startsWith("./") ? core.getMinecraft().runDirectory : null,
			path);
		try
		{
		...
		}catch(IOException e)
		{
			throw new CrashException(
				CrashReport.create(e, "Cannot create video folder."));
		}
		return folder;
	}
	...
	{
		return core.getMinecraft().runDirectory.toPath()
			.resolve("config/replaymod-rendersettings.json");
	}
	...
	public void saveRenderQueue()
	{
		...
		{
			e.printStackTrace();
			VanillaGuiScreen screen =
				VanillaGuiScreen.wrap(getCore().getMinecraft().currentScreen);
