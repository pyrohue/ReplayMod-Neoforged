...
public class EXRWriter implements FrameConsumer<BitmapFrame>
{
	...
	public static FrameConsumer<BitmapFrame> create(Path outputFolder,
		boolean keepAlpha)
	{
		return Lwjgl3Loader.createFrameConsumer(EXRWriter.class,
			new Class[]{Path.class, boolean.class},
			new Object[]{outputFolder, keepAlpha});
	}

	// Compression is pretty slow, so we'll only use it when we've got enough
	// cpu cores to make up for that
	private static final int COMPRESSION =
		Runtime.getRuntime().availableProcessors() >= 8
			? TINYEXR_COMPRESSIONTYPE_ZIPS : TINYEXR_COMPRESSIONTYPE_NONE;
	...
	public void consume(Map<Channel, BitmapFrame> channels)
	{
		...
		EXRChannelInfo.Buffer channelInfos =
			EXRChannelInfo.mallocStack(numChannels);
		...
		try
		{
			...
			for(int y = 0; y < height; y++)
			{
				for(int x = 0; x < width; x++)
				{
					...
					bgrChannels[3]
						.put(((int)bgra.get() & 0xff | alphaMask) / 255f); // a
				}
			}
		...
		{
			MCVer.getMinecraft().setCrashReportSupplier(
				CrashReport.create(t, "Exporting EXR frame"));
		}finally
		{
			memFree(images);
			stackPop();
