...
public class StereoscopicToBitmapProcessor
	extends AbstractFrameProcessor<StereoscopicOpenGlFrame, BitmapFrame>
{
	@Override
	public BitmapFrame process(StereoscopicOpenGlFrame rawFrame)
	{
		...
		ByteBuffer result =
			ByteBufferPool.allocate(width * 2 * size.getHeight() * bpp);
		openGlBytesToBitmap(rawFrame.getLeft(), 0, 0, result, width * 2);
		openGlBytesToBitmap(rawFrame.getRight(), size.getWidth(), 0, result,
			width * 2);
		...
		return new BitmapFrame(rawFrame.getFrameId(),
			new Dimension(width * 2, size.getHeight()), bpp, result);
	}
}
