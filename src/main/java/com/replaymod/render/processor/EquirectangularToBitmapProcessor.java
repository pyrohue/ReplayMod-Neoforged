...
public class EquirectangularToBitmapProcessor
	extends AbstractFrameProcessor<CubicOpenGlFrame, BitmapFrame>
{
	...
	public EquirectangularToBitmapProcessor(int outputWidth, int outputHeight,
		int sphericalFovX)
	{
		...
		for(int x = 0; x < width; x++)
		{
			...
			for(int y = 0; y < height; y++)
			{
				...
				image[y][x] = pt;
				imageX[y][x] = imgX;
				imageY[y][x] = frameSize - imgY - 1; // The OpenGl buffer
														// contains data flipped
														// vertically
			}
		}
	}
	...
	{
		Validate.isTrue(rawFrame.getLeft().getSize().getWidth() == frameSize,
			"Frame size must be %d but was %d", frameSize,
			rawFrame.getLeft().getSize().getWidth());
		int bpp = rawFrame.getLeft().getBytesPerPixel();
		ByteBuffer result = ByteBufferPool.allocate(width * height * bpp);
		ByteBuffer[] images = {rawFrame.getBack().getByteBuffer(),
			rawFrame.getFront().getByteBuffer(),
			rawFrame.getLeft().getByteBuffer(),
			rawFrame.getRight().getByteBuffer(),
			rawFrame.getTop().getByteBuffer(),
		...
		return new BitmapFrame(rawFrame.getFrameId(),
			new Dimension(width, height), bpp, result);
	}
