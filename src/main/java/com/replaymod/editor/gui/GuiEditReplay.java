...
public class GuiEditReplay extends AbstractGuiPopup<GuiEditReplay>
{
	...
	private final GuiHorizontalScrollbar scrollbar =
		new GuiHorizontalScrollbar().setSize(300, 9);
	...
	private final GuiButton zoomInButton = new GuiButton().setSize(9, 9)
		...
		.setSpriteUV(40, 20).setTooltip(
			new GuiTooltip().setI18nText("replaymod.gui.ingame.menu.zoomin"));

	private final GuiButton zoomOutButton = new GuiButton().setSize(9, 9)
		.onClick(() -> zoomTimeline(3d / 2d))
		.setTexture(ReplayMod.TEXTURE, ReplayMod.TEXTURE_SIZE)
		.setSpriteUV(40, 30).setTooltip(
			new GuiTooltip().setI18nText("replaymod.gui.ingame.menu.zoomout"));
	...
	protected GuiEditReplay(GuiContainer container, Path inputPath)
		throws IOException
	{
		super(container);
		...
		try(ReplayFile replayFile = ReplayMod.instance.files.open(inputPath))
		{
			...
			timeline = new EditTimeline(new HashSet<>(markers),
				markers -> this.markers = markers);
			timeline.setSize(300, 20).setMarkers()
		...
		GuiPanel timelinePanel = new GuiPanel().setSize(300, 40)
			...
			}).addElements(null, timelineTime, timeline, scrollbar,
				zoomButtonPanel);

		GuiButton buttonAddSplit = new GuiButton().setSize(100, 20)
			.setI18nLabel("replaymod.gui.edit.split")
			.setTooltip(new GuiTooltip()
				.setI18nText("replaymod.gui.edit.split.tooltip"))
			.onClick(() -> {
		...
		GuiButton buttonInsertCut = new GuiButton().setSize(100, 20)
			.setI18nLabel("replaymod.gui.edit.cut.start")
			.setTooltip(
				new GuiTooltip().setI18nText("replaymod.gui.edit.cut.tooltip"))
			.onClick(() -> {
				Marker marker = new Marker();
				marker.setTime(timeline.getCursorPosition());
		...
		GuiButton buttonEndCut = new GuiButton().setSize(100, 20)
		...
		GuiButton applyButton =
			new GuiButton().setI18nLabel("replaymod.gui.edit.apply")
				.setSize(150, 20).onClick(this::apply);
		GuiButton closeButton =
			new GuiButton().setI18nLabel("replaymod.gui.close").setSize(150, 20)
				.onClick(this::close);
		GuiPanel buttonPanel =
			new GuiPanel().setLayout(new HorizontalLayout().setSpacing(8))
				.addElements(null, applyButton, closeButton);

		popup.setLayout(
			new VerticalLayout(VerticalLayout.Alignment.TOP).setSpacing(10));
		popup.addElements(new VerticalLayout.Data(0.5), timelinePanel,
	...
	private void apply()
	{
		...
		new Thread(() -> {
			try(ReplayFile replayFile =
				ReplayMod.instance.files.open(inputPath))
			{
				replayFile.writeMarkers(markers);
				replayFile.save();
			}catch(IOException e)
			{
				Utils.error(ReplayModEditor.LOGGER, this,
					CrashReport.create(e, "Writing markers"), this::close);
			}

			try
			{
				MarkerProcessor.apply(inputPath,
					progressPopup.progressBar::setProgress);
			...
			}catch(Throwable e)
			{
				...
				CrashReport crashReport =
					CrashReport.create(e, "Running marker processor");
				ReplayMod.instance.runLater(() -> Utils
					.error(ReplayModEditor.LOGGER, this, crashReport, () -> {
						progressPopup.close();
						close();
					}));
			}
	...
	private class ProgressPopup extends AbstractGuiPopup<ProgressPopup>
	{
		private final GuiProgressBar progressBar =
			new GuiProgressBar(popup).setSize(300, 20);
	...
	private class EditTimeline extends GuiMarkerTimeline
	{
		...
		@Override
		protected void drawMarker(GuiRenderer renderer, ReadableDimension size,
			Marker marker, int markerX)
		{
			if(MarkerProcessor.MARKER_NAME_SPLIT.equals(marker.getName()))
			{
				int height = size.getHeight() - BORDER_BOTTOM - BORDER_TOP
					- MARKER_SIZE + 1;
				for(int y = 0; y < height; y += 3)
				{
					renderer.drawRect(markerX, BORDER_TOP + y, 1, 2,
						Color.WHITE);
				}
			}
		...
		private void drawCutQuads(GuiRenderer renderer, ReadableDimension size)
		{
			boolean inCut = false;
			int startTime = 0;
			for(Marker marker : markers.stream()
				.sorted(Comparator.comparing(Marker::getTime))
				.collect(Collectors.toList()))
			{
				if(MarkerProcessor.MARKER_NAME_START_CUT
					.equals(marker.getName()) && !inCut)
				{
					inCut = true;
					startTime = marker.getTime();
				}else if(MarkerProcessor.MARKER_NAME_END_CUT
					.equals(marker.getName()) && inCut)
				{
		...
		private void drawCutQuad(GuiRenderer renderer, ReadableDimension size,
			int startFrameTime, int endFrameTime)
		{
			...
			if(startX < endX)
			{
				renderer.drawRect(startX + 1,
					size.getHeight() - BORDER_BOTTOM - MARKER_SIZE,
