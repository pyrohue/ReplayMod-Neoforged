...
public class ReplayFoldersService
{
	...
	public Path getReplayFolder() throws IOException
	{
		return ensureDirectoryExists(
			mcDir.resolve(settings.get(Setting.RECORDING_PATH)));
	}

	/**
	 * Folder into which replay backups are saved before the MarkerProcessor is
	 * unleashed.
	 */
	 ...
	 * Distinct from the main folder, so they cannot be opened while they are
	 * still saving.
	 */
	 ...
	 * Distinct from the recording folder cause people kept confusing them with
	 * recordings.
	 */
	public Path getCacheFolder() throws IOException
	{
		Path path = ensureDirectoryExists(
			mcDir.resolve(settings.get(Setting.CACHE_PATH)));
		try
		{
	...
	private static final PercentEscaper CACHE_FILE_NAME_ENCODER =
	...
	public Path getCachePathForReplay(Path replay) throws IOException
	{
		...
		Path relative =
			replayFolder.toAbsolutePath().relativize(replay.toAbsolutePath());
		return cacheFolder
			.resolve(CACHE_FILE_NAME_ENCODER.escape(relative.toString()));
	}

	public Path getReplayPathForCache(Path cache) throws IOException
	{
		String relative =
			URLDecoder.decode(cache.getFileName().toString(), "UTF-8");
		Path replayFolder = getReplayFolder();
