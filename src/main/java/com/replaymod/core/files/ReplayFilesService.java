...
public class ReplayFilesService
{
	...
	private final Set<Path> lockedPaths =
	...
	{
		Path realInput =
			input != null ? input.toAbsolutePath().normalize() : null;
		...
		if(!Objects.equals(realInput, realOutput)
			&& !lockedPaths.add(realOutput))
		{
			if(realInput != null)
			{
		...
		try
		{
			replayFile = new ZipReplayFile(new ReplayStudio(),
	...
	{
		// Move anything which is still in the recording folder into the regular
		// replay folder
		// so it can be opened and/or recovered
		try(DirectoryStream<Path> paths =
			Files.newDirectoryStream(folders.getRecordingFolder()))
		{
			for(Path path : paths)
			{
				Path destination =
					folders.getReplayFolder().resolve(path.getFileName());
		...
		try(DirectoryStream<Path> paths =
			Files.newDirectoryStream(folders.getReplayFolder()))
		{
			for(Path path : paths)
			{
				...
				{
					Path original =
						path.resolveSibling(FilenameUtils.getBaseName(name));
					Path noRecoverMarker = original
						.resolveSibling(original.getFileName() + ".no_recover");
					if(Files.exists(noRecoverMarker))
					{
						// This file, when its markers are processed, doesn't
						// actually result in any replays.
						// So we don't really need to recover it either, let's
						// just get rid of it.
						FileUtils.deleteDirectory(path.toFile());
						Files.delete(noRecoverMarker);
						continue;
					}
					new RestoreReplayGui(core,
						GuiScreen.wrap(core.getMinecraft().currentScreen),
						original.toFile()).display();
				}
			}
		}catch(IOException e)
		{
	...
	private void cleanup()
	{
		...
		// Cleanup any cache folders still remaining in the recording folder (we
		// once used to put them there)
		try
		{
			Files.walkFileTree(folders.getReplayFolder(),
		...
		// Cleanup raw folder content three weeks after creation (these are
		// pretty valuable for debugging)
		try(DirectoryStream<Path> paths =
			Files.newDirectoryStream(folders.getRawReplayFolder()))
		{
			for(Path path : paths)
			{
				if(Files.getLastModifiedTime(path).toMillis()
					+ 21 * DAYS < System.currentTimeMillis())
				{
					Files.delete(path);
				}
			}
		...
		// Cleanup cache folders 7 days after last modification or when its
		// replay is gone
		try(DirectoryStream<Path> paths =
			Files.newDirectoryStream(folders.getCacheFolder()))
		{
			for(Path path : paths)
			{
				if(Files.isDirectory(path))
				{
					...
					if(lastModified + 7 * DAYS < System.currentTimeMillis()
						|| !Files.exists(replay))
					{
						FileUtils.deleteDirectory(path.toFile());
					}
