...
public class ReplayModMixinConfigPlugin implements IMixinConfigPlugin
{
		...
		String mixinClassName)
	{
		...
		{
			// #if MC>=11500
			// OF renames the lambda method name and I see no way we can target
			// it now, so we give up on that patch
			if(mixinClassName.endsWith("MixinTileEntityEndPortalRenderer"))
				return false;
			// #endif
		}
		...
		if(mixinClassName.endsWith("_NoOF"))
			return !hasOF;
		// #if FABRIC
		if(mixinClassName.endsWith("_Iris"))
			return hasIris;
		// #endif
		return true;
	}
