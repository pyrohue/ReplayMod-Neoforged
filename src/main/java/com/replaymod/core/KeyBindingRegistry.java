...
public class KeyBindingRegistry extends EventRegistrations
{
	...
	private Multimap<Integer, Supplier<Boolean>> rawHandlers =
		ArrayListMultimap.create();
	private static List<KeyBinding> keybinds = new ArrayList<>();

	public Binding registerKeyBinding(String name, int keyCode,
		Runnable whenPressed, boolean onlyInRepay)
	{
	...
	public Binding registerRepeatedKeyBinding(String name, int keyCode,
		Runnable whenPressed, boolean onlyInRepay)
	{
		Binding binding = registerKeyBinding(name, keyCode, onlyInRepay);
	...
	private Binding registerKeyBinding(String name, int keyCode,
		boolean onlyInRepay)
	{
		...
		if(binding == null)
		{
			...
			Identifier id = Identifier.of(MOD_ID,
				name.substring(LangResourcePack.LEGACY_KEY_PREFIX.length()));
			// #if MC>=11600
			String key =
				String.format("key.%s.%s", id.getNamespace(), id.getPath());
			KeyBinding keyBinding =
				new KeyBinding(key, InputUtil.Type.KEYSYM, keyCode, CATEGORY);
			// KeyBindingHelper.registerKeyBinding(keyBinding);
			keybinds.add(keyBinding);
			// #else
			// $$ FabricKeyBinding fabricKeyBinding =
			// FabricKeyBinding.Builder.create(id, InputUtil.Type.KEYSYM,
			// keyCode, CATEGORY).build();
			// $$
	...
	private void invokeKeyBindingHandlers(Binding binding,
		Collection<Runnable> handlers)
	{
		for(final Runnable runnable : handlers)
		{
			...
			}catch(Throwable cause)
			{
				CrashReport crashReport =
					CrashReport.create(cause, "Handling Key Binding");
				CrashReportSection category =
					crashReport.addElement("Key Binding");
	...
	{
		on(KeyEventCallback.EVENT, (keyCode, scanCode, action,
			modifiers) -> handleRaw(keyCode, action));
	}
	...
	private boolean handleRaw(int keyCode, int action)
	{
		...
		for(final Supplier<Boolean> handler : rawHandlers.get(keyCode))
		{
			...
			{
				CrashReport crashReport =
					CrashReport.create(cause, "Handling Raw Key Binding");
				CrashReportSection category =
					crashReport.addElement("Key Binding");
	...
	public class Binding
	{
		...
		public String getBoundKey()
		{
			...
			{
				// Apparently windows likes to press strange keys, see
				// https://www.replaymod.com/forum/thread/55
				return "Unknown";
			}
		}
		...
		public void registerAutoActivationSupport(boolean active,
