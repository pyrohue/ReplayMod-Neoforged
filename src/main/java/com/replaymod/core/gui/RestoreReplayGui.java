...
public class RestoreReplayGui extends AbstractGuiScreen<RestoreReplayGui>
{
	...
	public final GuiPanel textPanel =
		new GuiPanel().setLayout(new VerticalLayout().setSpacing(3));
	public final GuiPanel buttonPanel =
		new GuiPanel().setLayout(new HorizontalLayout().setSpacing(5));
	public final GuiPanel contentPanel = new GuiPanel(this)
	...
	public final GuiButton yesButton =
		new GuiButton(buttonPanel).setSize(150, 20).setI18nLabel("gui.yes");
	public final GuiButton noButton =
		new GuiButton(buttonPanel).setSize(150, 20).setI18nLabel("gui.no");
	...
	public RestoreReplayGui(ReplayMod core, GuiScreen parent, File file)
	{
		...
		{
			@Override
			protected void layout(RestoreReplayGui container, int width,
				int height)
			{
	...
	{
		GuiLabel label =
			new GuiLabel().setI18nText("replaymod.gui.replaysaving.title")
				.setColor(Colors.BLACK);
		GuiProgressBar progressBar = new GuiProgressBar().setHeight(14);
		GuiPanel savingProcess = new GuiPanel().setLayout(new VerticalLayout())
			.addElements(new VerticalLayout.Data(0.5), label, progressBar);
		new Thread(() -> {
			core.runLater(
				() -> core.getBackgroundProcesses().addProcess(savingProcess));
			try
			{
			...
			}catch(IOException e)
			{
				...
				CrashReport crashReport =
					CrashReport.create(e, "Recovering replay file");
				core.runLater(() -> Utils.error(LOGGER,
					VanillaGuiScreen.wrap(getMinecraft().currentScreen),
					crashReport, () -> {}));
			}finally
			{
				core.runLater(() -> core.getBackgroundProcesses()
					.removeProcess(savingProcess));
			}
		}).start();
	}
	...
	private void tryRecover(Consumer<Float> progress) throws IOException
	{
		...
		// If we don't do this, then re-writing packet data below can actually
		// overwrite uncommitted packet data!
		replayFile.save();
		...
		if(metaData != null && metaData.getDuration() == 0)
		{
			...
			// We need to re-write the packet data in case there are any
			// incomplete packets dangling at the end
			try(ReplayInputStream in = replayFile
				.getPacketData(MCVer.getPacketTypeRegistry(State.LOGIN));
				ReplayOutputStream out = replayFile.writePacketData())
			{
				while(true)
				{
					// To prevent failing at un-parsable packets and to support
					// recovery in minimal mode,
					// we do not use the ReplayIn/OutputStream methods but
					// instead parse the packets ourselves.
					int time = readInt(in);
			...
			try(OutputStream out = replayFile.write("metaData.json"))
			{
				metaData.setGenerator(
					metaData.getGenerator() + "(+ ReplayMod Replay Recovery)");
