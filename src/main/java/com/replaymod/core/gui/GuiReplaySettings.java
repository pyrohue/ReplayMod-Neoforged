...
public class GuiReplaySettings extends AbstractGuiScreen<GuiReplaySettings>
{
	...
	public GuiReplaySettings(
		final net.minecraft.client.gui.screen.Screen parent,
		final SettingsRegistry settingsRegistry)
	{
		final GuiButton doneButton = new GuiButton(this)
			.setI18nLabel("gui.done").setSize(200, 20).onClick(new Runnable()
			{
				@Override
				public void run()
				{
		...
		final GuiPanel allElements =
			new GuiPanel(this).setLayout(new HorizontalLayout().setSpacing(10));
		GuiPanel leftColumn =
			new GuiPanel().setLayout(new VerticalLayout().setSpacing(4));
		GuiPanel rightColumn =
			new GuiPanel().setLayout(new VerticalLayout().setSpacing(4));
		allElements.addElements(new VerticalLayout.Data(0), leftColumn,
			rightColumn);
		HorizontalLayout.Data leftHorizontalData = new HorizontalLayout.Data(1);
		HorizontalLayout.Data rightHorizontalData =
			new HorizontalLayout.Data(0);
		int i = 0;
		for(final SettingsRegistry.SettingKey<?> key : settingsRegistry
			.getSettings())
		{
			if(key.getDisplayString() != null)
			{
				...
				if(key.getDefault() instanceof Boolean)
				{
					...
					final SettingsRegistry.SettingKey<Boolean> booleanKey =
						(SettingsRegistry.SettingKey<Boolean>)key;
					final GuiToggleButton button = new GuiToggleButton<>()
						.setSize(150, 20).setI18nLabel(key.getDisplayString())
						.setSelected(settingsRegistry.get(booleanKey) ? 0 : 1)
						.setValues(I18n.translate("options.on"),
					...
					element = button.onClick(new Runnable()
					{
						...
						public void run()
						{
							settingsRegistry.set(booleanKey,
								button.getSelected() == 0);
				...
				}else if(key instanceof SettingsRegistry.MultipleChoiceSettingKey)
				{
					...
					MultipleChoiceDropdownEntry[] entries =
						new MultipleChoiceDropdownEntry[values.size()];
					int selected = 0;
					Object currentValue =
						settingsRegistry.get(multipleChoiceKey);
					for(int j = 0; j < entries.length; j++)
					{
						Object value = values.get(j);
						entries[j] = new MultipleChoiceDropdownEntry(value,
							I18n.translate(multipleChoiceKey.getDisplayString())
								+ ": " + I18n.translate(value.toString()));
					...
					final GuiDropdownMenu<MultipleChoiceDropdownEntry> menu =
						new GuiDropdownMenu<MultipleChoiceDropdownEntry>()
						{
							@Override
						...
						.onSelection(new Consumer<Integer>()
						{
							...
							public void consume(Integer obj)
							{
								settingsRegistry.set(
									(SettingsRegistry.SettingKey)multipleChoiceKey,
				...
				}else
				{
					throw new IllegalArgumentException("Type "
						+ key.getDefault().getClass() + " not supported.");
				}
		...
		{
			@Override
			protected void layout(GuiReplaySettings container, int width,
				int height)
			{
