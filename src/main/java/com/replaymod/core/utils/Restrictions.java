 ...
 *
 * @see <a href="https://gist.github.com/Johni0702/2547c463e51f65f312cb">Replay
 *      Restrictions Gist</a>
 */
public class Restrictions
{
	// #if MC>=11400
	public static final Identifier PLUGIN_CHANNEL =
		Identifier.of("replaymod", "restrict");
	// #else
	...
	public String handle(CustomPayloadS2CPacket packet)
	{
		...
		PacketByteBuf buffer = new PacketByteBuf(
			Unpooled.wrappedBuffer(((Payload)packet.payload()).bytes()));
		// #elseif MC>=12002
		...
		// PacketBuffer(Unpooled.wrappedBuffer(packet.func_149168_d()));
		// #endif
	...
	public static final CustomPayload.Id<Payload> ID =
		new CustomPayload.Id<>(PLUGIN_CHANNEL);
	public static final PacketCodec<? super PacketByteBuf, Payload> CODEC =
		PacketCodec.ofStatic((buf, payload) -> buf.writeBytes(payload.bytes()),
			buf -> {
