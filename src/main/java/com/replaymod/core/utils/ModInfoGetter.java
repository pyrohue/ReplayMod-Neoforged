...
class ModInfoGetter
{
	...
	{
		Map<String, ModInfo> modInfoMap =
			LoadingModList.get().getMods().stream()
				.map(m -> new ModInfo(m.getModId(), m.getDisplayName(),
		...
		return net.minecraft.registry.Registries.REGISTRIES.stream()
			...
			.map(Identifier::getNamespace).filter(s -> !s.equals("minecraft"))
			.distinct().map(modInfoMap::get).filter(Objects::nonNull)
