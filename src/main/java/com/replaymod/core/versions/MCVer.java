...
public class MCVer
{
	...
	public static PacketTypeRegistry getPacketTypeRegistry(State state)
	{
		return PacketTypeRegistry
			.get(ProtocolVersion.getProtocol(getProtocolVersion()), state);
	}
	...
	public static void resizeMainWindow(MinecraftClient mc, int width,
		int height)
	{
		// #if MC>=11400
		...
		mainWindow.invokeOnFramebufferSizeChanged(window.getHandle(), width,
			height);
		// #else
	...
	// #else
	// $$ ResourcePackRepositoryAccessor acc = (ResourcePackRepositoryAccessor)
	// repo;
	// $$ acc.setActive(false);
	...
	public static List<VertexFormatElement> getElements(
	...
	// #if MC<10800
	// $$ public static RenderManager
	// getRenderManager(@SuppressWarnings("unused") Minecraft mc) {
	...
	// #if MC>=11400
	public static Optional<ClickableWidget> findButton(
		Iterable<? extends Element> buttonList,
		...
		@SuppressWarnings("unused") int id)
	{
		...
		for(Element e : buttonList)
		{
			...
			{
				Optional<ClickableWidget> button =
					findButton(((ParentElement)e).children(), text, id);
				if(button.isPresent())
				{
					return button;
				}
			...
			if(b.getMessage() != null
				&& b.getMessage().copyContentOnly().equals(message))
			{
				return Optional.of(b);
			}
			// #endif
	...
	// #else
	// $$ public static Optional<GuiButton> findButton(Iterable<GuiButton>
	// buttonList, @SuppressWarnings("unused") String text, int id) {
	...
	// #if MC>=10904
	// TODO: this can be inlined once
	// https://github.com/SpongePowered/Mixin/issues/305 is fixed
	public static Vec3d getPosition(Particle particle, float partialTicks)
	{
		ParticleAccessor acc = (ParticleAccessor)particle;
		double x = acc.getPrevPosX()
			+ (acc.getPosX() - acc.getPrevPosX()) * partialTicks;
		double y = acc.getPrevPosY()
			+ (acc.getPosY() - acc.getPrevPosY()) * partialTicks;
		double z = acc.getPrevPosZ()
			+ (acc.getPosZ() - acc.getPrevPosZ()) * partialTicks;
	...
	// $$ return new Vec3d(entity.trackedX / 4096.0, entity.trackedY / 4096.0,
	// entity.trackedZ / 4096.0);
	// $$ }
	// #endif
	...
	public static void openFile(File file)
	{
		...
		// $$ case WINDOWS:
		// $$ Runtime.getRuntime().exec(String.format("cmd.exe /C start \"Open
		// file\" \"%s\"", path));
		// $$ return;
		// $$ case OSX:
	...
	// #if MC>=11700
	public static org.joml.Quaternionf quaternion(float angle,
		org.joml.Vector3f axis)
	{
		// #if MC>=11903
		return new org.joml.Quaternionf().fromAxisAngleDeg(axis.x, axis.y,
			axis.z, angle);
		// #else
	...
	public static Matrix4f ortho(float left, float right, float top,
	...
	public static void emitLine(MatrixStack matrixStack, BufferBuilder buffer,
		Vector2f p1, Vector2f p2, int color)
	{
		emitLine(matrixStack, buffer, new Vector3f(p1.x, p1.y, 0),
			new Vector3f(p2.x, p2.y, 0), color);
	}

	public static void emitLine(MatrixStack matrixStack, BufferBuilder buffer,
		Vector3f p1, Vector3f p2, int color)
	{
		int r = color >> 24 & 0xff;
		int g = color >> 16 & 0xff;
		int b = color >> 8 & 0xff;
	...
	// $$ public static void alphaFunc(int func, float ref) { glAlphaFunc(func,
	// ref); }
	...
	public static abstract class Keyboard
	{
		...
		// org.lwjgl.input.Keyboard.KEY_END;
		...
		// org.lwjgl.input.Keyboard.KEY_DELETE;
		...
		public static boolean isKeyDown(int keyCode)
		{
			// #if MC>=11500
			return InputUtil
				.isKeyPressed(getMinecraft().getWindow().getHandle(), keyCode);
			// #else
			// #if MC>=11400
			// $$ return
			// InputUtil.isKeyPressed(getMinecraft().window.getHandle(),
			// keyCode);
			// #else
			// #if MC>=11400
