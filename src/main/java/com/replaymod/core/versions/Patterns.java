...
class Patterns
{
	// #if MC>=10904
	@Pattern
	private static void addCrashCallable(CrashReportSection category,
		String name, CrashCallable<String> callable)
	{
		// #if MC>=11200
	...
	// #else
	// $$ @Pattern
	// $$ private static void addCrashCallable(CrashReportCategory category,
	// String name, Callable<String> callable) {
	...
	@Pattern
	private static void Entity_setPos(Entity entity, double x, double y,
		double z)
	{
		// #if MC>=11500
		entity.setPos(x, y, z);
		// #else
		// $$ { net.minecraft.entity.Entity self = entity; self.x = x; self.y =
		// y; self.z = z; }
		// #endif
	}
	...
	private static int getHeight(ClickableWidget button)
	{
		...
		// #else
		// $$ return ((com.replaymod.core.mixin.AbstractButtonWidgetAccessor)
		// button).getHeight();
		// #endif
	}
	// #else
	...
	private static String readString(PacketByteBuf buffer, int max)
	{
		...
		// #else
		// $$ return com.replaymod.core.versions.MCVer.tryReadString(buffer,
		// max);
		// #endif
	}
	...
	// #else
	// $$ private static void setRenderViewEntity(Minecraft mc, EntityLivingBase
	// entity) {
	...
	private static Iterable<Entity> loadedEntityList(ClientWorld world)
	{
		...
		// #else
		// $$ return ((java.util.List<net.minecraft.entity.Entity>)
	...
	{}
	// #else
	// $$ private static Collection<Entity>[] getEntitySectionArray(WorldChunk
	// chunk) {
	// #if MC>=11700
	...
	private static List<? extends PlayerEntity> playerEntities(World world)
	{
		...
		// #else
		// $$ return ((List<? extends net.minecraft.entity.player.EntityPlayer>)
	...
	@Pattern
	private static void scheduleOnMainThread(MinecraftClient mc,
		Runnable runnable)
	{
		// #if MC>=11400
		mc.send(runnable);
	...
	// #else
	// $$ @Pattern
	// $$ private static BufferBuilder Tessellator_getBuffer(Tessellator
	// tessellator) {
	// $$ //#if MC>=10800
	...
	// #if MC<11700
	// $$ @Pattern
	// $$ private static void BufferBuilder_beginPosCol(BufferBuilder buffer,
	// int mode) {
	// #if MC>=10809
	...
	// $$
	// $$ @Pattern
	// $$ private static void BufferBuilder_addPosCol(BufferBuilder buffer,
	// double x, double y, double z, int r, int g, int b, int a) {
	// #if MC>=10809
	// $$ buffer.vertex(x, y, z).color(r, g, b, a).next();
	// #else
	// $$ { WorldRenderer $buffer = buffer; double $x = x; double $y = y; double
	// $z = z; $buffer.setColorRGBA(r, g, b, a); $buffer.addVertex($x, $y, $z);
	// }
	// #endif
	// $$ }
	// $$
	// $$ @Pattern
	// $$ private static void BufferBuilder_beginPosTex(BufferBuilder buffer,
	// int mode) {
	// #if MC>=10809
	...
	// $$
	// $$ @Pattern
	// $$ private static void BufferBuilder_addPosTex(BufferBuilder buffer,
	...
	// $$
	// $$ @Pattern
	// $$ private static void BufferBuilder_beginPosTexCol(BufferBuilder buffer,
	// int mode) {
	// #if MC>=10809
	...
	// $$
	// $$ @Pattern
	// $$ private static void BufferBuilder_addPosTexCol(BufferBuilder buffer,
	// double x, double y, double z, float u, float v, int r, int g, int b, int
	// a) {
	// #if MC>=10809
	// $$ buffer.vertex(x, y, z).texture(u, v).color(r, g, b, a).next();
	// #else
	// $$ { WorldRenderer $buffer = buffer; double $x = x; double $y = y; double
	// $z = z; float $u = u; float $v = v; $buffer.setColorRGBA(r, g, b, a);
	// $buffer.addVertexWithUV($x, $y, $z, $u, $v); }
	// #endif
	// $$ }
	// #else
	...
	private static float getRenderPartialTicks(MinecraftClient mc)
	{
		...
		// #else
		// $$ return ((com.replaymod.core.mixin.MinecraftAccessor)
		// mc).getTimer().renderPartialTicks;
		// #endif
	}
	...
	private static String getBoundKeyName(KeyBinding keyBinding)
	{
		...
		// #else
		// $$ return
		// org.lwjgl.input.Keyboard.getKeyName(keyBinding.getKeyCode());
		// #endif
	}
	...
	private static PositionedSoundInstance master(Identifier sound, float pitch)
	{
		...
		// #else
		// $$ return PositionedSoundRecord.createPositionedSoundRecord(sound,
		// pitch);
		// #endif
	}
	...
	@Pattern
	private static void BufferBuilder_beginLineStrip(BufferBuilder buffer,
	...
	@Pattern
	private static void BufferBuilder_beginQuads(BufferBuilder buffer,
		VertexFormat vertexFormat)
	{
		// #if MC>=11700
		// $$
	...
	{
		// #if MC>=11700
		com.mojang.blaze3d.systems.RenderSystem.getModelViewStack().translate(x,
			y, z);
		// #else
	...
	private static void GL11_glRotatef(float angle, float x, float y, float z)
	{
		...
		com.mojang.blaze3d.systems.RenderSystem.getModelViewStack()
			.rotate(com.replaymod.core.versions.MCVer.quaternion(angle,
				new org.joml.Vector3f(x, y, z)));
		// #elseif MC>=11700
		// $$
		// com.mojang.blaze3d.systems.RenderSystem.getModelViewStack().multiply(com.replaymod.core.versions.MCVer.quaternion(angle,
		// new org.joml.Vector3f(x, y, z)));
		// #else
	...
	// FIXME preprocessor bug: there are mappings for this, not sure why it
	// doesn't remap by itself
	// #if MC>=11600
	@Pattern
	...
	@SuppressWarnings("rawtypes") // preprocessor bug: doesn't work with
									// generics
	@Pattern
	private static void Futures_addCallback(ListenableFuture future,
	...
	private static CrashException crashReportToException(MinecraftClient mc)
	{
		...
		// #else
		// $$ return new CrashException(((MinecraftAccessor)
		// mc).getCrashReporter());
		// #endif
	}
	...
	private static Vec3d getTrackedPosition(Entity entity)
	{
		...
		// #else
		// $$ return
		// com.replaymod.core.versions.MCVer.getTrackedPosition(entity);
		// #endif
	}
	...
	{
		// #if MC>=11900
		((com.replaymod.core.mixin.SimpleOptionAccessor<Double>)(Object)options
			.getGamma()).setRawValue(value);
		// #elseif MC>=11400
		// $$ options.gamma = value;
		// #else
	...
	@Pattern
	private static Resource getResource(ResourceManager manager, Identifier id)
		throws IOException
	{
		// #if MC>=11900
	...
	@Pattern
	private static List<ItemStack> DefaultedList_ofSize_ItemStack_Empty(
		int size)
	{
		// #if MC>=11100
	...
	@Pattern
	private static void setSoundVolume(GameOptions options,
		SoundCategory category, float value)
	{
		// #if MC>=11903
	...
	// #if MC>=11700
	@Pattern
	private static Matrix4f Matrix4f_perspectiveMatrix(float left, float right,
		float top, float bottom, float zNear, float zFar)
	{
		// #if MC>=11903
		return com.replaymod.core.versions.MCVer.ortho(left, right, top, bottom,
			zNear, zFar);
		// #else
		// $$ return Matrix4f.projectionMatrix(left, right, top, bottom, zNear,
		// zFar);
		// #endif
	}
	// #else
	...
	// #else
	// $$ @Pattern
	// $$ public Integer getPacketId(NetworkState state, NetworkSide side,
