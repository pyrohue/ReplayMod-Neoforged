...
public class SchedulerImpl implements Scheduler
{
	 ...
	 * If this is the case, subsequent calls to runLater have to be delayed
	 * until all scheduled tasks have been
	...
	// This works fine for MC since it uses the queue only for packet handling
	// but breaks our assumption that
	// stuff submitted via runLater is actually always run (e.g. recording might
	// not be fully stopped because parts
	// of that are run via runLater and stopping the recording happens right
	// around the time MC clears the queue).
	// Luckily, that's also the version where MC pulled out the executor
	// implementation, so we can just spin up our own.
	...
	public final ReplayModExecutor executor =
		new ReplayModExecutor("Client/ReplayMod");
			...
			executor.send(() -> {
				...
				}catch(CrashException e)
				{
					...
					System.err.println(e.getReport()
						.asString(ReportType.MINECRAFT_CRASH_REPORT));
					// #else
