...
public class ReplayMod implements Module, Scheduler
{
	...
	public static final Identifier TEXTURE =
		Identifier.of("replaymod", "replay_gui.png");
	public static final int TEXTURE_SIZE = 256;
	public static final Identifier LOGO_FAVICON =
		Identifier.of("replaymod", "favicon_logo.png");
	...
	private final GuiBackgroundProcesses backgroundProcesses =
		new GuiBackgroundProcesses();
	public final ReplayFoldersService folders =
		new ReplayFoldersService(settingsRegistry);
	public final ReplayFilesService files = new ReplayFilesService(folders);

	/**
	 * Whether the current MC version is supported by the embedded ReplayStudio
	 * version.
	 * If this is not the case (i.e. if this is variable true), any feature of
	 * the RM which depends on the ReplayStudio
	 * lib will be disabled.
	 *
	 * Only supported on Fabric builds, i.e. will always be false / crash the
	 * game with Forge/pre-1.14 builds.
	 * (specifically the code below and MCVer#getProtocolVersion make this
	 * assumption)
	 */
	...
	public ReplayMod(ReplayModBackend backend)
	{
		...
		if(!ProtocolVersion.isRegistered(MCVer.getProtocolVersion())
			&& !Boolean.parseBoolean(
				System.getProperty("replaymod.skipversioncheck", "false")))
		{
			minimalMode = true;
		}
	...
	private static DirectoryResourcePack createJGuiResourcePack()
	{
		...
		// #if MC>=12006
		return new DirectoryResourcePack(
			new ResourcePackInfo(JGUI_RESOURCE_PACK_NAME, Text.literal("jGui"),
				ResourcePackSource.NONE, Optional.empty()),
			folder.toPath())
		{
			// #elseif MC>=11903
			// $$ return new DirectoryResourcePack(JGUI_RESOURCE_PACK_NAME,
			// folder.toPath(), true) {
			// #else
			...
			// #if MC>=11903
			@Override
			public net.minecraft.resource.InputSupplier<InputStream> openRoot(
				String... segments)
			{
			...
			// #else
			// $$ @Override
			// $$ protected InputStream openFile(String resourceName) throws
			// IOException {
			// $$ try {
	...
	public void initClient()
	{
		...
		// Post 1.19 this has become non-trivial to do, install Sodium+Bobby or
		// OptiFine if you need it
	...
	private void printToChat(boolean warning, String message, Object... args)
	{
		if(getSettingsRegistry().get(Setting.NOTIFICATIONS))
		{
			...
			Style alert = Style.EMPTY
				.withColor(warning ? Formatting.RED : Formatting.DARK_GREEN);
			// #else
			// $$ Style coloredDarkGray = new
			// Style().setColor(Formatting.DARK_GRAY);
			// $$ Style coloredGold = new Style().setColor(Formatting.GOLD);
			// $$ Style alert = new Style().setColor(warning ? Formatting.RED :
			// Formatting.DARK_GREEN);
			// #endif
			Text text =
				net.minecraft.text.Text.literal("[").setStyle(coloredDarkGray)
					.append(net.minecraft.text.Text
						.translatable("replaymod.title").setStyle(coloredGold))
					.append(net.minecraft.text.Text.literal("] "))
					.append(net.minecraft.text.Text.translatable(message, args)
						.setStyle(alert));
			// #else
			// $$ ChatStyle coloredDarkGray = new
			// ChatStyle().setColor(EnumChatFormatting.DARK_GRAY);
			// $$ ChatStyle coloredGold = new
			// ChatStyle().setColor(EnumChatFormatting.GOLD);
			// $$ IChatComponent text = new
			// ChatComponentText("[").setChatStyle(coloredDarkGray)
			// $$ .appendSibling(new
			// ChatComponentTranslation("replaymod.title").setChatStyle(coloredGold))
			// $$ .appendSibling(new ChatComponentText("] "))
			// $$ .appendSibling(new ChatComponentTranslation(message,
			// args).setChatStyle(new ChatStyle()
			// $$ .setColor(warning ? EnumChatFormatting.RED :
			// EnumChatFormatting.DARK_GREEN)));
			// #endif
			// Send message to chat GUI
			// The ingame GUI is initialized at startup, therefore this is
			// possible before the client is connected
	...
	// This method is static because it depends solely on the environment, not
	// on the actual RM instance.
	public static boolean isMinimalMode()
	{
		return ReplayMod.instance.minimalMode;
	}

	public static boolean isCompatible(int fileFormatVersion,
		int protocolVersion)
	{
		if(isMinimalMode())
		{
		...
		{
			return new ReplayStudio().isCompatible(fileFormatVersion,
				protocolVersion, MCVer.getProtocolVersion());
		}
	}
}
