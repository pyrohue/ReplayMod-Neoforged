...
public class ShaderReflection
{
	...
	static
	{
		try
		{
			...
			try
			{
				shadersClass = Class.forName("shadersmod.client.Shaders"); // Pre
																			// Optifine
																			// 1.12.2
																			// E1
			}catch(ClassNotFoundException ignore)
			{
				shadersClass = Class.forName("net.optifine.shaders.Shaders"); // Post
																				// Optifine
																				// 1.12.2
																				// E1
			}
			shaders_frameTimeCounter =
				shadersClass.getDeclaredField("frameTimeCounter");
			shaders_frameTimeCounter.setAccessible(true);

			shaders_isShadowPass =
				shadersClass.getDeclaredField("isShadowPass");
			shaders_isShadowPass.setAccessible(true);

			shaders_beginRender = shadersClass.getDeclaredMethod("beginRender",
			...
			renderGlobal_chunksToUpdateForced =
			...
			config_isShaders =
				Class.forName("Config").getDeclaredMethod("isShaders");
