...
public class NoGuiScreenshot
{
	...
	public static ListenableFuture<NoGuiScreenshot> take(
		final MinecraftClient mc, final int width, final int height)
	{
		...
		Runnable runnable = new Runnable()
		{
			...
			public void run()
			{
				...
				try
				{
					...
					// #if MC>=12006
					// $$ mc.gameRenderer.renderWorld(tickDelta,
					// System.nanoTime());
					// #elseif MC>=11500
					// $$ mc.gameRenderer.renderWorld(tickDelta,
					// System.nanoTime(), new MatrixStack());
					// #else
					// #if MC>=11400
					// $$ mc.gameRenderer.renderWorld(tickDelta,
					// System.nanoTime());
				...
				try
				{
					...
					Image image = new Image(ScreenshotRecorder.takeScreenshot(
						...
						mc.getFramebuffer()));
					// #else
					// $$ // We're using Minecraft's ScreenShotHelper even
					// though it writes the screenshot to
					// $$ // disk for better maintainability
					// $$ File tmpFolder = Files.createTempDir();
					// $$ Image image;
					// $$ try {
					// $$ ScreenShotHelper.saveScreenshot(tmpFolder, "tmp",
					// frameWidth, frameHeight, mc.getFramebuffer());
					// $$ File screenshotFile = new File(tmpFolder,
					// "screenshots/tmp");
					...
					// Scale & crop
					float scaleFactor = Math.max((float)width / imageWidth,
						(float)height / imageHeight);
					int croppedWidth = Math.min(
						Math.max(0, (int)(width / scaleFactor)), imageWidth);
					int croppedHeight = Math.min(
						Math.max(0, (int)(height / scaleFactor)), imageHeight);
					int offsetX = (imageWidth - croppedWidth) / 2;
					int offsetY = (imageHeight - croppedHeight) / 2;
					image = image.scaledSubRect(offsetX, offsetY, croppedWidth,
						croppedHeight, width, height);
		...
		// Make sure we are not somewhere in the middle of the rendering process
		// but always at the beginning
		// of the game loop. We cannot use the addScheduledTask method as it'll
		// run the task if called
