...
public class GuiReplayOverlay extends AbstractGuiOverlay<GuiReplayOverlay>
{
	...
	public final GuiPanel topPanel = new GuiPanel(this).setLayout(
		new HorizontalLayout(HorizontalLayout.Alignment.LEFT).setSpacing(5));
		...
		public GuiElement getTooltip(RenderInfo renderInfo)
		{
			...
			if(tooltip != null)
			{
				...
				tooltip.setText(I18n.translate(label) + " ("
					+ mod.keyPlayPause.getBoundKey() + ")");
			}
			return tooltip;
		}
	}.setSize(20, 20).setTexture(ReplayMod.TEXTURE, TEXTURE_SIZE)
		.setTooltip(new GuiTooltip());
	public final GuiSlider speedSlider =
		new GuiSlider().setSize(100, 20).setSteps(37); // 0.0 is not included
	public final GuiMarkerTimeline timeline;

	/**
	 * This is not used by the replay module itself but may be used by other
	 * modules/extras to show
	 * when they're active.
	 */
	public final GuiPanel statusIndicatorPanel =
		new GuiPanel(this).setSize(100, 16)
	...
	public GuiReplayOverlay(final ReplayHandler replayHandler)
	{
		...
		speedSlider.onValueChanged(new Runnable()
		{
			...
			public void run()
			{
				...
				speedSlider.setText(
					I18n.translate("replaymod.gui.speed") + ": " + speed + "x");
				ReplaySender replaySender = replayHandler.getReplaySender();
				if(!replaySender.paused())
				{
	...
	public void draw(GuiRenderer renderer, ReadableDimension size,
		RenderInfo renderInfo)
	{
		// Do not render overlay if all hud, or this one specifically, is hidden
		// and we're not in some popup
		if((getMinecraft().options.hudHidden || hidden) && isAllowUserInput())
		{
			// Note that this only applies to when the mouse is visible,
			// otherwise
	...
	private class EventHandler extends EventRegistrations
	{
		...
		private void onKeyBindingEvent()
		{
			...
			while(gameSettings.chatKey.wasPressed()
				|| gameSettings.commandKey.wasPressed())
			{
				if(!isMouseVisible())
				{
		...
		private void onKeyInput(int key, int action)
		{
			if(action != KeyEventCallback.ACTION_PRESS)
				return;
			// Allow F1 to be used to hide the replay gui (e.g. for recording
			// with OBS)
