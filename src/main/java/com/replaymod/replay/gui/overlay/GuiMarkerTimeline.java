...
public class GuiMarkerTimeline extends AbstractGuiTimeline<GuiMarkerTimeline>
	implements Draggable, Typeable
{
	...
	public GuiMarkerTimeline(@Nonnull ReplayHandler replayHandler)
	{
		...
		{
			this.markers =
				replayHandler.getReplayFile().getMarkers().or(HashSet::new);
		}catch(IOException e)
		{
			ReplayModReplay.LOGGER.error("Failed to get markers from replay",
				e);
		...
		this.saveMarkers = (markers) -> {
			...
			{
				ReplayModReplay.LOGGER.error("Failed to save markers to replay",
					e);
			}
		};
	}

	public GuiMarkerTimeline(Set<Marker> markers,
		...
		Marker marker)
	{
		...
		int markerPos =
			clamp(marker.getTime(), getOffset(), getOffset() + visibleLength);
		double positionInVisible = markerPos - getOffset();
		double fractionOfVisible = positionInVisible / visibleLength;
		int markerX = (int)(BORDER_LEFT + fractionOfVisible
			* (size.getWidth() - BORDER_LEFT - BORDER_RIGHT));
		...
		Marker marker, int markerX)
	{
		...
		renderer.drawTexturedRect(markerX - 2,
			size.getHeight() - BORDER_BOTTOM - MARKER_SIZE, textureX, textureY,
	...
	protected Marker getMarkerAt(int mouseX, int mouseY)
	{
		...
		if(mouseX < 0
			|| mouseY < lastSize.getHeight() - BORDER_BOTTOM - MARKER_SIZE
			|| mouseX > lastSize.getWidth()
			|| mouseY > lastSize.getHeight() - BORDER_BOTTOM)
		{
			return null;
		}
		...
		for(Marker marker : markers)
		{
			int markerPos = clamp(marker.getTime(), getOffset(),
				getOffset() + visibleLength);
			double positionInVisible = markerPos - getOffset();
			double fractionOfVisible = positionInVisible / visibleLength;
	...
	public boolean mouseClick(ReadablePoint position, int button)
	{
		...
		if(marker != null)
		{
			...
			{ // Left click
				...
				if(Math.abs(lastClickTime - now) > 500)
				{ // Single click
					draggingStartX = position.getX();
					draggingTimeDelta = marker.getTime()
						- getTimeAt(position.getX(), position.getY());
				}else
				{ // Double click
					new GuiEditMarkerPopup(getContainer(), marker,
						(updatedMarker) -> {
			...
			{ // Right click
				...
				if(replayHandler != null)
				{
					...
					{
						cameraEntity.setCameraPosRot(new Location(marker.getX(),
							marker.getY(), marker.getZ(), marker.getYaw(),
							marker.getPitch()));
					}
	...
	protected String getTooltipText(RenderInfo renderInfo)
	{
		...
		if(marker != null)
		{
			return marker.getName() != null ? marker.getName()
				: I18n.translate("replaymod.gui.ingame.unnamedmarker");
		}
