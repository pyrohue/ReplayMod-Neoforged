...
public class GuiReplayViewer extends GuiScreen
{
	...
	public final GuiReplayList list =
		new GuiReplayList(this).onSelectionChanged(this::updateButtons)
	...
	public final GuiButton loadButton = new GuiButton().onClick(new Runnable()
	{
		...
		public void run()
		{
			...
			{
				Iterator<Pair<File, List<RenderJob>>> replays =
					selected.stream().filter(it -> !it.renderQueue.isEmpty())
						.map(it -> Pair.of(it.file, it.renderQueue)).iterator();
				GuiRenderQueue.processMultipleReplays(GuiReplayViewer.this, mod,
					replays, () -> {
						loading = false;
						updateButtons();
						display();
	...
	public final GuiButton renameButton = new GuiButton().onClick(new Runnable()
	{
		...
		public void run()
		{
			...
			String name =
				Utils.fileNameToReplayName(path.getFileName().toString());
			final GuiTextField nameField = new GuiTextField().setSize(200, 20)
				.setFocused(true).setText(name);
			final GuiYesNoPopup popup = GuiYesNoPopup.open(GuiReplayViewer.this,
				...
				nameField).setYesI18nLabel("replaymod.gui.rename")
				.setNoI18nLabel("replaymod.gui.cancel");
			...
			nameField.onEnter(new Runnable()
			{
				...
				popup.getYesButton().setEnabled(!nameField.getText().isEmpty()
					&& Files.notExists(Utils.replayNameToPath(path.getParent(),
						nameField.getText())));
			});
			popup.onAccept(() -> {
				...
				Path targetPath =
					Utils.replayNameToPath(path.getParent(), newName);
				try
				{
				...
				}catch(IOException e)
				{
					...
					getMinecraft().setScreen(new NoticeScreen(
						...
						net.minecraft.text.Text.translatable(
							"replaymod.gui.viewer.delete.failed1"),
						net.minecraft.text.Text
							.translatable("replaymod.gui.viewer.delete.failed2")
					// #else
	...
	public final GuiButton deleteButton = new GuiButton().onClick(() -> {
		for(GuiReplayEntry entry : list.getSelected())
		{
			...
			GuiYesNoPopup
				.open(GuiReplayViewer.this, new GuiLabel()
					.setI18nText("replaymod.gui.viewer.delete.linea")
					.setColor(Colors.BLACK),
					new GuiLabel()
						.setI18nText("replaymod.gui.viewer.delete.lineb",
							name + Formatting.RESET)
						.setColor(Colors.BLACK))
				.setYesI18nLabel("replaymod.gui.delete")
				.setNoI18nLabel("replaymod.gui.cancel").onAccept(() -> {
					try
					{
	...
	public final GuiButton settingsButton = new GuiButton(this).setSize(20, 20)
		.setTexture(ReplayMod.TEXTURE, ReplayMod.TEXTURE_SIZE)
		.setSpriteUV(20, 0)
		.setTooltip(new GuiTooltip().setI18nText("replaymod.gui.settings"))
		.onClick(() -> new GuiReplaySettings(toMinecraft(),
			getMod().getCore().getSettingsRegistry()).display());
	...
	public final GuiPanel lowerButtonPanel = new GuiPanel()
		.setLayout(new HorizontalLayout().setSpacing(5)).addElements(null,
			renameButton, deleteButton, editorButton, cancelButton);
	public final GuiPanel buttonPanel =
		new GuiPanel(this).setLayout(new VerticalLayout().setSpacing(5))
	...
	public GuiReplayViewer(ReplayModReplay mod)
	{
		...
		}catch(IOException e)
		{
			throw new CrashException(
				CrashReport.create(e, "Getting replay folder"));
		}
	...
	private void updateButtons()
	{
		...
		{
			Set<RenderJob> jobs =
				selected.stream().flatMap(entry -> entry.renderQueue.stream())
					.collect(Collectors.toSet());
			String[] tooltipLines =
				jobs.stream().map(RenderJob::getName).toArray(String[]::new);
			loadButton.setI18nLabel("replaymod.gui.viewer.bulkrender",
				jobs.size());
			loadButton.setTooltip(new GuiTooltip().setText(tooltipLines));
			loadButton.setEnabled(!jobs.isEmpty());

			String[] compatError = VideoRenderer
				.checkCompat(jobs.stream().map(RenderJob::getSettings));
			if(compatError != null)
			{
	...
	private static final GuiImage DEFAULT_THUMBNAIL =
		new GuiImage().setTexture(Utils.DEFAULT_THUMBNAIL);

	public static class GuiSelectReplayPopup
		extends AbstractGuiPopup<GuiSelectReplayPopup>
	{
		public static GuiSelectReplayPopup openGui(GuiContainer container,
			File folder)
		{
			GuiSelectReplayPopup popup =
				new GuiSelectReplayPopup(container, folder);
			popup.list.load();
			popup.open();
			return popup;
		}
		...
		private final GuiButton acceptButton = new GuiButton(popup)
			.setI18nLabel("gui.done").setSize(50, 20).setDisabled();

		private final GuiButton cancelButton =
			new GuiButton(popup).setI18nLabel("gui.cancel").setSize(50, 20);
		...
		public GuiSelectReplayPopup(GuiContainer container, File folder)
		{
			...
			popup.setLayout(new CustomLayout<GuiPanel>()
			{
				...
				{
					pos(cancelButton, width - width(cancelButton),
						height - height(cancelButton));
					pos(acceptButton, x(cancelButton) - 5 - width(acceptButton),
						y(cancelButton));
					pos(list, 0, 5);
		...
		implements Typeable
	{
		...
		// Not actually a child of this element, we just use it for text
		// manipulation
		private final GuiTextField filterTextField =
			new GuiTextField().setFocused(true);
...

		{
			onLoad((Consumer<Supplier<GuiReplayEntry>> results) -> {
				File[] files =
					folder.listFiles((FileFilter)new SuffixFileFilter(".mcpr",
						IOCase.INSENSITIVE));
				if(files == null)
				{
				...
				Arrays.sort(files, Comparator.<File> comparingLong(
					f -> lastModified.computeIfAbsent(f, File::lastModified))
					.reversed());
				for(final File file : files)
				{
					if(Thread.interrupted())
						break;
					try(ReplayFile replayFile =
						ReplayMod.instance.files.open(file.toPath()))
					{
						final Image thumb = Optional
							.ofNullable(replayFile.getThumbBytes().orNull())
							.flatMap(stream -> {
						...
						final ReplayMetaData metaData =
							replayFile.getMetaData();
						List<RenderJob> renderQueue =
							RenderJob.readQueue(replayFile);

						if(metaData != null)
						{
							results.consume(() -> new GuiReplayEntry(file,
								metaData, thumb, renderQueue)
							{
								@Override
					...
					{
						LOGGER.error("Could not load Replay File {}",
							file.getName(), e);
					}
				}
		...
		@Override
		public boolean typeKey(ReadablePoint mousePosition, int keyCode,
			char keyChar, boolean ctrlDown, boolean shiftDown)
		{
			if(keyCode == Keyboard.KEY_F1)
			{
				SettingsRegistry reg = ReplayMod.instance.getSettingsRegistry();
				reg.set(Setting.SHOW_SERVER_IPS,
					!reg.get(Setting.SHOW_SERVER_IPS));
				reg.save();
				load();
			}

			boolean filterHasPriority = !filterTextField.getText().isEmpty();
			if(filterHasPriority && filterTextField.typeKey(mousePosition,
				keyCode, keyChar, ctrlDown, shiftDown))
			{
				scrollY(0); // ensure we scroll to top if most entries are
							// filtered
				return true;
			}

			if(super.typeKey(mousePosition, keyCode, keyChar, ctrlDown,
				shiftDown))
			{
				return true;
			}
			...
			RenderInfo renderInfo)
		{
			...
			{
				boolean anyMatches =
					getListPanel().calcMinSize().getHeight() > 0;
		...
		implements Comparable<GuiReplayEntry>
	{
		...
		public final GuiLabel server =
			new GuiLabel().setColor(Colors.LIGHT_GRAY);
		public final GuiLabel date = new GuiLabel().setColor(Colors.LIGHT_GRAY);
		public final GuiPanel infoPanel =
			new GuiPanel(this).setLayout(new VerticalLayout().setSpacing(2))
		...
		public final GuiPanel durationPanel = new GuiPanel()
			.setBackgroundColor(Colors.HALF_TRANSPARENT)
			.addElements(null, duration).setLayout(new CustomLayout<GuiPanel>()
			{
				...
				public ReadableDimension calcMinSize(GuiContainer<?> container)
				{
					...
					return new Dimension(dimension.getWidth() + 2,
						dimension.getHeight() + 2);
				}
			});
		public final GuiImage renderQueueIcon = new GuiImage().setSize(10, 10)
		...
		public GuiReplayEntry(File file, ReplayMetaData metaData,
			Image thumbImage, List<RenderJob> renderQueue)
		{
			this.file = file;
			this.renderQueue = renderQueue;

			name.setText(Formatting.UNDERLINE
				+ Utils.fileNameToReplayName(file.getName()));
			if(!StringUtils.isEmpty(metaData.getCustomServerName()))
			{
				...
				|| !ReplayMod.instance.getSettingsRegistry()
					.get(Setting.SHOW_SERVER_IPS))
			{
				server.setI18nText("replaymod.gui.iphidden")
			...
			incompatible =
				!ReplayMod.isCompatible(metaData.getFileFormatVersion(),
			...
			{
				thumbnail =
					new GuiImage(DEFAULT_THUMBNAIL).setSize(30 * 16 / 9, 30);
			...
			duration.setText(Utils
				.convertSecondsToShortString(metaData.getDuration() / 1000));
			addElements(null, durationPanel);

			if(!renderQueue.isEmpty())
			{
				renderQueueIcon.setTooltip(new GuiTooltip().setText(renderQueue
					.stream().map(RenderJob::getName).toArray(String[]::new)));
			...
			{
				@Override
				protected void layout(GuiReplayEntry container, int width,
					int height)
				{
					pos(thumbnail, 0, 0);
					...
					{
						pos(renderQueueIcon,
							width(thumbnail) - width(renderQueueIcon), 0);
					}
				}
				...
				public ReadableDimension calcMinSize(GuiContainer<?> container)
				{
					return new Dimension(300,
						thumbnail.getMinSize().getHeight());
				}
			});
		}
