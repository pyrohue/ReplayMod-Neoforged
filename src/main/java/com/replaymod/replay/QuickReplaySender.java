 ...
 * quickly as possible.
 * To do so, it performs an initial analysis of the replay, scanning all of its
 * packets and storing entity positions
 * and chunk states while doing so.
 * This allows it to later jump to any time by doing a diff from the current
 * time (including backwards jumping).
 */
@ChannelHandler.Sharable
public class QuickReplaySender extends ChannelHandlerAdapter
	implements ReplaySender
{
	...
	{
		this.mod = mod;
		this.replay = new RandomAccessReplay(replayFile,
			getPacketTypeRegistry(State.PLAY))
		{
				...
				com.replaymod.replaystudio.protocol.Packet packet)
			{
				...
				com.github.steveice10.netty.buffer.ByteBuf byteBuf =
					packet.getBuf();
				...
				// Combine id + payload
				ByteBuf bufWithId =
					channel.alloc().heapBuffer(2 + wrappedBuf.readableBytes());
	...
	public ListenableFuture<Void> initialize(Consumer<Double> progress)
	{
		...
		new Thread(() -> {
			try
			{
				...
				LOGGER.info("Initialized quick replay sender in "
			...
			}catch(Throwable e)
			{
				...
				mod.getCore().runLaterWithoutLock(() -> {
					mod.getCore().printWarningToChat(
						"Error initializing quick replay sender: %s",
	...
	private void ensureInitialized(Runnable body)
	{
		if(initPromise == null)
		{
			LOGGER.warn("QuickReplaySender used without prior initialization!",
				new Throwable());
			initialize(progress -> {});
		}
