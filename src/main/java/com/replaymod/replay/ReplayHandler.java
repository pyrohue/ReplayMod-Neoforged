...
// #if MC<10800
// $$ import
// de.johni0702.minecraft.de.johni0702.minecraft.gui.element.gui.GuiLabel;
// $$ import
// de.johni0702.minecraft.de.johni0702.minecraft.gui.popup.gui.GuiInfoPopup;
// $$ import
// de.johni0702.minecraft.de.johni0702.minecraft.gui.utils.gui.Colors;F
// #endif
...
public class ReplayHandler
{
	...
	public static final String PACKET_HANDLER_NAME =
		"ReplayModReplay_packetHandler";
	...
	public ReplayHandler(ReplayFile replayFile, boolean asyncMode)
		throws IOException
	{
		Preconditions.checkState(mc.isOnThread(),
			"Must be called from Minecraft thread.");
		...
		// #if MC>=10800
		quickReplaySender =
			new QuickReplaySender(ReplayModReplay.instance, replayFile);
		// #endif
	...
	void restartedReplay()
	{
		...
		mc.disconnect();
		// #else
		// $$ // We need to re-set the GUI screen because having one with
		// `allowsUserInput = true` active during world
	...
	private void setup()
	{
		...
		// $$ if
		// ("com.dyonovan.tcnodetracker.events.ClientConnectionEvent".equals(elem.getClassName()))
		// {
		...
		// $$ FMLClientHandler.instance().connectToRealmsServer(null, 0); //
		// just to init the playClientBlock latch
		// #endif

		// #if MC>=11200
		channel = new EmbeddedChannel();
		// #else
		// $$ ChannelOutboundHandlerAdapter dummyHandler = new
		// ChannelOutboundHandlerAdapter();
		// $$ channel = new EmbeddedChannel(dummyHandler);
		// $$ channel.pipeline().remove(dummyHandler);
		// $$ channel.pipeline().removeLast();
		// #endif
		channel.pipeline().addFirst("ReplayModReplay_head",
			new DropOutboundMessagesHandler());

		quickReplaySender.setChannel(channel);
		fullReplaySender.setChannel(channel);

		// #if MC>=12006
		channel.pipeline().addLast("inbound_config",
			new NetworkStateTransitions.InboundConfigurer());
		channel.pipeline().addLast("outbound_config",
			new NetworkStateTransitions.OutboundConfigurer());
		// #else
		// #if MC>=12002
		// $$ channel.pipeline().addLast("decoder", new
		// DecoderHandler(ClientConnection.CLIENTBOUND_PROTOCOL_KEY));
		// $$ channel.pipeline().addLast("encoder", new
		// PacketEncoder(ClientConnection.SERVERBOUND_PROTOCOL_KEY));
		// #else
		// $$ channel.pipeline().addLast("decoder", new
		// DecoderHandler<>(NetworkState.CLIENTBOUND));
		// $$ channel.pipeline().addLast("encoder", new
		// PacketEncoder<>(NetworkSide.SERVERBOUND));
		// #endif
		// #if MC>=12002
		// $$ channel.pipeline().addLast("bundler", new
		// PacketBundler(ClientConnection.CLIENTBOUND_PROTOCOL_KEY));
		// #elseif MC>=11904
		// $$ channel.pipeline().addLast("bundler", new
		// PacketBundler(NetworkSide.CLIENTBOUND));
		// #endif
		// #endif
		channel.pipeline().addLast(PACKET_HANDLER_NAME,
			quickMode ? quickReplaySender : fullReplaySender);
		channel.pipeline().addLast("packet_handler", networkManager);
		channel.pipeline().fireChannelActive();

		// MC usually transitions from handshake to login via the packets it
		// sends.
		// We don't send any packets (there is no server to receive them), so we
		// need to switch manually.
		// #if MC>=12006
		networkManager.transitionInbound(LoginStates.S2C,
			new ClientLoginNetworkHandler(networkManager, mc, null, null, false,
				null, it -> {}, null));
		...
		// #if MC>=12003
		mc.getServerResourcePackProvider().init(networkManager,
	...
	public void ensureQuickModeInitialized(Runnable andThen)
	{
		...
		ListenableFuture<Void> future =
			quickReplaySender.getInitializationPromise();
		if(future == null)
		{
			InitializingQuickModePopup popup =
				new InitializingQuickModePopup(overlay);
			future = quickReplaySender.initialize(progress -> popup.progressBar
			...
			Futures.addCallback(future, new FutureCallback<Void>()
			{
				...
				{
					String message =
						"Failed to initialize quick mode. It will not be available.";
	...
	{
		private final GuiProgressBar progressBar = new GuiProgressBar(popup)
			.setSize(300, 20).setI18nLabel("replaymod.gui.loadquickmode");
	...
	public void setQuickMode(boolean quickMode)
	{
		if(ReplayMod.isMinimalMode())
		{
			throw new UnsupportedOperationException(
				"Quick Mode not supported in minimal mode.");
		}
		if(quickMode == this.quickMode)
			return;
		if(quickMode && fullReplaySender.isAsyncMode())
		{
			// If this method is called via runLater, then it cannot switch to
			// sync mode by itself as there might be
			// some rogue packets in the task queue after it. Instead the caller
			// must switch to sync mode first and
			// use runLater until all packets have been processed (when using
			// setAsyncModeAndWait, one runLater should
			// be sufficient).
			throw new IllegalStateException(
				"Cannot switch to quick mode while in async mode.");
		}
		...
		if(cam != null)
		{
			targetCameraPosition = new Location(cam.getX(), cam.getY(),
				cam.getZ(), cam.getYaw(), cam.getPitch());
		}else
		{
			targetCameraPosition = null;
		}

		channel.pipeline().replace(PACKET_HANDLER_NAME, PACKET_HANDLER_NAME,
			quickMode ? quickReplaySender : fullReplaySender);
		...
		if(quickMode)
		{
			...
			quickReplaySender
				.sendPacketsTill(fullReplaySender.currentTimeStamp());
		}else
		{
			quickReplaySender.unregister();
			fullReplaySender.sendPacketsTill(0);
			fullReplaySender
				.sendPacketsTill(quickReplaySender.currentTimeStamp());
		}
	...
	// #else
	// $$ public void ensureQuickModeInitialized(@SuppressWarnings("unused")
	// Runnable andThen) {
	// $$ GuiInfoPopup.open(overlay,
	// $$ new GuiLabel().setI18nText("replaymod.gui.noquickmode",
	// QUICK_MODE_MIN_MC).setColor(Colors.BLACK));
	// $$ }
	// $$
	// $$ public void setQuickMode(@SuppressWarnings("unused") boolean
	// quickMode) {
	// $$ throw new UnsupportedOperationException("Quick Mode not supported on
	// this version.");
	// $$ }
	// $$
	// $$ public boolean isQuickMode() {
	...
	/**
	 * Return whether camera movement by user inputs and/or server packets
	...
	/**
	 * Set whether camera movement by user inputs and/or server packets should
	 * be suppressed.
	 *
	 * @param suppressCameraMovements
	 *            {@code true} to suppress these kinds of movement,
	 ...
	 * When the entity is {@code null} or the camera entity, the camera becomes
	 * the view entity.
	 *
	 * @param e
	...
	public void spectateEntity(Entity e)
	{
		...
		if(e == cameraEntity)
		{
			cameraEntity.setCameraController(
				ReplayModReplay.instance.createCameraController(cameraEntity));
		}else
		{
			cameraEntity.setCameraController(
	...
	{
		return mc.player instanceof CameraEntity
			&& mc.player == mc.getCameraEntity();
	}
	...
	{
		return mc.player instanceof CameraEntity ? (CameraEntity)mc.player
			: null;
	}
	...
	public void doJump(int targetTime, boolean retainCameraPosition)
	{
		...
		long diff = targetTime
			- (replaySender.isHurrying() ? replaySender.getDesiredTimestamp()
				: replaySender.currentTimeStamp());
		if(diff != 0)
		{
			if(diff > 0 && diff < 5000)
			{ // Small difference and no time travel
				if(replaySender.paused())
				{
					...
					for(int i = 0; i < Math.min(diff / 50, 3); i++)
					{
						...
						// $$ mc.runTick();
						// $$ } catch (IOException e) {
						// $$ e.printStackTrace(); // This should never be
						// thrown but whatever
						// $$ }
						// #else
						mc.tick();
			...
			{ // We either have to restart the replay or send a significant
				// amount of packets
				// Render our please-wait-screen
				GuiScreen guiScreen = new GuiScreen();
				guiScreen.setBackground(AbstractGuiScreen.Background.DIRT);
				guiScreen.setLayout(
					new HorizontalLayout(HorizontalLayout.Alignment.CENTER));
				...
				RenderSystem.setProjectionMatrix(
					com.replaymod.core.versions.MCVer.ortho(0,
						...
						0, (float)(window.getFramebufferHeight()
							/ window.getScaleFactor()),
						1000, 3000)
					// #if MC>=12000
					, VertexSorter.BY_Z
				...
				// #if MC>=12006
				org.joml.Matrix4fStack matrixStack =
				...
				// $$ RenderSystem.ortho(0, window.getFramebufferWidth() /
				// window.getScaleFactor(), window.getFramebufferHeight() /
				// window.getScaleFactor(), 0, 1000, 3000);
				...
				guiScreen.toMinecraft().init(mc, window.getScaledWidth(),
					window.getScaledHeight());
				// #if MC>=12000
				DrawContext drawContext = new DrawContext(mc,
					mc.getBufferBuilders().getEntityVertexConsumers());
				guiScreen.toMinecraft().render(drawContext, 0, 0, 0);
				drawContext.draw();
				// #elseif MC>=11600
				// $$ guiScreen.toMinecraft().render(new MatrixStack(), 0, 0,
				// 0);
				// #else
				// #if MC>=11400
				...
				popMatrix();
				pushMatrix();
				mc.getFramebuffer().draw(mc.getWindow().getFramebufferWidth(),
				...
				// If the packets we just sent somehow caused the client to
				// disconnect, then the above connection tick
				// call will have unloaded the world, and we'll have to abort
				// what we were doing.
				if(mc.world == null)
				{
					return;
				...
				// $$ mc.runTick();
				// $$ } catch (IOException e) {
				// $$ e.printStackTrace(); // This should never be thrown but
				// whatever
				// $$ }
				// #else
				mc.tick();
				...
				// finally, updating the camera's position (which is not done by
				// the sync jumping)
				moveCameraToTargetPosition();

				// No need to remove our please-wait-screen. It'll vanish with
				// the next
				// render pass as it's never been a real GuiScreen in the first
				// place.
			}
		}
	}
	...
	private void skipTeleportInterpolation(Entity entity)
	{
		...
		if(entity instanceof LivingEntity && !(entity instanceof CameraEntity))
		{
			...
			e.updatePosition(ea.getInterpTargetX(), ea.getInterpTargetY(),
				ea.getInterpTargetZ());
		...
		// $$ e.setPosition(ea.getOtherPlayerMPX(), ea.getOtherPlayerMPY(),
		// ea.getOtherPlayerMPZ());
		...
		extends ChannelOutboundHandlerAdapter
	{
		...
		{
			// The embedded channel's event loop will consider every thread to
			// be in it and as such provides no
			// guarantees that only one thread is using the pipeline at any one
			// time.
			// For reading the replay sender (either sync or async) is the only
			// thread ever writing.
			// For writing it may very well happen that multiple threads want to
			// use the pipline at the same time.
			// It's unclear whether the EmbeddedChannel is supposed to be
			// thread-safe (the behavior of the event loop
			// does suggest that). However it seems like it either isn't
			// (likely) or there is a race condition.
			// See: https://www.replaymod.com/forum/thread/1752#post8045
			// (https://paste.replaymod.com/lotacatuwo)
			// To work around this issue, we just outright drop all write/flush
			// requests (they aren't needed anyway).
			// This still leaves channel handlers upstream with the threading
			// issue but they all seem to cope well with it.
			promise.setSuccess();
		}
