...
public class GuiHandler extends EventRegistrations
{
		...
		Collection<ClickableWidget> buttonList)
	{
		...
		if(mod.getReplayHandler() != null)
		{
			...
			// #if MC>=11600
			final Text BUTTON_OPTIONS =
				net.minecraft.text.Text.translatable("menu.options");
			final Text BUTTON_EXIT_SERVER =
				net.minecraft.text.Text.translatable("menu.disconnect");
			final Text BUTTON_ADVANCEMENTS =
				net.minecraft.text.Text.translatable("gui.advancements");
			final Text BUTTON_STATS =
				net.minecraft.text.Text.translatable("gui.stats");
			final Text BUTTON_OPEN_TO_LAN =
				net.minecraft.text.Text.translatable("menu.shareToLan");
			// #else
			// #if MC>=11400
			// $$ final String BUTTON_OPTIONS = I18n.translate("menu.options");
			// $$ final String BUTTON_EXIT_SERVER =
			// I18n.translate("menu.disconnect");
			// $$ final String BUTTON_ADVANCEMENTS =
			// I18n.translate("gui.advancements");
			// $$ final String BUTTON_STATS = I18n.translate("gui.stats");
			// $$ final String BUTTON_OPEN_TO_LAN =
			...
			for(ClickableWidget b : new ArrayList<>(buttonList))
			{
				...
				if(id == null)
				{
					...
					// e.g.
					// https://github.com/Pokechu22/WorldDownloader/blob/b1b279f948beec2d7dac7524eea8f584a866d8eb/share_14/src/main/java/wdl/WDLHooks.java#L491
					continue;
				}
				// #else
				// $$ Integer id = b.id;
				...
				if(id.equals(BUTTON_EXIT_SERVER))
				{
					...
					remove = true;
					addButton(guiScreen,
						new InjectedButton(guiScreen, BUTTON_EXIT_REPLAY,
							b.getX(), b.getY(), b.getWidth(), b.getHeight(),
							"replaymod.gui.exit", null, this::onButton));
				...
				if(remove)
				{
					// Moving the button far off-screen is easier to do
					// cross-version than actually removing it
					b.setX(-1000);
					b.setY(-1000);
				}
			}
			...
			{
				moveAllButtonsInRect(buttonList, achievements.getX(),
					stats.getX() + stats.getWidth(), achievements.getY(),
					Integer.MAX_VALUE, -24);
			}
			// In 1.13+ Forge, the Options button shares one row with the Open
			// to LAN button
			// #if MC<11400
			// $$ if (openToLan != null) {
	...
	/**
	 * Moves all buttons that in any way intersect a rectangle by a given amount
	 * on the y axis.
	 *
	 * @param buttons
	 *            List of buttons
	 ...
	 * @param moveBy
	 *            Signed distance to move the buttons
	 */
	private void moveAllButtonsInRect(
		...
		// #endif
		int xStart, int xEnd, int yStart, int yEnd, int moveBy)
	{
		buttons.stream()
			.filter(button -> button.getX() <= xEnd
				&& button.getX() + button.getWidth() >= xStart)
			.filter(button -> button.getY() <= yEnd
				&& button.getY() + button.getHeight() >= yStart)
			// FIXME remap bug: needs the {} to recognize the setter (it also
			// doesn't understand +=)
			.forEach(button -> {
...

	{
		on(InitScreenCallback.EVENT,
			(screen, buttons) -> ensureReplayStopped(screen));
	}
	...
	private void ensureReplayStopped(Screen guiScreen)
	{
		...
		if(mod.getReplayHandler() != null)
		{
			// Something went terribly wrong and we ended up in the main menu
			// with the replay still active.
			// To prevent players from joining live servers and using the
			// CameraEntity, try to stop the replay now.
			try
			{
	...
	private void injectIntoMainMenu(Screen guiScreen,
		Collection<ClickableWidget> buttonList)
	{
		if(!(guiScreen instanceof TitleScreen))
		{
		...
		// #if MC>=11604
		if(mod.getCore().getSettingsRegistry()
			.get(Setting.LEGACY_MAIN_MENU_BUTTON))
		{
	...
	{
		List<ClickableWidget> buttonList =
			((ScreenButtonExtension)screen).replay_getButtons();
		MainMenuButtonPosition buttonPosition = MainMenuButtonPosition.valueOf(
			mod.getCore().getSettingsRegistry().get(Setting.MAIN_MENU_BUTTON));

		// Workaround for FancyMenu v2 initializing the screen twice, likely
		// fixed in v3
		...
		if(buttonPosition == MainMenuButtonPosition.BIG)
		{
			...
			Optional<ClickableWidget> targetButton =
				findButton(buttonList, "menu.online", 14).map(Optional::of)
					// or, if someone removed the realms button, we'll
					// alternatively take the multiplayer one
					.orElseGet(
						() -> findButton(buttonList, "menu.multiplayer", 2));

			int y = targetButton
				// if we found some button, put our button at its position
				// (we'll move it out of the way shortly)
				.map(it -> it.getY())
			...
			moveAllButtonsInRect(buttonList, x, x + 200, Integer.MIN_VALUE, y,
				-24);
		...
		if(buttonPosition == MainMenuButtonPosition.BIG)
		{
			replayViewerButton = new InjectedButton(screen,
				BUTTON_REPLAY_VIEWER, pos.getX(), pos.getY(), 200, 20,
				"replaymod.gui.replayviewer", null, this::onButton);
		}else
		{
			replayViewerButton = new InjectedButton(screen,
				BUTTON_REPLAY_VIEWER, pos.getX(), pos.getY(), 20, 20, "",
				"replaymod.gui.replayviewer", this::onButton)
			{
				@Override
				// #if MC>=12000
				public void renderWidget(DrawContext context, int mouseX,
					int mouseY, float delta)
				{
					super.renderWidget(context, mouseX, mouseY, delta);
					// #elseif MC>=11904
					// $$ public void renderButton(MatrixStack context, int
					// mouseX, int mouseY, float delta) {
					// $$ super.renderButton(context, mouseX, mouseY, delta);
					// #else
					// $$ protected void renderBackground(MatrixStack context,
					// MinecraftClient client, int mouseX, int mouseY) {
					// $$ super.renderBackground(context, client, mouseX,
					// mouseY);
					// #endif

					MinecraftGuiRenderer renderer =
						new MinecraftGuiRenderer(context);
					renderer.bindTexture(GuiReplayButton.ICON);
					renderer.drawTexturedRect(this.getX() + 3, this.getY() + 3,
						0, 0, this.width - 6, this.height - 6, 1, 1, 1, 1);
				}
			};
		}
	...
	{
		ModFileInfo mod =
			FMLLoader.getLoadingModList().getModFileById("fancymenu");
		if(mod == null)
		{
			return false;
		}
	...
	private void legacyInjectIntoMainMenu(Screen guiScreen,
		Collection<ClickableWidget> buttonList)
	{
		boolean isCustomMainMenuMod = guiScreen.getClass().getName()
			.endsWith("custommainmenu.gui.GuiFakeMain");
		...
		if(buttonPosition != MainMenuButtonPosition.BIG && !isCustomMainMenuMod)
		{
			...
			replayButton.onClick(() -> new GuiReplayViewer(mod).display())
				.setTooltip(
					new GuiTooltip().setI18nText("replaymod.gui.replayviewer"));

			vanillaGui.setLayout(
				new CustomLayout<de.johni0702.minecraft.gui.container.GuiScreen>(
					vanillaGui.getLayout())
				{
					private Point pos;

					@Override
					protected void layout(
						de.johni0702.minecraft.gui.container.GuiScreen container,
						int width, int height)
					{
						if(pos == null)
						{
							// Delaying computation so we can take into account
							// buttons
							// added after our callback.
							pos = determineButtonPos(buttonPosition, guiScreen,
								buttonList);
						}
		...
		int y = findButton(buttonList, "menu.online", 14).map(Optional::of)
			// or, if someone removed the realms button, we'll alternatively
			// take the multiplayer one
			.orElse(findButton(buttonList, "menu.multiplayer", 2))
			// if we found some button, put our button at its position (we'll
			// move it out of the way shortly)
			.map(it -> it.getY())
			// and if we can't even find that one, then just guess
		...
		InjectedButton button =
			new InjectedButton(guiScreen, BUTTON_REPLAY_VIEWER, x, y, 200, 20,
				"replaymod.gui.replayviewer", null, this::onButton);
		// #if FABRIC<=0
		// $$ if (isCustomMainMenuMod) {
		// $$ // CustomMainMenu uses a different list in the event than in its
		// Fake gui
	...
	private Point determineButtonPos(MainMenuButtonPosition buttonPosition,
		Screen guiScreen, Collection<ClickableWidget> buttonList)
	{
		...
		{
			return Stream
				.of(findButton(buttonList, "menu.singleplayer", 1),
					findButton(buttonList, "menu.multiplayer", 2),
				...
				.filter(it -> buttonList.stream()
					.noneMatch(button -> button.getX() <= it.getX()
						+ it.getWidth() + 4 + 20
						&& button.getY() <= it.getY() + it.getHeight()
						&& button.getX() + button.getWidth() >= it.getX()
							+ it.getWidth() + 4
						&& button.getY() + button.getHeight() >= it.getY()))
				// then take the bottom-most and if there's two, the right-most
				.max(Comparator.<ClickableWidget> comparingInt(it -> it.getY())
					.thenComparingInt(it -> it.getX()))
		...
		}else
		{
			return Optional.of(buttonList).flatMap(buttons -> {
				...
				switch(buttonPosition)
				{
					...
					case RIGHT_OF_MULTIPLAYER:
					case RIGHT_OF_REALMS:
					return new Point(button.getX() + button.getWidth() + 4,
						button.getY());
				}
				throw new RuntimeException();
			}).orElse(topRight);
	...
	private int determineButtonIndex(Collection<ClickableWidget> buttons,
		ClickableWidget button)
	{
		...
		{
			if(other.getY() > button.getY() || other.getY() == button.getY()
				&& other.getX() > button.getX())
			{
				index++;
				continue;
			}

			if(best == null || other.getY() > best.getY()
				|| other.getY() == best.getY() && other.getX() > best.getX())
			{
				best = other;
				bestIndex = index + 1;
			}
	...
	private void onButton(InjectedButton button)
	{
		...
		// #else
		// $$ @SubscribeEvent
		// $$ public void onButton(GuiScreenEvent.ActionPerformedEvent.Pre
		// event) {
	...
	// #endif
	{
		...
		public InjectedButton(Screen guiScreen, int buttonId, int x, int y,
			int width, int height, String buttonText, String tooltip,
			// #if MC>=11400
		...
		)
		{
			super(
				...
				// guiScreen.renderTooltip(matrices,
				// net.minecraft.text.Text.translatable(tooltip), mouseX,
				// mouseY)
				// $$ : EMPTY
				// #endif
				// #if MC>=11903
	...
	{
		// The old big button below Realms/Mods which pushes other buttons
		// around.
		BIG,
		// Right of the bottom-most button in the main block of buttons (so not
		// the quit button).
		// That will generally be either RIGHT_OF_REALMS or RIGHT_OF_MODS
		// depending on version and installed mods.
		DEFAULT,
		TOP_LEFT,
		TOP_RIGHT,
