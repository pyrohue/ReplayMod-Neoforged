...
public class SpectatorCameraController implements CameraController
{
	...
	public void update(float partialTicksPassed)
	{
		...
		for(KeyBinding binding : Arrays.asList(mc.options.attackKey,
			mc.options.useKey, mc.options.jumpKey, mc.options.sneakKey,
			mc.options.forwardKey, mc.options.backKey, mc.options.leftKey,
			mc.options.rightKey))
		{
		...
		// Always make sure the camera is in the exact same spot as the
		// spectated entity
		// This is necessary as some rendering code for the hand doesn't respect
		// the view entity
