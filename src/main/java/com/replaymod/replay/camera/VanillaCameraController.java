...
public class VanillaCameraController implements CameraController
{
	...
	public void update(float partialTicksPassed)
	{
		...
		for(int i = 0; i < 6; i++)
		{ // First, get movement direction depending on keys pressed
		...
		if(direction.length() == 0)
			return;
		direction.normalise(direction); // Normalize, so we don't move quicker
										// if we hold down multiple keys
		double yawRadians = Math.toRadians(camera.getYaw());
		float yawSin = (float)Math.sin(yawRadians),
			yawCos = (float)Math.cos(yawRadians);
		// Rotate by yaw
