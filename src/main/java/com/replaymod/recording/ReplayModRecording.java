...
public class ReplayModRecording implements Module
{
	...
	// #if MC>=11400
	private static final AttributeKey<Void> ATTR_CHECKED =
		AttributeKey.newInstance("ReplayModRecording_checked");
	// #endif
			...
			new Runnable()
			{
				...
				public void run()
				{
					PacketListener packetListener =
						connectionEventHandler.getPacketListener();
	...
	public void initClient()
	{
		...
		// #if FABRIC>=1
		// #if MC>=11700
		// ClientPlayNetworking.registerGlobalReceiver(Restrictions.PLUGIN_CHANNEL,
		// (client, handler, buf, resp) -> {});
		// #else
		// $$
		// ClientSidePacketRegistry.INSTANCE.register(Restrictions.PLUGIN_CHANNEL,
		// (packetContext, packetByteBuf) -> {});
		// #endif
		// #else
		// #if MC>=11400
		// NetworkRegistry.newEventChannel(Restrictions.PLUGIN_CHANNEL, () ->
		// "0", s -> true, s -> true);
		// #else
		// $$ NetworkRegistry.INSTANCE.newChannel(Restrictions.PLUGIN_CHANNEL,
		// new RestrictionsChannelHandler());
		// #endif
		// #endif
	}
	...
	// $$ private static class RestrictionsChannelHandler extends
	...
	public void initiateRecording(ClientConnection networkManager)
	{
		...
		if(channel.pipeline().get(ReplayHandler.PACKET_HANDLER_NAME) != null)
			return;
		// #if MC>=11400
