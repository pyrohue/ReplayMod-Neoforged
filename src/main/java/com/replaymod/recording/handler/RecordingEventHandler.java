...
public class RecordingEventHandler extends EventRegistrations
{
	...
	private final List<ItemStack> playerItems =
		DefaultedList.ofSize(EQUIPMENT_SLOTS, ItemStack.EMPTY);
	...
	{
		super.unregister();
		RecordingEventSender recordingEventSender =
			((RecordingEventSender)mc.worldRenderer);
	...
	public void spawnRecordingPlayer()
	{
		try
		{
			...
			assert player != null;
			// #if MC>=12100
			packetListener.save(new EntitySpawnS2CPacket(player.getId(),
				player.getUuid(), player.getX(), player.getY(), player.getZ(),
				player.getPitch(), player.getYaw(), player.getType(), 0,
				player.getVelocity(), player.getHeadYaw()));
			// #elseif MC>=12002
			...
			// #endif
			// #if MC>=11903
			packetListener.save(new EntityTrackerUpdateS2CPacket(player.getId(),
				player.getDataTracker().getChangedEntries()));
			// #elseif MC>=11500
			// $$ packetListener.save(new
			// EntityTrackerUpdateS2CPacket(player.getId(),
			// player.getDataTracker(), true));
			// #endif
	...
	private void onPlayerTick()
	{
		...
		try
		{
			...
			Packet packet;
			if(force || Math.abs(dx) > maxRelDist || Math.abs(dy) > maxRelDist
				|| Math.abs(dz) > maxRelDist)
			{
				// #if MC>=10800
				packet = new EntityPositionS2CPacket(player);
				// #else
				// $$ // In 1.7.10 the client player entity has its posY at eye
				// height
				// $$ // but for all other entities it's at their feet (as it
				// should be).
				// $$ // So, to correctly position the player, we teleport them
				// to their feet (i.a. directly after spawn).
				// $$ // Note: this leaves the lastY value offset by the eye
				// height but because it's only used for relative
				// $$ // movement, that doesn't matter.
				// $$ S18PacketEntityTeleport teleportPacket = new
				// S18PacketEntityTeleport(player);
			...
			}else
			{
				...
				packet = new EntityS2CPacket.RotateAndMoveRelative(
					...
					player.getId(),
					// #if MC>=10904
					(short)Math.round(dx * 4096), (short)Math.round(dy * 4096),
					(short)Math.round(dz * 4096),
					// #else
					// $$ (byte) Math.round(dx * 32), (byte) Math.round(dy *
					// 32), (byte) Math.round(dz * 32),
					// #endif
					newYaw, newPitch
					// #if MC>=11600
					, player.isOnGround()
			...
			{
				packetListener.save(new EntitySetHeadYawS2CPacket(player,
					(byte)rotationYawHead));
				rotationYawHeadBefore = rotationYawHead;
			}

			packetListener
				.save(new EntityVelocityUpdateS2CPacket(player.getId(),
					// #if MC>=11400
					player.getVelocity()
				// #else
			...
			{
				packetListener.save(new EntityAnimationS2CPacket(player,
					// #if MC>=10904
			 ...
			 * S1DPacketEntityEffect pee = new S1DPacketEntityEffect(entityID,
			 * pe);
			 ...
			 * S1EPacketRemoveEntityEffect pre = new
			 * S1EPacketRemoveEntityEffect(entityID, new PotionEffect(id, 0));
			...
			for(EquipmentSlot slot : EquipmentSlot.values())
			{
				...
				{
					// ItemStack has internal mutability, so we need to make a
					// copy now if we want to compare its
					// current state with future states (e.g. dropping on modern
					// versions will set the count to zero).
					stack = stack != null ? stack.copy() : null;
					playerItems.set(index, stack);
					// #if MC>=11600
					packetListener
						.save(new EntityEquipmentUpdateS2CPacket(player.getId(),
							Collections.singletonList(Pair.of(slot, stack))));
					// #else
					// $$ packetListener.save(new
					// EntityEquipmentUpdateS2CPacket(player.getEntityId(),
					// slot, stack));
					// #endif
				}
			}
	...
	{
		// #else
		// $$ public void onBlockBreakAnim(int breakerId, int x, int y, int z,
		// int progress) {
		// #endif
	...
	private void checkForGamePaused()
	{
		if(mc.isIntegratedServerRunning())
		{
			...
			if(server != null
				&& ((IntegratedServerAccessor)server).isGamePaused())
			{
