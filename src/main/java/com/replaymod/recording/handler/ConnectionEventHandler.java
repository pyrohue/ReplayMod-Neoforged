		...
		try
		{
			...
			if(local)
			{
				...
				if(mc.getServer().getWorld(World.OVERWORLD).isDebugWorld())
				{
					...
					// #if MC>=11400
					// $$ if
					// (mc.getServer().getWorld(DimensionType.OVERWORLD).getGeneratorType()
					// == LevelGeneratorType.DEBUG_ALL_BLOCK_STATES) {
					// #else
					// $$ if
					// (mc.getIntegratedServer().getEntityWorld().getWorldType()
			...
			// #if MC>=11903
			serverInfo = networkManager
				.getPacketListener() instanceof ClientLoginNetworkHandlerAccessor loginNetworkHandler
					? loginNetworkHandler.getServerInfo() : null;
			// #else
			...
			boolean autoStart =
				core.getSettingsRegistry().get(Setting.AUTO_START_RECORDING);
			if(local)
			{
				// #if MC>=11600
			...
			}else if(serverInfo != null && serverInfo.isRealm())
			{
				...
				// #endif
				// we can't access the server name without tapping too deep in
				// the Realms Library
			...
			}else if(serverInfo != null)
			{
				...
				if(!I18n.translate("selectServer.defaultName")
					.equals(serverInfo.name))
				{
					serverName = serverInfo.name;
				}

				Boolean autoStartServer =
					ServerInfoExt.from(serverInfo).getAutoRecording();
			...
			{
				// Recording controls are not supported in minimal mode, so
				// always auto-start
				autoStart = true;
			}
			...
			Path outputPath =
				Utils.replayNameToPath(core.folders.getRecordingFolder(), name);
			...
			metaData
				.setGenerator("ReplayMod v" + ReplayMod.instance.getVersion());
			metaData.setDate(System.currentTimeMillis());
			metaData.setMcVersion(ReplayMod.instance.getMinecraftVersion());
			Channel channel =
				((NetworkManagerAccessor)networkManager).getChannel();
			packetListener = new PacketListener(core, channel, outputPath,
				replayFile, metaData);

			// #if MC>=12005
			String target = channel.pipeline().get("inbound_config") != null
				? "inbound_config" : PacketListener.DECODER_KEY;
			channel.pipeline().addBefore(target,
				PacketListener.RAW_RECORDER_KEY, packetListener);
			channel.pipeline().addAfter(target,
				PacketListener.DECODED_RECORDER_KEY,
				packetListener.new DecodedPacketListener());
			// #else
			// $$ if (channel.pipeline().get(PacketListener.DECODER_KEY) !=
			// null) {
			// $$ // Regular channel, we'll inject our recorder directly before
			// the decoder
			// $$ channel.pipeline().addBefore(PacketListener.DECODER_KEY,
			// PacketListener.RAW_RECORDER_KEY, packetListener);
			// $$ channel.pipeline().addAfter(PacketListener.DECODER_KEY,
			// PacketListener.DECODED_RECORDER_KEY, packetListener.new
			// DecodedPacketListener());
			// $$ } else {
			// $$ // Integrated server passes packets directly, there's no
			// splitting, decompression or decoding
			// $$ channel.pipeline().addFirst(PacketListener.RAW_RECORDER_KEY,
			// packetListener);
			// $$ channel.pipeline().addAfter(PacketListener.RAW_RECORDER_KEY,
			// PacketListener.DECODED_RECORDER_KEY, packetListener.new
			// DecodedPacketListener());
			// $$ }
			// #endif
			...
			guiControls =
				new GuiRecordingControls(core, packetListener, autoStart);
