	...
	private void onGuiInit(net.minecraft.client.gui.screen.Screen gui)
	{
		if(gui instanceof SelectWorldScreen || gui instanceof MultiplayerScreen)
		{
			...
			Setting<Boolean> setting =
				sp ? Setting.RECORD_SINGLEPLAYER : Setting.RECORD_SERVER;

			GuiCheckbox recordingCheckbox = new GuiCheckbox()
				.setI18nLabel("replaymod.gui.settings.record"
					+ (sp ? "singleplayer" : "server"))
			...
			vanillaGui
				.setLayout(new CustomLayout<GuiScreen>(vanillaGui.getLayout())
				{
					@Override
					protected void layout(GuiScreen container, int width,
						int height)
					{
						// size(recordingCheckbox, 200, 20);
						pos(recordingCheckbox,
							width - width(recordingCheckbox) - 5, 5);
					}
		...
		if(gui instanceof AddServerScreen)
		{
			...
			GuiButton replayButton = new GuiReplayButton().onClick(() -> {
				...
				autoRecording.onClick(() -> {
					...
					serverInfoExt
						.setAutoRecording(selected == 0 ? null : selected == 2);
				});
