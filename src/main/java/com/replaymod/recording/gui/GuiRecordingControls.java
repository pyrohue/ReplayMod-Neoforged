...
public class GuiRecordingControls extends EventRegistrations
{
	...
	private GuiButton buttonStartStop = new GuiButton(panel).onClick(() -> {
		...
		}else
		{
			...
			packetListener.addMarker(MarkerProcessor.MARKER_NAME_SPLIT,
				timestamp + 1);
		}
		stopped = !stopped;
		updateState();
	...
	{
		buttonPauseResume.setI18nLabel(
			"replaymod.gui.recording." + (paused ? "resume" : "pause"));
		buttonStartStop.setI18nLabel(
	...
	)
	{
		...
		Function<Integer, Integer> yPos = MCVer
			.findButton(buttonList, "menu.returnToMenu", 1).map(Optional::of)
