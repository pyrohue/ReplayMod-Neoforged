...
public class GuiRecordingOverlay extends EventRegistrations
{
	...
	public GuiRecordingOverlay(MinecraftClient mc,
		SettingsRegistry settingsRegistry, GuiRecordingControls guiControls)
	{
		this.mc = mc;
	 ...
	 */
	{
		on(RenderHudCallback.EVENT,
			(stack, partialTicks) -> renderRecordingIndicator(stack));
	}
	...
	private void renderRecordingIndicator(DrawContext stack)
	{
		...
		if(settingsRegistry.get(Setting.INDICATOR))
		{
			...
			String text =
				guiControls.isPaused() ? I18n.translate("replaymod.gui.paused")
					: I18n.translate("replaymod.gui.recording");
			MinecraftGuiRenderer renderer = new MinecraftGuiRenderer(stack);
			renderer.drawString(30, 18 - (fontRenderer.fontHeight / 2),
				0xffffffff, text.toUpperCase());
			renderer.bindTexture(TEXTURE);
			// #if MC<11700
			// $$ enableAlphaTest();
			// #endif
			renderer.drawTexturedRect(10, 10, 58, 20, 16, 16, 16, 16,
