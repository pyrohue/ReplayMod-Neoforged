...
/**
 * Records resource packs and handles incoming resource pack packets during
 * recording.
 */
public class ResourcePackRecorder
{
	...
	public void recordResourcePack(Path file, int requestId)
	{
		try
		{
			...
			boolean doWrite = false; // Whether we are the first and have to
										// write it
			synchronized(replayFile)
			{ // Need to read, modify and write the resource pack index
				// atomically
				...
				{
					// Hash is unknown, we have to write the resource pack
					// ourselves
					doWrite = true;
				}
				// Save this request
	...
	public synchronized ResourcePackSendS2CPacket handleResourcePack(
		ClientConnection netManager, ResourcePackSendS2CPacket packet)
	{
		final int requestId = nextRequestId++;
		mcIdToReplayId.put(packet.id(), requestId);
		return new ResourcePackSendS2CPacket(packet.id(),
			"replay://" + requestId, "", packet.required(), packet.prompt());
	}
	...
	public void recordResourcePack(Path file, UUID uuid)
	{
		...
		if(id == null)
		{
			logger.warn("Got resource pack download for unexpected uuid " + uuid
				+ " at " + file);
			return;
		}
	...
	// #elseif MC>=10800
	// $$ public ResourcePackStatusC2SPacket makeStatusPacket(String hash,
	// Status action) {
	// $$ //#if MC>=11002
	...
	// $$ public synchronized ResourcePackSendS2CPacket
	// handleResourcePack(ClientConnection netManager, ResourcePackSendS2CPacket
	// packet) {
	// $$ final int requestId = nextRequestId++;
	...
	// $$ if (serverData != null && serverData.getResourcePackPolicy() ==
	// ServerInfo.ResourcePackPolicy.ENABLED) {
	// $$ netManager.send(makeStatusPacket(hash, Status.ACCEPTED));
	// $$ downloadResourcePackFuture(netManager, requestId, url, hash);
	// $$ } else if (serverData != null && serverData.getResourcePackPolicy() !=
	// ServerInfo.ResourcePackPolicy.PROMPT) {
	// $$ netManager.send(makeStatusPacket(hash, Status.DECLINED));
	// $$ } else {
	// $$ // Lambdas MUST NOT be used with methods that need re-obfuscation in
	// FG prior to 2.2 (will result in AbstractMethodError)
	// $$ //#if MC>=11400
	// $$ mc.execute(() -> mc.setScreen(new ConfirmScreen(result -> {
	// $$ //#else
	// $$ //$$ //noinspection Convert2Lambda
	// $$ //$$ mc.addScheduledTask(() -> mc.displayGuiScreen(new GuiYesNo(new
	// GuiYesNoCallback() {
	// $$ //$$ @Override
	// $$ //$$ public void confirmClicked(boolean result, int id) {
	// $$ //#endif
	// $$ if (serverData != null) {
	// $$ serverData.setResourcePackPolicy(result ?
	// ServerInfo.ResourcePackPolicy.ENABLED :
	...
	// $$ //#if MC>=11400
	// $$ ,
	// net.minecraft.text.Text.translatable("multiplayer.texturePrompt.line1"),
	// net.minecraft.text.Text.translatable("multiplayer.texturePrompt.line2"))));
	// $$ //#else
	...
	// $$ }
	// $$ //$$
	// $$ private void downloadResourcePackFuture(ClientConnection connection,
	// int requestId, String url, final String hash) {
	// $$ addCallback(downloadResourcePack(requestId, url, hash),
	// $$ result -> connection.send(makeStatusPacket(hash,
	// Status.SUCCESSFULLY_LOADED)),
	// $$ throwable -> connection.send(makeStatusPacket(hash,
	...
	// $$ ServerResourcePackProvider packFinder =
	// mc.getServerResourcePackProvider();
	// $$ ((IDownloadingPackFinder) packFinder).setRequestCallback(file ->
	...
	// $$ private record ResourcePackSendS2CPacket(String s, String s1, Object
	// required, Object prompt) {
	// $$ }
	// #else
	// $$ public synchronized S3FPacketCustomPayload
	// handleResourcePack(S3FPacketCustomPayload packet) {
	// $$ final int requestId = nextRequestId++;
	// $$ final String url = new String(packet.func_149168_d(), Charsets.UTF_8);
	// $$
	// $$ ServerData serverData = mc.getCurrentServerData();
	// $$ ServerResourceMode resourceMode = serverData == null ?
	// ServerResourceMode.PROMPT : serverData.getResourceMode();
	...
	// $$ }
	// $$
	// $$ return new S3FPacketCustomPayload(packet.func_149169_c(), ("replay://"
	// + requestId).getBytes(Charsets.UTF_8));
	// $$ }
	// $$
	...
	// $$ final ResourcePackRepositoryAccessor acc =
	// (ResourcePackRepositoryAccessor) repo;
	// $$
	...
	// FG prior to 2.2 (will result in AbstractMethodError)
	// $$ //noinspection Convert2Lambda
	// $$ HttpUtil.downloadResourcePack(file, url, new
	...
	// $$ }
	// $$ }
	// $$ }, hashmap, 50*1024*1024, guiScreen,
