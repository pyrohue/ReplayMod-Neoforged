...
public class HotkeyButtons extends EventRegistrations implements Extra
{
	...
	{
		on(ReplayOpenedCallback.EVENT,
			replayHandler -> new Gui(mod, replayHandler.getOverlay()));
	}
	...
	public static final class Gui
	{
		...
		public Gui(ReplayMod mod, GuiReplayOverlay overlay)
		{
			...
			panel = new GuiPanel(overlay)
			{
				...
				{
					return open ? super.getChildren()
						: Collections.<GuiElement> emptyList();
				}

				@Override
				public Map<GuiElement, LayoutData> getElements()
				{
					return open ? super.getElements()
						: Collections.<GuiElement, LayoutData> emptyMap();
				}
			}.setLayout(panelLayout =
				new GridLayout().setSpacingX(5).setSpacingY(5).setColumns(1));

			final KeyBindingRegistry keyBindingRegistry =
				mod.getKeyBindingRegistry();
			keyBindingRegistry.getBindings().values().stream()
				...
				.forEachOrdered(keyBinding -> {
					GuiButton button = new GuiButton()
					{
						...
						{
							// There doesn't seem to be an KeyBindingUpdate
							// event, so we'll just update it every time
							setLabel(keyBinding.isBound()
								? keyBinding.getBoundKey() : "");

							if(keyBinding.supportsAutoActivation())
							{
								setTooltip(new GuiTooltip()
									.setText(new String[]{I18n.translate(
										"replaymod.gui.ingame.autoactivating"),
										I18n.translate(
											"replaymod.gui.ingame.autoactivating."
												+ (keyBinding.isAutoActivating()
													? "disable"
													: "enable")),}));
								setLabelColor(keyBinding.isAutoActivating()
									? 0x00ff00 : 0xe0e0e0);
							}
					...
					panel.addElements(null,
						new GuiPanel().setLayout(new CustomLayout<GuiPanel>()
						{
							...
							{
								width(button,
									Math.max(10 /* consistent min width */,
										width(button)) + 10 /* padding */);
								height(button, 20);
								...
								{
									height(label, height(label) * 2); // split
																		// over
																		// two
																		// lines
								}
			...
			overlay.setLayout(
				new CustomLayout<GuiReplayOverlay>(overlay.getLayout())
				{
					@Override
