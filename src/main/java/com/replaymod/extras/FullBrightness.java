...
public class FullBrightness extends EventRegistrations implements Extra
{
	...
	private final IGuiImage indicator = new GuiImage()
		.setTexture(ReplayMod.TEXTURE, 90, 20, 19, 16).setSize(19, 16);
	...
	public void register(final ReplayMod mod) throws Exception
	{
		...
		mod.getKeyBindingRegistry().registerKeyBinding(
			"replaymod.input.lighting", Keyboard.KEY_Z, new Runnable()
			{
				@Override
				public void run()
				{
					active = !active;
					// need to tick once to update lightmap when replay is
					// paused
					// #if MC>=11400
	...
	private void preRender()
	{
		if(active && module.getReplayHandler() != null)
		{
			...
			if(type == Type.Gamma || type == Type.Both)
			{
				...
				((com.replaymod.core.mixin.SimpleOptionAccessor<Double>)(Object)mc.options
					.getGamma()).setRawValue(1000.0);
			}
	...
	private void postRender()
	{
		if(active && module.getReplayHandler() != null)
		{
			...
			{
				((com.replaymod.core.mixin.SimpleOptionAccessor<Double>)(Object)mc.options
					.getGamma()).setRawValue(originalGamma);
			}
			if(type == Type.NightVision || type == Type.Both)
			{
				if(mc.player != null)
...

	{
		on(ReplayOpenedCallback.EVENT,
			replayHandler -> updateIndicator(replayHandler.getOverlay()));
	}
	...
	private void updateIndicator(GuiReplayOverlay overlay)
	{
		if(active)
		{
			overlay.statusIndicatorPanel
				.addElements(new HorizontalLayout.Data(1), indicator);
		}else
		{
	...
	enum Type
	{
		...
		{
			return "replaymod.gui.settings.fullbrightness."
				+ name().toLowerCase();
		}
	}
}
