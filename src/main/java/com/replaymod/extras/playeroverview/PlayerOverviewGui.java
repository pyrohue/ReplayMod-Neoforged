...
public class PlayerOverviewGui extends GuiScreen implements Closeable
{
	...
	public final GuiCheckbox saveCheckbox = new GuiCheckbox(contentPanel)
		.setTooltip(new GuiTooltip().setI18nText(
			"replaymod.gui.playeroverview.remembersettings.description"))
	...
	public final GuiCheckbox checkAll = new GuiCheckbox(contentPanel)
	{
		...
		public void onClick()
		{
			playersScrollable.invokeAll(IGuiCheckbox.class,
				e -> e.setChecked(true));
		}
	}.setLabel("").setChecked(true).setTooltip(
		new GuiTooltip().setI18nText("replaymod.gui.playeroverview.showall"));
	...
	}.setLabel("").setChecked(false).setTooltip(
		new GuiTooltip().setI18nText("replaymod.gui.playeroverview.hideall"));

	{
		...
		contentPanel.setLayout(new CustomLayout<GuiPanel>()
		{
			...
			protected void layout(GuiPanel container, int width, int height)
			{
				...
				pos(playersScrollable, 10,
					y(spectateLabel) + height(spectateLabel) + 5);
				size(playersScrollable, width - 10 - 5,
					height - 15 - height(saveCheckbox) - y(playersScrollable));
				pos(saveCheckbox, 10, height - 10 - height(saveCheckbox));
				pos(uncheckAll, width - width(uncheckAll) - 8,
					height - height(uncheckAll) - 10);
				pos(checkAll, x(uncheckAll) - 3 - width(checkAll),
					y(uncheckAll));
			}
		});
	}
	...
	public PlayerOverviewGui(final PlayerOverview extra,
		List<PlayerEntity> players)
	{
		this.extra = extra;

		Collections.sort(players, new PlayerComparator()); // Sort by name,
															// spectators last
		for(final PlayerEntity p : players)
		{
			...
			final Identifier texture =
				((AbstractClientPlayerEntity)p).getSkinTextures().texture();
			final GuiClickable panel = new GuiClickable()
				.setLayout(new HorizontalLayout().setSpacing(2))
				.addElements(new HorizontalLayout.Data(0.5), new GuiImage()
				{
					@Override
					public void draw(GuiRenderer renderer,
						ReadableDimension size, RenderInfo renderInfo)
					{
						renderer.bindTexture(texture);
						renderer.drawTexturedRect(0, 0, 8, 8, 16, 16, 8, 8, 64,
							64);
						// #if MC>=10809
						if(p.isPartVisible(PlayerModelPart.HAT))
						{
							...
							// #endif
							// #endif
							renderer.drawTexturedRect(0, 0, 40, 8,
								size.getWidth(), size.getHeight(), 8, 8, 64,
								64);
						}
					}
				}.setSize(16, 16), new GuiLabel().setText(
					// #if MC>=11400
					p.getName().getString()
				// #else
				// #if MC>=10800
			...
			new GuiPanel(playersScrollable.getListPanel())
				.setLayout(new CustomLayout<GuiPanel>()
				{
					@Override
					protected void layout(GuiPanel container, int width,
						int height)
					{
						pos(panel, 5, 0);
						pos(checkbox, width - width(checkbox) - 5,
							height / 2 - height(checkbox) / 2);
					}
					...
					{
						return new Dimension(ENTRY_WIDTH,
							panel.getMinSize().getHeight());
					}
	...
	private static boolean isSpectator(PlayerEntity e)
	{
		...
		return e.isInvisible()
			&& e.getStatusEffect(StatusEffects.INVISIBILITY) == null;
		// #else
		// $$ return e.isInvisible() &&
		// e.getActivePotionEffect(Potion.invisibility) == null;
		// #endif
	}

	private static final class PlayerComparator
		implements Comparator<PlayerEntity>
	{
		@Override
		public int compare(PlayerEntity o1, PlayerEntity o2)
		{
			...
			if(isSpectator(o2) && !isSpectator(o1))
				return -1;
			// #if MC>=11400
			return o1.getName().getString()
				.compareToIgnoreCase(o2.getName().getString());
			// #else
			// #if MC>=10800
