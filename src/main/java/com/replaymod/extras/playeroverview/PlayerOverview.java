...
public class PlayerOverview extends EventRegistrations implements Extra
{
	...
	public void register(final ReplayMod mod) throws Exception
	{
		...
		mod.getKeyBindingRegistry().registerKeyBinding(
			"replaymod.input.playeroverview", Keyboard.KEY_B, new Runnable()
			{
				@Override
				public void run()
				{
					...
					{
						// #if MC>=11400
						List<PlayerEntity> players = mod.getMinecraft().world
							.getPlayers().stream().map(it -> (PlayerEntity)it)
						...
						// mod.getMinecraft().world.getPlayers(EntityPlayer.class,
						// new Predicate() {
						// $$ @Override
						// $$ public boolean apply(Object input) {
						// $$ return !(input instanceof CameraEntity); //
						// Exclude the camera entity
						// $$ }
						// $$ });
						// #else
						// $$ List<EntityPlayer> players =
						// mod.getMinecraft().theWorld.playerEntities;
						...
						if(!Utils.isCtrlDown())
						{
							// Hide all players that have an UUID v2 (commonly
							// used for NPCs)
							Iterator<PlayerEntity> iter = players.iterator();
							while(iter.hasNext())
							{
								UUID uuid =
									iter.next().getGameProfile().getId();
						...
						new PlayerOverviewGui(PlayerOverview.this, players)
							.display();
					}
				}
			}, true);
	...
	private void onReplayOpen(ReplayHandler replayHandler) throws IOException
	{
		Optional<Set<UUID>> savedData =
			replayHandler.getReplayFile().getInvisiblePlayers();
	...
	public void saveHiddenPlayers()
	{
		if(savingEnabled)
		{
			try
			{
				module.getReplayHandler().getReplayFile()
					.writeInvisiblePlayers(hiddenPlayers);
			}catch(IOException e)
			{
				e.printStackTrace();
			}
