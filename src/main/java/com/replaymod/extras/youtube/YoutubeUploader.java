...
public class YoutubeUploader
{
	private static final String CLIENT_ID =
		"743126594724-mfe7pj1k7e47uu5pk4503c8st9vj9ibu.apps.googleusercontent.com";
	private static final String CLIENT_SECRET = "gMwcy3mRYCRamCIjJIYP7rqc";
	private static final String FFMPEG_MP4 =
		"-i %s -c:v libx264 -preset slow -crf 16 %s";
	...
	public YoutubeUploader(MinecraftClient minecraft, File videoFile,
		int videoFrames, String thumbnailFormat, byte[] thumbnailImage,
		RenderSettings settings, VideoVisibility videoVisibility,
		VideoSnippet videoSnippet) throws GeneralSecurityException, IOException
	{
		...
		this.dataStoreFactory =
			new FileDataStoreFactory(minecraft.runDirectory);
	...
	private File preUpload() throws InterruptedException, IOException
	{
		...
		if(settings.getRenderMethod().isSpherical())
		{
			...
			boolean isMp4 = Files.getFileExtension(outputFile.getName())
				.equalsIgnoreCase("mp4");

			if(!isMp4)
			{
				// convert non-mp4 videos into mp4 format to be able to inject
				// metadata
				outputFile = new File(outputFile.getParentFile(),
					System.currentTimeMillis() + ".mp4");
				outputFile.deleteOnExit();

				String args = String.format(FFMPEG_MP4, videoFile.getName(),
					outputFile.getName());

				CommandLine commandLine =
					new CommandLine(settings.getExportCommandOrDefault());
				commandLine.addArguments(args);
				LOGGER.info("Re-encoding for metadata injection with {} {}",
					commandLine.getExecutable(), args);
				Process process = new ProcessBuilder(commandLine.toStrings())
					.directory(outputFile.getParentFile()).start();
			...
			if(!settings.isInjectSphericalMetadata() || !isMp4)
			{
				MetadataInjector.injectMetadata(settings.getRenderMethod(),
					outputFile, settings.getTargetVideoWidth(),
					settings.getTargetVideoHeight(),
	...
	private Video doUpload(YouTube youTube, File processedFile)
		throws IOException
	{
		...
		try(InputStream inputStream =
			new BufferedInputStream(new FileInputStream(processedFile)))
		{
			InputStreamContent content =
				new InputStreamContent("video/*", inputStream);
			content.setLength(processedFile.length());
			YouTube.Videos.Insert videoInsert = youTube.videos()
				.insert("snippet,statistics,status", video, content);
			final MediaHttpUploader uploader =
	...
	private void doThumbUpload(YouTube youTube, Video video) throws IOException
	{
		...
		{
			InputStreamContent content =
				new InputStreamContent("image/" + thumbnailFormat, inputStream);
		...
		}catch(GoogleJsonResponseException e)
		{
			...
			if("Authorization".equals(info.getLocation())
				&& "forbidden".equals(info.getReason()))
			{
				e.printStackTrace();
