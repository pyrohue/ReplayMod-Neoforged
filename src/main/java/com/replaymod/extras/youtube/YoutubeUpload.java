...
public class YoutubeUpload extends EventRegistrations implements Extra
{
	...
	{
		AbstractGuiScreen<?> abstractScreen =
			de.johni0702.minecraft.gui.container.GuiScreen.from(vanillaGui);
		if(abstractScreen instanceof GuiRenderingDone)
		{
			...
			if(gui.actionsPanel.getChildren().stream()
				.anyMatch(it -> it instanceof YoutubeButton))
			{
				return; // Button already added
			}
			// Add the Upload to YouTube button to actions panel
			gui.actionsPanel.addElements(null, new YoutubeButton()
				.onClick(() -> new GuiYoutubeUpload(gui, gui.videoFile,
					gui.videoFrames, gui.settings).display())
