...
public class GuiYoutubeUpload extends GuiScreen
{
		...
		public void run()
		{
			...
			if(upload == null)
			{
				...
				}else
				{
					...
					uploadButton
						.setTooltip(new GuiTooltip().setI18nText(problem));
				}
			}
		}
	};
	...
	public final GuiTextField nameField =
		new GuiTextField().setI18nHint("replaymod.gui.videotitle")
			.onTextChanged(s -> inputValidation.run());

	public final GuiTextArea descriptionField = new GuiTextArea()
		.setMaxCharCount(Integer.MAX_VALUE).setMaxTextWidth(Integer.MAX_VALUE)
		.setMaxTextHeight(Integer.MAX_VALUE);
	{
		descriptionField.setText(
			new String[]{I18n.translate("replaymod.gui.videodescription")});
	}

	public final GuiTextField tagsField =
		new GuiTextField().setI18nHint("replaymod.gui.videotags");

	{
		nameField.setNext(descriptionField).getNext().setNext(tagsField)
	...
	public final GuiPanel leftPanel =
		...
		}).addElements(null, nameField, descriptionField, tagsField,
			progressBar);

	public final GuiDropdownMenu<VideoVisibility> visibilityDropdown =
		new GuiDropdownMenu<VideoVisibility>().setSize(200, 20)
			.setValues(VideoVisibility.values())
			.setSelected(VideoVisibility.PUBLIC);
	...
	public final GuiButton thumbnailButton =
		new GuiButton().onClick(new Runnable()
		{
			...
			public void run()
			{
				GuiFileChooserPopup.openLoadGui(GuiYoutubeUpload.this,
					"replaymod.gui.load", ImageIO.getReaderFileSuffixes())
					.onAccept(file -> {
						...
						try
						{
							...
							ImageReader reader =
								ImageIO.getImageReaders(in).next();
							thumbnailFormat =
								reader.getFormatName().toLowerCase();
							image = Image
								.read(new ByteArrayInputStream(thumbnailImage));
						}catch(Throwable t)
						{
							t.printStackTrace();
							thumbnailImage = null;
	...
	public final GuiImage thumbnail =
		new GuiImage().setSize(200, 112).setTexture(Utils.DEFAULT_THUMBNAIL);

	public final GuiButton uploadButton = new GuiButton(this).setSize(98, 20);

	public final GuiButton closeButton =
		new GuiButton(this).onClick(new Runnable()
		{
			@Override
			public void run()
			{
	...
	public GuiYoutubeUpload(GuiScreen previousScreen, File videoFile,
		int videoFrames, RenderSettings settings)
	{
			...
			uploadButton.onClick(() -> {
				try
				{
					...
					VideoVisibility visibility =
						visibilityDropdown.getSelectedValue();
					VideoSnippet snippet = new VideoSnippet();
					snippet.setTitle(nameField.getText());
					snippet.setDescription(
						Strings.join(descriptionField.getText(), "\n"));
					snippet.setTags(asList(tagsField.getText().split(",")));
					upload = new YoutubeUploader(getMinecraft(), videoFile,
						videoFrames, thumbnailFormat, thumbnailImage, settings,
						visibility, snippet);
					...
					Futures.addCallback(future, new FutureCallback<Video>()
					{
						...
						public void onSuccess(Video result)
						{
							...
							{
								LOGGER.error("Failed to open video URL \"{}\":",
									url, throwable);
							}
							upload = null;
							progressBar.setLabel(I18n.translate(
								"replaymod.gui.ytuploadprogress.done", url));
							setState(false);
						}
		...
		RenderInfo renderInfo)
	{
		if(upload != null && upload.getState() != null)
		{
			...
			progressBar.setI18nLabel("replaymod.gui.ytuploadprogress."
				+ upload.getState().name().toLowerCase());
		}
