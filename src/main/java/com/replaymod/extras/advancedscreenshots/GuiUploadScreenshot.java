...
public class GuiUploadScreenshot extends AbstractGuiPopup<GuiUploadScreenshot>
{
	...
	public final GuiLabel veerLabel = new GuiLabel()
		.setI18nText(
			"replaymod.gui.advancedscreenshots.finished.description.veer")
		.setColor(ReadableColor.BLACK);

	public final GuiButton veerUploadButton = new GuiButton().setSize(150, 20)
		.setI18nLabel("replaymod.gui.advancedscreenshots.finished.upload.veer");

	public final GuiButton showOnDiskButton = new GuiButton().setSize(150, 20)
		.setI18nLabel("replaymod.gui.advancedscreenshots.finished.showfile");

	public final GuiButton closeButton =
		new GuiButton().setSize(150, 20).setI18nLabel("replaymod.gui.close");

	public final GuiCheckbox neverOpenCheckbox = new GuiCheckbox();

	public final GuiLabel neverOpenLabel = new GuiLabel()
		.setI18nText("replaymod.gui.notagain").setColor(ReadableColor.BLACK);

	public final GuiPanel checkboxPanel = GuiPanel.builder()
		.layout(new HorizontalLayout(HorizontalLayout.Alignment.RIGHT)
			.setSpacing(5))
		.with(neverOpenCheckbox, new HorizontalLayout.Data(0.5))
		.with(neverOpenLabel, new HorizontalLayout.Data(0.5)).build();

	public GuiUploadScreenshot(GuiContainer container, ReplayMod mod,
		RenderSettings renderSettings)
	{
		...
		boolean veer = renderSettings
			.getRenderMethod() == RenderSettings.RenderMethod.EQUIRECTANGULAR;

		if(renderSettings
			.getRenderMethod() == RenderSettings.RenderMethod.EQUIRECTANGULAR)
		{
			successLabel.setI18nText(
				"replaymod.gui.advancedscreenshots.finished.description.360");
		}
		...
		showOnDiskButton.onClick(() -> MCVer
			.openFile(renderSettings.getOutputFile().getParentFile()));
		...
		if(veer)
		{
			popup.addElements(new VerticalLayout.Data(0.5), veerLabel,
				veerUploadButton);
		}

		popup.addElements(new VerticalLayout.Data(0.5), successLabel,
			showOnDiskButton, closeButton);

		popup.addElements(new VerticalLayout.Data(1), checkboxPanel);
